...
pub fn CONNECT(cn: *Core, allocator: Allocator, co: protocol.ConnectOpts) !void {
    cn.mutex.lock();
...
pub fn DISCONNECT(cn: *Core) void {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn PUB(cn: *Core, subject: []const u8, reply2: ?[]const u8, payload: ?[]const u8) !void {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn HPUB(cn: *Core, subject: []const u8, reply2: ?[]const u8, headers: *Headers, payload: ?[]const u8) !void {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn SUB(cn: *Core, subject: []const u8, queue_group: ?[]const u8, sid: []const u8) !void {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
///
/// UNSUB <sid>  [max_msgs]␍␊
pub fn UNSUB(cn: *Core, sid: []const u8, max_msgs: ?u32) !void {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn FETCH(cn: *Core, timeout_ns: u64) !?*AllocatedMSG {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn REQUEST(cn: *Core, subject: []const u8, payload: ?[]const u8, timeout_ns: u64) !*AllocatedMSG {
    cn.mutex.lock();
    defer cn.mutex.unlock();
...
pub fn REUSE(cn: *Core, msg: *AllocatedMSG) void {
    cn.reuse(msg);
}
...
fn _pub(cn: *Core, subject: []const u8, reply2: ?[]const u8, payload: ?[]const u8) !void {
...
fn hpub(cn: *Core, subject: []const u8, reply2: ?[]const u8, headers: *Headers, payload: ?[]const u8) !void {
    var repl: []const u8 = undefined;
...
fn sub(cn: *Core, subject: []const u8, queue_group: ?[]const u8, sid: []const u8) !void {
...
fn unsub(cn: *Core, sid: []const u8, max_msgs: ?u32) !void {
...
fn _unsub_(cn: *Core, sid: []const u8) void {
    if (cn.unsub(sid, 0)) |_| {
        return;
    } else |_| {
        return;
...
fn ping(cn: *Core) !void {
    try cn.write("PING\r\n");
}

fn pong(cn: *Core) !void {
    try cn.write("PONG\r\n");
}

// Writes the formatted output to underlying stream.
fn print(cn: *Core, comptime fmt: []const u8, args: anytype) !void {
    try cn._format(fmt, args, &cn.printbuf);
    try cn.connection.?.writeAll(cn.printbuf.body().?);
}

fn _format(cn: *Core, comptime fmt: []const u8, args: anytype, fbuff: *Appendable) !void {
    while (true) {
...
fn _tryformat(cn: *Core, comptime fmt: []const u8, args: anytype, fbuff: *Appendable) !void {
    cn.*.fbs.reset();
...
fn write(cn: *Core, buffer: []const u8) !void {
    if (cn.connection == null) {
...
fn writev(cn: *Core, iovecs: []posix.iovec_const) !void {
    if (cn.connection == null) {
        return ReturnedError.CommunicationFailure;
    }
    if (iovecs.len == 0) {
...
fn read_msg(cn: *Core) !?*AllocatedMSG {
    if (cn.read_mt()) |mt| {
        switch (mt) {
            .UNKNOWN => {
                return ReturnedError.CommunicationFailure;
...
fn read_oneliner(cn: *Core, mt: MT) !?*AllocatedMSG {
...
//
// TOT_LEN the payload length plus the HDR_LEN
fn read_HMSG(cn: *Core) !?*AllocatedMSG {
    const recvd = cn.line.body().?;
...
fn read_buffer(cn: *Core, buffer: *Appendable, len: usize) !void {
    if (cn.wasRaised()) {
        return error.WasCancelled;
    }
...
fn connectTcp(client: *Core, host: []const u8, port: u16) !*Stream {
...
fn raiseAttention(cn: *Core) void {
    _ = cn.attention.post();
}

inline fn wasRaised(cn: *Core) bool {
...
fn waitFinish(cn: *Core) void {
    cn.thread.join();
}

inline fn setTimeOut(cn: *Core) !void {
...
inline fn resetTimeOut(cn: *Core) !void {
    const timeout = posix.timeval{ .sec = 0, .usec = 0 };
    try posix.setsockopt(cn.connection.?.handle, posix.SOL.SOCKET, posix.SO.RCVTIMEO, &std.mem.toBytes(timeout));
}

fn request(cn: *Core, subject: []const u8, payload: ?[]const u8, timeout_ns: u64) !*AllocatedMSG {
...
fn waitResponse(cn: *Core, timeout_ns: u64) !*AllocatedMSG {
    for (0..10) |_| {
...
fn read_PUB(cn: *Core) !?*AllocatedMSG {
    _ = cn;
    return error.NotImplemented;
}

fn read_HPUB(cn: *Core) !?*AllocatedMSG {
    _ = cn;
    return error.NotImplemented;
}

...
pub export fn natsHash_Create(arg_newHash: [*c][*c]natsHash, arg_initialSize: c_int) natsStatus {
    var newHash = arg_newHash;
    _ = &newHash;
    var initialSize = arg_initialSize;
    _ = &initialSize;
    var hash: [*c]natsHash = null;
    _ = &hash;
    if (initialSize <= @as(c_int, 0)) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHash_Create", @as(c_int, 41), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    if ((initialSize & (initialSize - @as(c_int, 1))) != @as(c_int, 0)) {
        initialSize -= 1;
        initialSize |= initialSize >> @intCast(1);
        initialSize |= initialSize >> @intCast(2);
        initialSize |= initialSize >> @intCast(4);
        initialSize |= initialSize >> @intCast(8);
        initialSize |= initialSize >> @intCast(16);
        initialSize += 1;
    }
    hash = @as([*c]natsHash, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsHash)))));
    if (hash == @as([*c]natsHash, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHash_Create", @as(c_int, 57), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    hash.*.mask = initialSize - @as(c_int, 1);
    hash.*.numBkts = initialSize;
    hash.*.canResize = @as(c_int, 1) != 0;
    hash.*.bkts = @as([*c][*c]natsHashEntry, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, initialSize))), @sizeOf([*c]natsHashEntry)))));
    if (hash.*.bkts == @as([*c][*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        free(@as(?*anyopaque, @ptrCast(hash)));
        return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHash_Create", @as(c_int, 66), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    newHash.* = hash;
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsHash_Set(arg_hash: [*c]natsHash, arg_key: i64, arg_data: ?*anyopaque, arg_oldData: [*c]?*anyopaque) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var data = arg_data;
    _ = &data;
    var oldData = arg_oldData;
    _ = &oldData;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var index_1: c_int = @as(c_int, @bitCast(@as(c_int, @truncate(key & @as(i64, @bitCast(@as(c_long, hash.*.mask)))))));
    _ = &index_1;
    var newEntry: [*c]natsHashEntry = null;
    _ = &newEntry;
    var e: [*c]natsHashEntry = undefined;
    _ = &e;
    if (oldData != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        oldData.* = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    }
    e = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    while (e != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if (e.*.key == key) {
            if (oldData != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                oldData.* = e.*.data;
            }
    ...
    if (newEntry == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHash_Set", @as(c_int, 175), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    newEntry.*.next = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    ...
    hash.*.used += 1;
    if ((@as(c_int, @intFromBool(hash.*.canResize)) != 0) and (hash.*.used > hash.*.numBkts)) {
        s = _grow(hash);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsHash_Set");
}
pub export fn natsHash_Get(arg_hash: [*c]natsHash, arg_key: i64) ?*anyopaque {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    ...
    _ = &e;
    e = (blk: {
        const tmp = key & @as(i64, @bitCast(@as(c_long, hash.*.mask)));
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    while (e != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub export fn natsHash_Remove(arg_hash: [*c]natsHash, arg_key: i64) ?*anyopaque {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var entryRemoved: [*c]natsHashEntry = null;
    _ = &entryRemoved;
    var dataRemoved: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &dataRemoved;
    var e: [*c][*c]natsHashEntry = undefined;
    _ = &e;
    e = &(blk: {
        const tmp = key & @as(i64, @bitCast(@as(c_long, hash.*.mask)));
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    ...
    while (e.* != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if (e.*.*.key == key) {
            ...
            e.* = entryRemoved.*.next;
            {
                free(@as(?*anyopaque, @ptrCast(entryRemoved)));
                entryRemoved = null;
            }
            hash.*.used -= 1;
            _maybeShrink(hash);
        ...
        e = @as([*c][*c]natsHashEntry, @ptrCast(@alignCast(&e.*.*.next)));
    }
    return dataRemoved;
}
pub export fn natsHash_RemoveSingle(arg_hash: [*c]natsHash, arg_key: [*c]i64, arg_data: [*c]?*anyopaque) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var data = arg_data;
    _ = &data;
    var e: [*c]natsHashEntry = null;
    _ = &e;
    var i: c_int = undefined;
    _ = &i;
    if (hash.*.used != @as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHash_RemoveSingle", @as(c_int, 256), @as(c_uint, @bitCast(NATS_ERR)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_ERR))));
    {
        i = 0;
        while (i < hash.*.numBkts) : (i += 1) {
            e = (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            if (e != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                if (key != @as([*c]i64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    key.* = e.*.key;
                }
                if (data != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    data.* = e.*.data;
                }
                {
...
pub export fn natsHash_Destroy(arg_hash: [*c]natsHash) void {
    var hash = arg_hash;
    _ = &hash;
    var e: [*c]natsHashEntry = undefined;
    _ = &e;
    var ne: [*c]natsHashEntry = undefined;
    _ = &ne;
    var i: c_int = undefined;
    _ = &i;
    if (hash == @as([*c]natsHash, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    {
        i = 0;
        while (i < hash.*.numBkts) : (i += 1) {
            e = (blk: {
    ...
    free(@as(?*anyopaque, @ptrCast(hash.*.bkts)));
    free(@as(?*anyopaque, @ptrCast(hash)));
}
pub export fn natsHashIter_Init(arg_iter: [*c]natsHashIter, arg_hash: [*c]natsHash) void {
    var iter = arg_iter;
    _ = &iter;
    var hash = arg_hash;
    _ = &hash;
    _ = memset(@as(?*anyopaque, @ptrCast(iter)), @as(c_int, 0), @sizeOf(natsHashIter));
    hash.*.canResize = @as(c_int, 0) != 0;
    iter.*.hash = hash;
    iter.*.current = hash.*.bkts[@as(c_uint, @intCast(@as(c_int, 0)))];
    iter.*.next = iter.*.current;
}
pub export fn natsHashIter_Next(arg_iter: [*c]natsHashIter, arg_key: [*c]i64, arg_value: [*c]?*anyopaque) bool {
    var iter = arg_iter;
    _ = &iter;
    var key = arg_key;
    _ = &key;
    var value = arg_value;
    _ = &value;
    if ((@as(c_int, @intFromBool(iter.*.started)) != 0) and (iter.*.next == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return @as(c_int, 0) != 0;
    if (!iter.*.started and (iter.*.current == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        while ((iter.*.next == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (iter.*.currBkt < (iter.*.hash.*.numBkts - @as(c_int, 1)))) {
            iter.*.next = (blk: {
                const tmp = blk_1: {
                    const ref = &iter.*.currBkt;
                    ref.* += 1;
                    break :blk_1 ref.*;
                };
                if (tmp >= 0) break :blk iter.*.hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk iter.*.hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
        }
        if (iter.*.next == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            iter.*.started = @as(c_int, 1) != 0;
            return @as(c_int, 0) != 0;
        }
    }
    iter.*.started = @as(c_int, 1) != 0;
    iter.*.current = iter.*.next;
    if (iter.*.current != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if (key != @as([*c]i64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            key.* = iter.*.current.*.key;
        }
        if (value != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub export fn natsHashIter_RemoveCurrent(arg_iter: [*c]natsHashIter) natsStatus {
    var iter = arg_iter;
    _ = &iter;
    var key: i64 = undefined;
    _ = &key;
    if (iter.*.current == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsHashIter_RemoveCurrent", @as(c_int, 367), @as(c_uint, @bitCast(NATS_NOT_FOUND)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NOT_FOUND))));
    key = iter.*.current.*.key;
    iter.*.current = iter.*.next;
    _ = natsHash_Remove(iter.*.hash, key);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsHashIter_Done(arg_iter: [*c]natsHashIter) void {
    var iter = arg_iter;
    _ = &iter;
    iter.*.hash.*.canResize = @as(c_int, 1) != 0;
}
pub export fn natsStrHash_Create(arg_newHash: [*c][*c]natsStrHash, arg_initialSize: c_int) natsStatus {
    var newHash = arg_newHash;
    _ = &newHash;
    var initialSize = arg_initialSize;
    _ = &initialSize;
    var hash: [*c]natsStrHash = null;
    _ = &hash;
    if (initialSize <= @as(c_int, 0)) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_Create", @as(c_int, 429), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    if ((initialSize & (initialSize - @as(c_int, 1))) != @as(c_int, 0)) {
        initialSize -= 1;
        initialSize |= initialSize >> @intCast(1);
        initialSize |= initialSize >> @intCast(2);
    ...
    hash = @as([*c]natsStrHash, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsStrHash)))));
    if (hash == @as([*c]natsStrHash, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_Create", @as(c_int, 445), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    hash.*.mask = initialSize - @as(c_int, 1);
    hash.*.numBkts = initialSize;
    hash.*.canResize = @as(c_int, 1) != 0;
    hash.*.bkts = @as([*c][*c]natsStrHashEntry, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, initialSize))), @sizeOf([*c]natsStrHashEntry)))));
    if (hash.*.bkts == @as([*c][*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        free(@as(?*anyopaque, @ptrCast(hash)));
        return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_Create", @as(c_int, 454), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    newHash.* = hash;
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsStrHash_Hash(arg_data: [*c]const u8, arg_dataLen: c_int) u32 {
    var data = arg_data;
    _ = &data;
    ...
    var i: c_int = 0;
    _ = &i;
    var dlen: c_int = dataLen;
    _ = &dlen;
    var h32: u32 = @as(u32, @bitCast(@as(c_int, @truncate(@as(c_long, 2166136261)))));
    _ = &h32;
    var k1: u64 = undefined;
    _ = &k1;
    var k2: u64 = undefined;
    _ = &k2;
    var k3: u32 = undefined;
    _ = &k3;
    while (dlen >= _DDWSZ) : (dlen -= _DDWSZ) {
        _ = memcpy(@as(?*anyopaque, @ptrCast(&k1)), @as(?*const anyopaque, @ptrCast(&(blk: {
            const tmp = i;
            if (tmp >= 0) break :blk data + @as(usize, @intCast(tmp)) else break :blk data - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*)), @sizeOf(u64));
        _ = memcpy(@as(?*anyopaque, @ptrCast(&k2)), @as(?*const anyopaque, @ptrCast(&(blk: {
            const tmp = i + @as(c_int, 4);
            if (tmp >= 0) break :blk data + @as(usize, @intCast(tmp)) else break :blk data - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*)), @sizeOf(u64));
        h32 = @as(u32, @bitCast(@as(c_uint, @truncate((@as(u64, @bitCast(@as(c_ulong, h32))) ^ (((k1 << @intCast(5)) | (k1 >> @intCast(27))) ^ k2)) *% @as(u64, @bitCast(@as(c_long, @as(c_int, 709607))))))));
        i += _DDWSZ;
    }
    if ((dlen & _DWSZ) != @as(c_int, 0)) {
        _ = memcpy(@as(?*anyopaque, @ptrCast(&k1)), @as(?*const anyopaque, @ptrCast(&(blk: {
            const tmp = i;
            if (tmp >= 0) break :blk data + @as(usize, @intCast(tmp)) else break :blk data - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*)), @sizeOf(u64));
        h32 = @as(u32, @bitCast(@as(c_uint, @truncate((@as(u64, @bitCast(@as(c_ulong, h32))) ^ k1) *% @as(u64, @bitCast(@as(c_long, @as(c_int, 709607))))))));
        i += _DWSZ;
    }
    if ((dlen & @as(c_int, 4)) != @as(c_int, 0)) {
        _ = memcpy(@as(?*anyopaque, @ptrCast(&k3)), @as(?*const anyopaque, @ptrCast(&(blk: {
            const tmp = i;
            if (tmp >= 0) break :blk data + @as(usize, @intCast(tmp)) else break :blk data - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*)), @sizeOf(u32));
        h32 = @as(u32, @bitCast(@as(c_uint, @truncate((@as(u64, @bitCast(@as(c_ulong, h32))) ^ @as(u64, @bitCast(@as(c_ulong, k3)))) *% @as(u64, @bitCast(@as(c_long, @as(c_int, 709607))))))));
        i += @as(c_int, 4);
    }
    if ((dlen & @as(c_int, 1)) != @as(c_int, 0)) {
        h32 = (h32 ^ @as(u32, @bitCast(@as(c_uint, (blk: {
            const tmp = i;
            if (tmp >= 0) break :blk data + @as(usize, @intCast(tmp)) else break :blk data - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*)))) *% @as(u32, @bitCast(@as(c_int, 709607)));
    }
    return h32 ^ (h32 >> @intCast(16));
}
pub export fn natsStrHash_SetEx(arg_hash: [*c]natsStrHash, arg_key: [*c]u8, arg_copyKey: bool, arg_freeKey: bool, arg_data: ?*anyopaque, arg_oldData: [*c]?*anyopaque) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var copyKey = arg_copyKey;
    _ = &copyKey;
    var freeKey = arg_freeKey;
    _ = &freeKey;
    var data = arg_data;
    _ = &data;
    var oldData = arg_oldData;
    ...
    var index_1: c_int = 0;
    _ = &index_1;
    var newEntry: [*c]natsStrHashEntry = null;
    _ = &newEntry;
    var e: [*c]natsStrHashEntry = undefined;
    _ = &e;
    var oldKey: [*c]u8 = undefined;
    _ = &oldKey;
    if (oldData != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        oldData.* = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    }
    hk = natsStrHash_Hash(key, @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(key))))));
    index_1 = @as(c_int, @bitCast(hk & @as(u32, @bitCast(hash.*.mask))));
    e = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    while (e != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if ((e.*.hk == hk) and (strcmp(e.*.key, key) == @as(c_int, 0))) {
            if (oldData != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                oldData.* = e.*.data;
            }
            e.*.data = data;
            if ((key == e.*.key) and (@as(c_int, @intFromBool(freeKey)) == @as(c_int, @intFromBool(e.*.freeKey)))) return @as(c_uint, @bitCast(NATS_OK));
            oldKey = e.*.key;
            if (copyKey) {
                var newKey: [*c]u8 = strdup(key);
                _ = &newKey;
                if (newKey == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_SetEx", @as(c_int, 588), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
                e.*.key = newKey;
            }
            if (e.*.freeKey) {
                free(@as(?*anyopaque, @ptrCast(oldKey)));
            }
            e.*.freeKey = freeKey;
            return @as(c_uint, @bitCast(NATS_OK));
        }
    ...
    if (newEntry == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_SetEx", @as(c_int, 608), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    newEntry.*.next = (blk: {
        const tmp = index_1;
        if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsStrHash_SetEx");
}
pub export fn natsStrHash_GetEx(arg_hash: [*c]natsStrHash, arg_key: [*c]u8, arg_keyLen: c_int) ?*anyopaque {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    ...
    var e: [*c]natsStrHashEntry = undefined;
    _ = &e;
    var hk: u32 = natsStrHash_Hash(key, keyLen);
    _ = &hk;
    e = hash.*.bkts[hk & @as(u32, @bitCast(hash.*.mask))];
    while (e != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if ((e.*.hk == hk) and (strncmp(e.*.key, key, @as(c_ulong, @bitCast(@as(c_long, keyLen)))) == @as(c_int, 0))) {
            return e.*.data;
        }
        e = e.*.next;
    }
...
pub export fn natsStrHash_Remove(arg_hash: [*c]natsStrHash, arg_key: [*c]u8) ?*anyopaque {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var entryRemoved: [*c]natsStrHashEntry = null;
    _ = &entryRemoved;
    var dataRemoved: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &dataRemoved;
    var e: [*c][*c]natsStrHashEntry = undefined;
    _ = &e;
    var hk: u32 = undefined;
    _ = &hk;
    hk = natsStrHash_Hash(key, @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(key))))));
    e = &hash.*.bkts[hk & @as(u32, @bitCast(hash.*.mask))];
    while (e.* != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        ...
        e = @as([*c][*c]natsStrHashEntry, @ptrCast(@alignCast(&e.*.*.next)));
    }
    return dataRemoved;
}
pub export fn natsStrHash_RemoveSingle(arg_hash: [*c]natsStrHash, arg_key: [*c][*c]u8, arg_data: [*c]?*anyopaque) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var key = arg_key;
    _ = &key;
    var data = arg_data;
    _ = &data;
    var e: [*c]natsStrHashEntry = null;
    _ = &e;
    var i: c_int = undefined;
    _ = &i;
    if (hash.*.used != @as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_RemoveSingle", @as(c_int, 706), @as(c_uint, @bitCast(NATS_ERR)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_ERR))));
    {
        i = 0;
        while (i < hash.*.numBkts) : (i += 1) {
            e = (blk: {
            ...
            if (e != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                if (key != @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    var retKey: [*c]u8 = e.*.key;
                    _ = &retKey;
                    if (e.*.freeKey) {
                        retKey = strdup(e.*.key);
                        if (retKey == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHash_RemoveSingle", @as(c_int, 721), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
                    }
                    key.* = retKey;
                }
                if (data != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub export fn natsStrHash_Destroy(arg_hash: [*c]natsStrHash) void {
    var hash = arg_hash;
    _ = &hash;
    var e: [*c]natsStrHashEntry = undefined;
    _ = &e;
    var ne: [*c]natsStrHashEntry = undefined;
    _ = &ne;
    var i: c_int = undefined;
    _ = &i;
    if (hash == @as([*c]natsStrHash, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    {
        i = 0;
        while (i < hash.*.numBkts) : (i += 1) {
            e = (blk: {
...
pub export fn natsStrHashIter_Init(arg_iter: [*c]natsStrHashIter, arg_hash: [*c]natsStrHash) void {
    var iter = arg_iter;
    _ = &iter;
    var hash = arg_hash;
    _ = &hash;
    _ = memset(@as(?*anyopaque, @ptrCast(iter)), @as(c_int, 0), @sizeOf(natsStrHashIter));
    hash.*.canResize = @as(c_int, 0) != 0;
    iter.*.hash = hash;
    iter.*.current = hash.*.bkts[@as(c_uint, @intCast(@as(c_int, 0)))];
    iter.*.next = iter.*.current;
}
pub export fn natsStrHashIter_Next(arg_iter: [*c]natsStrHashIter, arg_key: [*c][*c]u8, arg_value: [*c]?*anyopaque) bool {
    var iter = arg_iter;
    _ = &iter;
    var key = arg_key;
    _ = &key;
    var value = arg_value;
    _ = &value;
    if ((@as(c_int, @intFromBool(iter.*.started)) != 0) and (iter.*.next == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return @as(c_int, 0) != 0;
    if (!iter.*.started and (iter.*.current == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        while ((iter.*.next == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (iter.*.currBkt < (iter.*.hash.*.numBkts - @as(c_int, 1)))) {
            iter.*.next = (blk: {
                const tmp = blk_1: {
                    const ref = &iter.*.currBkt;
                    ref.* += 1;
        ...
        if (iter.*.next == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            iter.*.started = @as(c_int, 1) != 0;
            return @as(c_int, 0) != 0;
        }
    }
    iter.*.started = @as(c_int, 1) != 0;
    iter.*.current = iter.*.next;
    if (iter.*.current != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if (key != @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            key.* = iter.*.current.*.key;
        }
        if (value != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub export fn natsStrHashIter_RemoveCurrent(arg_iter: [*c]natsStrHashIter) natsStatus {
    var iter = arg_iter;
    _ = &iter;
    var key: [*c]u8 = undefined;
    _ = &key;
    if (iter.*.current == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "natsStrHashIter_RemoveCurrent", @as(c_int, 827), @as(c_uint, @bitCast(NATS_NOT_FOUND)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NOT_FOUND))));
    key = iter.*.current.*.key;
    iter.*.current = iter.*.next;
    _ = natsStrHash_Remove(iter.*.hash, key);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsStrHashIter_Done(arg_iter: [*c]natsStrHashIter) void {
    var iter = arg_iter;
    _ = &iter;
    natsHashIter_Done(@as([*c]natsHashIter, @ptrCast(@alignCast(iter))));
}
...
pub var _DWSZ: c_int = @as(c_int, 4) << @intCast(1);
pub var _DDWSZ: c_int = @as(c_int, 4) << @intCast(2);
pub var _MAX_BKT_SIZE: c_int = (@as(c_int, 1) << @intCast(30)) - @as(c_int, 1);
pub fn _resize(arg_hash: [*c]natsHash, arg_newSize: c_int) callconv(.c) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var newSize = arg_newSize;
    _ = &newSize;
    var bkts: [*c][*c]natsHashEntry = null;
    _ = &bkts;
    var newMask: c_int = newSize - @as(c_int, 1);
    _ = &newMask;
    var ne: [*c]natsHashEntry = undefined;
    _ = &ne;
    var e: [*c]natsHashEntry = undefined;
    ...
    _ = &k;
    var newIndex: c_int = undefined;
    _ = &newIndex;
    bkts = @as([*c][*c]natsHashEntry, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, newSize))), @sizeOf([*c]natsHashEntry)))));
    if (bkts == @as([*c][*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "_resize", @as(c_int, 86), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    {
        k = 0;
        while (k < hash.*.numBkts) : (k += 1) {
            e = (blk: {
                const tmp = k;
                if (tmp >= 0) break :blk hash.*.bkts + @as(usize, @intCast(tmp)) else break :blk hash.*.bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            while (e != @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                ne = e;
                e = e.*.next;
                newIndex = @as(c_int, @bitCast(@as(c_int, @truncate(ne.*.key & @as(i64, @bitCast(@as(c_long, newMask)))))));
                ne.*.next = (blk: {
                    const tmp = newIndex;
                    if (tmp >= 0) break :blk bkts + @as(usize, @intCast(tmp)) else break :blk bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*;
                (blk: {
                    const tmp = newIndex;
                    if (tmp >= 0) break :blk bkts + @as(usize, @intCast(tmp)) else break :blk bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
...
pub fn _grow(arg_hash: [*c]natsHash) callconv(.c) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    if (hash.*.numBkts >= _MAX_BKT_SIZE) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "_grow", @as(c_int, 115), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    return _resize(hash, @as(c_int, 2) * hash.*.numBkts);
}
pub fn _shrink(arg_hash: [*c]natsHash) callconv(.c) void {
    var hash = arg_hash;
    _ = &hash;
    if (hash.*.numBkts <= @as(c_int, 8)) return;
    _ = _resize(hash, @divTrunc(hash.*.numBkts, @as(c_int, 2)));
}
pub fn _createEntry(arg_key: i64, arg_data: ?*anyopaque) callconv(.c) [*c]natsHashEntry {
    var key = arg_key;
    _ = &key;
    var data = arg_data;
    _ = &data;
    var e: [*c]natsHashEntry = @as([*c]natsHashEntry, @ptrCast(@alignCast(malloc(@sizeOf(natsHashEntry)))));
    _ = &e;
    if (e == @as([*c]natsHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return null;
    e.*.key = key;
    e.*.data = data;
    e.*.next = null;
    return e;
}
pub fn _maybeShrink(arg_hash: [*c]natsHash) callconv(.c) void {
    var hash = arg_hash;
    _ = &hash;
    if (((@as(c_int, @intFromBool(hash.*.canResize)) != 0) and (hash.*.numBkts > @as(c_int, 8))) and (hash.*.used < @divTrunc(hash.*.numBkts, @as(c_int, 4)))) {
        _shrink(hash);
    }
}
pub fn _resizeStr(arg_hash: [*c]natsStrHash, arg_newSize: c_int) callconv(.c) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    var newSize = arg_newSize;
    _ = &newSize;
    var bkts: [*c][*c]natsStrHashEntry = null;
    _ = &bkts;
    var newMask: c_int = newSize - @as(c_int, 1);
    _ = &newMask;
    var ne: [*c]natsStrHashEntry = undefined;
    ...
    var newIndex: c_int = undefined;
    _ = &newIndex;
    bkts = @as([*c][*c]natsStrHashEntry, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, newSize))), @sizeOf([*c]natsStrHashEntry)))));
    if (bkts == @as([*c][*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "_resizeStr", @as(c_int, 474), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    {
        k = 0;
        while (k < hash.*.numBkts) : (k += 1) {
            e = (blk: {
            ...
            while (e != @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                ne = e;
                e = e.*.next;
                newIndex = @as(c_int, @bitCast(ne.*.hk & @as(u32, @bitCast(newMask))));
                ne.*.next = (blk: {
                    const tmp = newIndex;
                    if (tmp >= 0) break :blk bkts + @as(usize, @intCast(tmp)) else break :blk bkts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*;
...
pub fn _growStr(arg_hash: [*c]natsStrHash) callconv(.c) natsStatus {
    var hash = arg_hash;
    _ = &hash;
    if (hash.*.numBkts >= _MAX_BKT_SIZE) return nats_setErrorReal("./src/submodules/nats.c/src/hash.c", "_growStr", @as(c_int, 503), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    return _resizeStr(hash, @as(c_int, 2) * hash.*.numBkts);
}
pub fn _shrinkStr(arg_hash: [*c]natsStrHash) callconv(.c) void {
    var hash = arg_hash;
    _ = &hash;
    if (hash.*.numBkts <= @as(c_int, 8)) return;
    _ = _resizeStr(hash, @divTrunc(hash.*.numBkts, @as(c_int, 2)));
}
pub fn _createStrEntry(arg_hk: u32, arg_key: [*c]u8, arg_copyKey: bool, arg_freeKey: bool, arg_data: ?*anyopaque) callconv(.c) [*c]natsStrHashEntry {
    var hk = arg_hk;
    _ = &hk;
    var key = arg_key;
    _ = &key;
    ...
    var data = arg_data;
    _ = &data;
    var e: [*c]natsStrHashEntry = @as([*c]natsStrHashEntry, @ptrCast(@alignCast(malloc(@sizeOf(natsStrHashEntry)))));
    _ = &e;
    if (e == @as([*c]natsStrHashEntry, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return null;
    e.*.hk = hk;
    e.*.key = if (@as(c_int, @intFromBool(copyKey)) != 0) strdup(key) else key;
    e.*.freeKey = freeKey;
    e.*.data = data;
    e.*.next = null;
    if (e.*.key == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        free(@as(?*anyopaque, @ptrCast(e)));
        return null;
    }
    return e;
}
pub fn _freeStrEntry(arg_e: [*c]natsStrHashEntry) callconv(.c) void {
    var e = arg_e;
    _ = &e;
    if (e.*.freeKey) {
        free(@as(?*anyopaque, @ptrCast(e.*.key)));
    }
    free(@as(?*anyopaque, @ptrCast(e)));
}
pub fn _maybeShrinkStr(arg_hash: [*c]natsStrHash) callconv(.c) void {
    var hash = arg_hash;
    _ = &hash;
    if (((@as(c_int, @intFromBool(hash.*.canResize)) != 0) and (hash.*.numBkts > @as(c_int, 8))) and (hash.*.used < @divTrunc(hash.*.numBkts, @as(c_int, 4)))) {
        _shrinkStr(hash);
...
pub const _freeEntry = @compileError("unable to translate C expr: unexpected token '{'");
// ./src/submodules/nats.c/src/hash.c:22:9
pub const _OFF32 = @import("std").zig.c_translation.promoteIntLiteral(c_int, 2166136261, .decimal);
pub const _YP32 = @import("std").zig.c_translation.promoteIntLiteral(c_int, 709607, .decimal);
pub const _BSZ = @as(c_int, 8);
pub const _WSZ = @as(c_int, 4);

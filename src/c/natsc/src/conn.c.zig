...
pub export fn natsConnection_Connect(arg_newConn: [*c][*c]natsConnection, arg_options: [*c]natsOptions) natsStatus {
    var newConn = arg_newConn;
    _ = &newConn;
    ...
    _ = &opts;
    if (options == @as([*c]natsOptions, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = natsConnection_ConnectTo(newConn, "nats://localhost:4222");
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_Connect");
    }
    opts = natsOptions_clone(options);
    if (opts == @as([*c]natsOptions, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = @as(c_uint, @bitCast(NATS_NO_MEMORY));
    }
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) or (s == @as(c_uint, @bitCast(NATS_NOT_YET_CONNECTED)))) {
        newConn.* = nc;
    } else {
        natsConn_release(nc);
    }
...
pub export fn natsConnection_Reconnect(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_Reconnect", @as(c_int, 3424), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        _ = natsMutex_Unlock(nc.*.mu);
        return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_Reconnect", @as(c_int, 3430), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    }
    natsSock_Shutdown(nc.*.sockCtx.fd);
    _ = natsMutex_Unlock(nc.*.mu);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsConnection_ProcessReadEvent(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    ...
    _ = &size;
    _ = natsMutex_Lock(nc.*.mu);
    if (!nc.*.el.attached or (nc.*.sockCtx.fd == -@as(c_int, 1))) {
        _ = natsMutex_Unlock(nc.*.mu);
        return;
    }
    if (nc.*.ps == @as([*c]natsParser, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = natsParser_Create(&nc.*.ps);
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            _ = if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_ProcessReadEvent");
            _ = natsMutex_Unlock(nc.*.mu);
            return;
        }
    }
    ...
    buffer = @as([*c]u8, @ptrCast(@alignCast(nc.*.el.buffer)));
    size = nc.*.opts.*.ioBufSize;
    s = natsSock_Read(&nc.*.sockCtx, buffer, @as(usize, @bitCast(@as(c_long, size))), &n);
    _ = natsMutex_Unlock(nc.*.mu);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsParser_Parse(nc, buffer, n);
    }
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        _ = _processOpError(nc, s, @as(c_int, 0) != 0);
    }
    natsConn_release(nc);
}
pub export fn natsConnection_ProcessCloseEvent(arg_socket_1: [*c]natsSock) void {
    var socket_1 = arg_socket_1;
    _ = &socket_1;
    if ((socket_1 == @as([*c]natsSock, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (socket_1.* == -@as(c_int, 1))) return;
    natsSock_Close(socket_1.*);
    socket_1.* = -@as(c_int, 1);
}
pub export fn natsConnection_ProcessWriteEvent(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        if (n == len) {
            natsBuf_Reset(nc.*.bw);
            s = nc.*.opts.*.evCbs.write.?(nc.*.el.data, @as(c_int, 0) != 0);
            if (s == @as(c_uint, @bitCast(NATS_OK))) {
                nc.*.el.writeAdded = @as(c_int, 0) != 0;
            } else {
                _ = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_ProcessWriteEvent", @as(c_int, 4175), s, "Error processing write request: %d - %s", s, natsStatus_GetText(s));
            }
        } else {
...
pub export fn natsConnection_ConnectTo(arg_newConn: [*c][*c]natsConnection, arg_url: [*c]const u8) natsStatus {
    var newConn = arg_newConn;
    _ = &newConn;
    var url = arg_url;
    _ = &url;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_ConnectTo");
}
pub export fn natsConnection_IsClosed(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    var closed: bool = undefined;
    _ = &closed;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_int, 1) != 0;
    _ = natsMutex_Lock(nc.*.mu);
    closed = natsConn_isClosed(nc);
    _ = natsMutex_Unlock(nc.*.mu);
    return closed;
}
pub export fn natsConnection_IsReconnecting(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    var reconnecting: bool = undefined;
    _ = &reconnecting;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_int, 0) != 0;
    _ = natsMutex_Lock(nc.*.mu);
    reconnecting = natsConn_isReconnecting(nc);
    _ = natsMutex_Unlock(nc.*.mu);
    return reconnecting;
}
pub export fn natsConnection_IsDraining(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    var draining: bool = undefined;
    _ = &draining;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_int, 0) != 0;
    _ = natsMutex_Lock(nc.*.mu);
...
pub export fn natsConnection_Status(arg_nc: [*c]natsConnection) natsConnStatus {
    var nc = arg_nc;
    _ = &nc;
    var cs: natsConnStatus = undefined;
    _ = &cs;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED));
    _ = natsMutex_Lock(nc.*.mu);
    cs = nc.*.status;
    _ = natsMutex_Unlock(nc.*.mu);
    return cs;
}
pub export fn natsConnection_Buffered(arg_nc: [*c]natsConnection) c_int {
    var nc = arg_nc;
    _ = &nc;
    var buffered: c_int = -@as(c_int, 1);
    _ = &buffered;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return buffered;
    _ = natsMutex_Lock(nc.*.mu);
    if ((nc.*.status != @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED))) and (nc.*.bw != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        buffered = nc.*.bw.*.len;
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return buffered;
}
pub export fn natsConnection_Flush(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = natsConnection_FlushTimeout(nc, @as(i64, @bitCast(@as(c_long, @as(c_int, 10000)))));
    _ = &s;
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_Flush");
}
pub export fn natsConnection_FlushTimeout(arg_nc: [*c]natsConnection, arg_timeout: i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_FlushTimeout", @as(c_int, 3593), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    if (timeout <= @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_FlushTimeout", @as(c_int, 3596), @as(c_uint, @bitCast(NATS_INVALID_TIMEOUT)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_TIMEOUT))));
    natsConn_lockAndRetain(nc);
    if (natsConn_isClosed(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_FlushTimeout", @as(c_int, 3601), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = _flushTimeout(nc, timeout);
    }
    natsConn_unlockAndRelease(nc);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_FlushTimeout");
}
pub export fn natsConnection_GetMaxPayload(arg_nc: [*c]natsConnection) i64 {
    var nc = arg_nc;
    _ = &nc;
    var mp: i64 = 0;
    _ = &mp;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 0;
    _ = natsMutex_Lock(nc.*.mu);
    mp = nc.*.info.maxPayload;
    _ = natsMutex_Unlock(nc.*.mu);
    return mp;
}
pub export fn natsConnection_GetStats(arg_nc: [*c]natsConnection, arg_stats: [*c]natsStatistics) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var stats = arg_stats;
    _ = &stats;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (stats == @as([*c]natsStatistics, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetStats", @as(c_int, 3906), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    natsMutex_Lock(nc.*.subsMu);
    _ = memcpy(@as(?*anyopaque, @ptrCast(stats)), @as(?*const anyopaque, @ptrCast(&nc.*.stats)), @sizeOf(natsStatistics));
    natsMutex_Unlock(nc.*.subsMu);
    _ = natsMutex_Unlock(nc.*.mu);
    return s;
}
pub export fn natsConnection_GetConnectedUrl(arg_nc: [*c]natsConnection, arg_buffer: [*c]u8, arg_bufferSize: usize) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (buffer == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetConnectedUrl", @as(c_int, 3927), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    buffer[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    if (((nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))) or (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTING)))) and (nc.*.cur.*.url.*.fullUrl != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        if (strlen(nc.*.cur.*.url.*.fullUrl) >= bufferSize) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetConnectedUrl", @as(c_int, 3937), @as(c_uint, @bitCast(NATS_INSUFFICIENT_BUFFER)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INSUFFICIENT_BUFFER))));
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            _ = snprintf(buffer, bufferSize, "%s", nc.*.cur.*.url.*.fullUrl);
        }
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return s;
}
pub export fn natsConnection_GetConnectedServerId(arg_nc: [*c]natsConnection, arg_buffer: [*c]u8, arg_bufferSize: usize) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var buffer = arg_buffer;
    _ = &buffer;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (buffer == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetConnectedServerId", @as(c_int, 3954), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    buffer[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    if (((nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))) or (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTING)))) and (nc.*.info.id != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        if (strlen(nc.*.info.id) >= bufferSize) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetConnectedServerId", @as(c_int, 3964), @as(c_uint, @bitCast(NATS_INSUFFICIENT_BUFFER)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INSUFFICIENT_BUFFER))));
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            _ = snprintf(buffer, bufferSize, "%s", nc.*.info.id);
        }
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return s;
}
pub export fn natsConnection_GetServers(arg_nc: [*c]natsConnection, arg_servers: [*c][*c][*c]u8, arg_count: [*c]c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var servers = arg_servers;
    _ = &servers;
    var count = arg_count;
    _ = &count;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (servers == @as([*c][*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) or (count == @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetServers", @as(c_int, 3981), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    s = natsSrvPool_GetServers(nc.*.srvPool, @as(c_int, 0) != 0, servers, count);
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_GetServers");
}
pub export fn natsConnection_GetDiscoveredServers(arg_nc: [*c]natsConnection, arg_servers: [*c][*c][*c]u8, arg_count: [*c]c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var servers = arg_servers;
    _ = &servers;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (servers == @as([*c][*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) or (count == @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetDiscoveredServers", @as(c_int, 3998), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    s = natsSrvPool_GetServers(nc.*.srvPool, @as(c_int, 1) != 0, servers, count);
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_GetDiscoveredServers");
}
pub export fn natsConnection_GetLastError(arg_nc: [*c]natsConnection, arg_lastError: [*c][*c]const u8) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var lastError = arg_lastError;
    _ = &lastError;
    var s: natsStatus = undefined;
    _ = &s;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetLastError", @as(c_int, 4015), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    s = nc.*.err;
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.errStr[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    } else if (@as(c_int, @bitCast(@as(c_uint, nc.*.errStr[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00')) {
        _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&nc.*.errStr))), @sizeOf([256]u8), "%s", natsStatus_GetText(s));
    }
    lastError.* = @as([*c]u8, @ptrCast(@alignCast(&nc.*.errStr)));
    _ = natsMutex_Unlock(nc.*.mu);
    return s;
}
pub export fn natsConnection_GetClientID(arg_nc: [*c]natsConnection, arg_cid: [*c]u64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var cid = arg_cid;
    _ = &cid;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (cid == @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetClientID", @as(c_int, 4206), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        s = @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED));
    } else {
        cid.* = nc.*.info.CID;
        if (cid.* == @as(u64, @bitCast(@as(c_long, @as(c_int, 0))))) {
            s = @as(c_uint, @bitCast(NATS_NO_SERVER_SUPPORT));
        }
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_GetClientID");
}
pub export fn natsConnection_Drain(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = _drain(nc, @as(i64, @bitCast(@as(c_long, @as(c_int, 30000)))));
    _ = &s;
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_Drain");
}
pub export fn natsConnection_DrainTimeout(arg_nc: [*c]natsConnection, arg_timeout: i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var timeout = arg_timeout;
    _ = &timeout;
    var s: natsStatus = _drain(nc, timeout);
    _ = &s;
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_DrainTimeout");
}
pub const userCreds = struct___userCreds;
pub export fn natsConnection_Sign(arg_nc: [*c]natsConnection, arg_payload: [*c]const u8, arg_payloadLen: c_int, arg_sig: [*c]u8) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var payload = arg_payload;
    _ = &payload;
    var payloadLen = arg_payloadLen;
    _ = &payloadLen;
    var sig = arg_sig;
    _ = &sig;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var uc: [*c]userCreds = null;
    _ = &uc;
    if (((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (payloadLen < @as(c_int, 0))) or (sig == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_Sign", @as(c_int, 4354), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    uc = nc.*.opts.*.userCreds;
    if (uc == @as([*c]userCreds, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_Sign", @as(c_int, 4360), @as(c_uint, @bitCast(NATS_ERR)), "%s", "unable to sign since no user credentials have been set");
    } else {
        s = _sign(uc, payload, payloadLen, sig);
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_Sign");
}
pub export fn natsConnection_GetClientIP(arg_nc: [*c]natsConnection, arg_ip: [*c][*c]u8) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var ip = arg_ip;
    _ = &ip;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (ip == @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetClientIP", @as(c_int, 4374), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    ip.* = null;
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetClientIP", @as(c_int, 4380), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    } else if (nc.*.info.clientIP == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetClientIP", @as(c_int, 4382), @as(c_uint, @bitCast(NATS_NO_SERVER_SUPPORT)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_SERVER_SUPPORT))));
    } else if ((blk: {
        const tmp = strdup(nc.*.info.clientIP);
        ip.* = tmp;
        break :blk tmp;
    }) == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetClientIP", @as(c_int, 4384), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return s;
}
pub export fn natsConnection_GetRTT(arg_nc: [*c]natsConnection, arg_rtt: [*c]i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var rtt = arg_rtt;
    _ = &rtt;
    ...
    _ = &start;
    if ((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (rtt == @as([*c]i64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetRTT", @as(c_int, 4397), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    rtt.* = 0;
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetRTT", @as(c_int, 4403), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    } else if (natsConn_isReconnecting(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetRTT", @as(c_int, 4405), @as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED))));
    } else {
        start = nats_NowInNanoSeconds();
        s = _flushTimeout(nc, @as(i64, @bitCast(@as(c_long, @as(c_int, 10000)))));
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
...
pub export fn natsConnection_HasHeaderSupport(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var headers: bool = @as(c_int, 0) != 0;
    _ = &headers;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_HasHeaderSupport", @as(c_int, 4424), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    headers = nc.*.info.headers;
    _ = natsMutex_Unlock(nc.*.mu);
    if (headers) return @as(c_uint, @bitCast(NATS_OK));
    return @as(c_uint, @bitCast(NATS_NO_SERVER_SUPPORT));
}
pub export fn natsConnection_GetLocalIPAndPort(arg_nc: [*c]natsConnection, arg_ip: [*c][*c]u8, arg_port: [*c]c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var ip = arg_ip;
    _ = &ip;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (((nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (ip == @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) or (port == @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetLocalIPAndPort", @as(c_int, 4442), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    ip.* = null;
    port.* = 0;
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetLocalIPAndPort", @as(c_int, 4449), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    } else if (!nc.*.sockCtx.fdActive) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConnection_GetLocalIPAndPort", @as(c_int, 4451), @as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED))));
    } else {
        s = natsSock_GetLocalIPAndPort(&nc.*.sockCtx, ip, port);
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConnection_GetLocalIPAndPort");
}
pub export fn natsConnection_Close(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var stanOwned: bool = undefined;
    _ = &stanOwned;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    _ = natsMutex_Lock(nc.*.mu);
...
pub export fn natsConnection_Destroy(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var stanOwned: bool = undefined;
    _ = &stanOwned;
    ...
    _ = natsMutex_Unlock(nc.*.mu);
    if (!stanOwned) {
        natsConn_destroy(nc, @as(c_int, 1) != 0);
    }
}
...
pub export fn natsConn_create(arg_newConn: [*c][*c]natsConnection, arg_options: [*c]natsOptions) natsStatus {
    var newConn = arg_newConn;
    _ = &newConn;
    var options = arg_options;
    _ = &options;
    ...
    var nc: [*c]natsConnection = null;
    _ = &nc;
    s = nats_Open(@as(i64, @bitCast(@as(c_long, -@as(c_int, 1)))));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc = @as([*c]natsConnection, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsConnection)))));
        if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_create", @as(c_int, 3235), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        }
    }
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        natsOptions_Destroy(options);
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_create");
    }
    natsLib_Retain();
    nc.*.refs = 1;
    nc.*.sockCtx.fd = -@as(c_int, 1);
    nc.*.opts = options;
    if (nc.*.opts.*.tlsHandshakeFirst) {
        nc.*.opts.*.secure = @as(c_int, 1) != 0;
    }
    nc.*.errStr[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsHash_Create(&nc.*.subs, @as(c_int, 8));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsSock_Init(&nc.*.sockCtx);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsBuf_Create(&nc.*.scratch, @as(c_int, 512));
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            s = natsBuf_Append(nc.*.scratch, "HPUB ", @as(c_int, 5));
        }
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        if (nc.*.opts.*.inboxPfx != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            nc.*.inboxPfx = @as([*c]const u8, @ptrCast(@alignCast(nc.*.opts.*.inboxPfx)));
        } else {
            nc.*.inboxPfx = "_INBOX.";
        }
        nc.*.inboxPfxLen = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(nc.*.inboxPfx)))));
        nc.*.reqIdOffset = (nc.*.inboxPfxLen + (@as(c_int, 12) + @as(c_int, 10))) + @as(c_int, 1);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newConn.* = nc;
    } else {
...
pub export fn natsConn_retain(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    _ = natsMutex_Lock(nc.*.mu);
...
pub export fn natsConn_release(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var refs: c_int = 0;
    _ = &refs;
...
pub export fn natsConn_bufferWrite(arg_nc: [*c]natsConnection, arg_buffer: [*c]const u8, arg_len: c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var buffer = arg_buffer;
    _ = &buffer;
    ...
    var avail: c_int = 0;
    _ = &avail;
    if (len <= @as(c_int, 0)) return @as(c_uint, @bitCast(NATS_OK));
    if (nc.*.usePending) return natsBuf_Append(nc.*.pending, buffer, len);
    if (nc.*.sockCtx.useEventLoop) {
        s = natsBuf_Append(nc.*.bw, buffer, len);
        if (((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.bw.*.len >= nc.*.opts.*.ioBufSize)) and !nc.*.el.writeAdded) {
            nc.*.el.writeAdded = @as(c_int, 1) != 0;
            s = nc.*.opts.*.evCbs.write.?(nc.*.el.data, @as(c_int, 1) != 0);
            if (s != @as(c_uint, @bitCast(NATS_OK))) {
                _ = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_bufferWrite", @as(c_int, 327), s, "Error processing write request: %d - %s", s, natsStatus_GetText(s));
            }
        }
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferWrite");
    }
    if (nc.*.dontSendInPlace) {
        s = natsBuf_Append(nc.*.bw, buffer, len);
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferWrite");
    }
    while ((s == @as(c_uint, @bitCast(NATS_OK))) and (len > (nc.*.bw.*.capacity - nc.*.bw.*.len))) {
        if (nc.*.bw.*.len == @as(c_int, 0)) {
            s = natsSock_WriteFully(&nc.*.sockCtx, buffer + @as(usize, @bitCast(@as(isize, @intCast(offset)))), len);
            return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferWrite");
        }
        avail = nc.*.bw.*.capacity - nc.*.bw.*.len;
        s = natsBuf_Append(nc.*.bw, buffer + @as(usize, @bitCast(@as(isize, @intCast(offset)))), avail);
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (len > @as(c_int, 0))) {
        s = natsBuf_Append(nc.*.bw, buffer + @as(usize, @bitCast(@as(isize, @intCast(offset)))), len);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferWrite");
}
pub export fn natsConn_bufferFlush(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var bufLen: c_int = nc.*.bw.*.len;
    _ = &bufLen;
    if (bufLen == @as(c_int, 0)) return @as(c_uint, @bitCast(NATS_OK));
    if (nc.*.usePending) {
        s = natsBuf_Append(nc.*.pending, nc.*.bw.*.data, bufLen);
    } else if (nc.*.sockCtx.useEventLoop) {
        if (!nc.*.el.writeAdded) {
            nc.*.el.writeAdded = @as(c_int, 1) != 0;
            s = nc.*.opts.*.evCbs.write.?(nc.*.el.data, @as(c_int, 1) != 0);
            if (s != @as(c_uint, @bitCast(NATS_OK))) {
                _ = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_bufferFlush", @as(c_int, 288), s, "Error processing write request: %d - %s", s, natsStatus_GetText(s));
            }
        }
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferFlush");
    } else {
        s = natsSock_WriteFully(&nc.*.sockCtx, nc.*.bw.*.data, bufLen);
    }
    natsBuf_Reset(nc.*.bw);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferFlush");
}
pub export fn natsConn_isClosed(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    return nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED));
}
pub export fn natsConn_isReconnecting(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    return nc.*.pending != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
}
pub export fn natsConn_flushOrKickFlusher(arg_nc: [*c]natsConnection) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (nc.*.opts.*.sendAsap) {
        s = natsConn_bufferFlush(nc);
    } else if (!nc.*.flusherSignaled and (nc.*.bw != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        nc.*.flusherSignaled = @as(c_int, 1) != 0;
        natsCondition_Signal(nc.*.flusherCond);
    }
    return s;
}
pub export fn natsConn_processMsg(arg_nc: [*c]natsConnection, arg_buf: [*c]u8, arg_bufLen: c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var buf = arg_buf;
    _ = &buf;
    ...
    _ = &msg;
    var sc: bool = @as(c_int, 0) != 0;
    _ = &sc;
    var sm: bool = @as(c_int, 0) != 0;
    _ = &sm;
    var jsi: [*c]jsSub = null;
    _ = &jsi;
    var ctrlMsg: bool = @as(c_int, 0) != 0;
    _ = &ctrlMsg;
    var fcReply: [*c]const u8 = null;
    _ = &fcReply;
    var jct: c_int = 0;
    _ = &jct;
    var mf: natsMsgFilter = null;
    _ = &mf;
    var mfc: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &mfc;
    s = _createMsg(&msg, nc, buf, bufLen, nc.*.ps.*.ma.hdr);
    if (s != @as(c_uint, @bitCast(NATS_OK))) return s;
    natsMutex_Lock(nc.*.subsMu);
    nc.*.stats.inMsgs +%= @as(u64, @bitCast(@as(c_long, @as(c_int, 1))));
    nc.*.stats.inBytes +%= @as(u64, @bitCast(@as(c_long, bufLen)));
    if ((blk: {
        const tmp = nc.*.filter;
        mf = tmp;
        break :blk tmp;
    }) != @as(natsMsgFilter, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        mfc = nc.*.filterClosure;
        natsMutex_Unlock(nc.*.subsMu);
        mf.?(nc, &msg, mfc);
        if (msg == @as([*c]natsMsg, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_uint, @bitCast(NATS_OK));
        natsMutex_Lock(nc.*.subsMu);
    }
    sub = @as([*c]natsSubscription, @ptrCast(@alignCast(natsHash_Get(nc.*.subs, nc.*.ps.*.ma.sid))));
    if (sub == @as([*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        natsMutex_Unlock(nc.*.subsMu);
        natsMsg_Destroy(msg);
        return @as(c_uint, @bitCast(NATS_OK));
    }
    ...
    natsMutex_Unlock(nc.*.subsMu);
    if ((@as(c_int, @intFromBool(sub.*.closed)) != 0) or (@as(c_int, @intFromBool(sub.*.drainSkip)) != 0)) {
        nats_unlockReleaseSubAndDispatcher(sub);
        natsMsg_Destroy(msg);
        return @as(c_uint, @bitCast(NATS_OK));
    }
    jsi = sub.*.jsi;
    if ((jsi != null) and !jsi.*.pull) {
        ctrlMsg = natsMsg_isJSCtrl(msg, &jct);
        if ((@as(c_int, @intFromBool(ctrlMsg)) != 0) and (jct == @as(c_int, 1))) {
            _ = natsMsgHeader_Get(msg, "Nats-Consumer-Stalled", &fcReply);
        } else if (!ctrlMsg and (@as(c_int, @intFromBool(jsi.*.ordered)) != 0)) {
            var replaced: bool = @as(c_int, 0) != 0;
            _ = &replaced;
            s = jsSub_checkOrderedMsg(sub, msg, &replaced);
            if ((s != @as(c_uint, @bitCast(NATS_OK))) or (@as(c_int, @intFromBool(replaced)) != 0)) {
                nats_unlockReleaseSubAndDispatcher(sub);
                natsMsg_Destroy(msg);
                return s;
            }
    ...
    if (!ctrlMsg) {
        ...
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            ...
            if (jsi != @as([*c]jsSub, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            ...
            sub.*.dropped += 1;
            sc = !sub.*.slowConsumer;
            sub.*.slowConsumer = @as(c_int, 1) != 0;
            s = @as(c_uint, @bitCast(NATS_OK));
        }
    } else if ((jct == @as(c_int, 1)) and (msg.*.reply == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        s = jsSub_processSequenceMismatch(sub, msg, &sm);
    } else if ((jct == @as(c_int, 2)) and (msg.*.reply != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
    ...
    if ((@as(c_int, @intFromBool(sc)) != 0) or (@as(c_int, @intFromBool(sm)) != 0)) {
        nats_unlockSubAndDispatcher(sub);
    } else {
        nats_unlockReleaseSubAndDispatcher(sub);
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (fcReply != null)) {
        s = natsConnection_Publish(nc, fcReply, null, @as(c_int, 0));
    }
    if (ctrlMsg) {
        natsMsg_Destroy(msg);
    }
    if ((@as(c_int, @intFromBool(sc)) != 0) or (@as(c_int, @intFromBool(sm)) != 0)) {
        _ = natsMutex_Lock(nc.*.mu);
        nc.*.err = @as(c_uint, @bitCast(if (@as(c_int, @intFromBool(sc)) != 0) NATS_SLOW_CONSUMER else NATS_MISMATCH));
...
pub export fn natsConn_processOK(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
}
pub export fn natsConn_processErr(arg_nc: [*c]natsConnection, arg_buf: [*c]u8, arg_bufLen: c_int) void {
    var nc = arg_nc;
    _ = &nc;
    var buf = arg_buf;
    _ = &buf;
    var bufLen = arg_bufLen;
    _ = &bufLen;
    var @"error": [256]u8 = undefined;
    _ = &@"error";
    var close_1: bool = @as(c_int, 0) != 0;
    _ = &close_1;
    var authErrCode: c_int = 0;
    _ = &authErrCode;
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&@"error"))), @sizeOf([256]u8), "%.*s", bufLen, buf);
    nats_NormalizeErr(@as([*c]u8, @ptrCast(@alignCast(&@"error"))));
    if (strcasecmp(@as([*c]u8, @ptrCast(@alignCast(&@"error"))), "Stale Connection") == @as(c_int, 0)) {
        _ = _processOpError(nc, @as(c_uint, @bitCast(NATS_STALE_CONNECTION)), @as(c_int, 0) != 0);
    } else if (strcasestr(@as([*c]u8, @ptrCast(@alignCast(&@"error"))), "Permissions Violation") != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _processPermissionViolation(nc, @as([*c]u8, @ptrCast(@alignCast(&@"error"))));
    } else if ((blk: {
        const tmp = _checkAuthError(@as([*c]u8, @ptrCast(@alignCast(&@"error"))));
        authErrCode = tmp;
        break :blk tmp;
    }) != @as(c_int, 0)) {
        _ = natsMutex_Lock(nc.*.mu);
        close_1 = _processAuthError(nc, authErrCode, @as([*c]u8, @ptrCast(@alignCast(&@"error"))));
        _ = natsMutex_Unlock(nc.*.mu);
    } else {
        close_1 = @as(c_int, 1) != 0;
        _ = natsMutex_Lock(nc.*.mu);
        nc.*.err = @as(c_uint, @bitCast(NATS_ERR));
        _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&nc.*.errStr))), @sizeOf([256]u8), "%s", @as([*c]u8, @ptrCast(@alignCast(&@"error"))));
        _ = natsMutex_Unlock(nc.*.mu);
    }
    if (close_1) {
        _close(nc, @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED)), @as(c_int, 0) != 0, @as(c_int, 1) != 0);
    }
}
pub export fn natsConn_processPing(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    _ = natsMutex_Lock(nc.*.mu);
    ...
    if (natsConn_bufferWrite(nc, "PONG\r\n", @as(c_int, 6)) == @as(c_uint, @bitCast(NATS_OK))) {
        _ = natsConn_flushOrKickFlusher(nc);
    }
    _ = natsMutex_Unlock(nc.*.mu);
}
pub export fn natsConn_processPong(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var pong: [*c]natsPong = null;
    _ = &pong;
    _ = natsMutex_Lock(nc.*.mu);
    nc.*.pongs.incoming += 1;
    if (((blk: {
        const tmp = nc.*.pongs.head;
        pong = tmp;
        break :blk tmp;
    }) != @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (pong.*.id == nc.*.pongs.incoming)) {
...
pub export fn natsConn_subscribeImpl(arg_newSub: [*c][*c]natsSubscription, arg_nc: [*c]natsConnection, arg_lock: bool, arg_subj: [*c]const u8, arg_queue: [*c]const u8, arg_timeout: i64, arg_cb: natsMsgHandler, arg_cbClosure: ?*anyopaque, arg_preventUseOfLibDlvPool: bool, arg_jsi: [*c]jsSub) natsStatus {
    ...
    _ = &cbClosure;
    var preventUseOfLibDlvPool = arg_preventUseOfLibDlvPool;
    _ = &preventUseOfLibDlvPool;
    var jsi = arg_jsi;
    _ = &jsi;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var sub: [*c]natsSubscription = null;
    _ = &sub;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_subscribeImpl", @as(c_int, 3044), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    if (!nats_IsSubjectValid(subj, @as(c_int, 1) != 0)) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_subscribeImpl", @as(c_int, 3047), @as(c_uint, @bitCast(NATS_INVALID_SUBJECT)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_SUBJECT))));
    if ((queue != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and !_isQueueNameValid(queue)) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_subscribeImpl", @as(c_int, 3050), @as(c_uint, @bitCast(NATS_INVALID_QUEUE_NAME)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_QUEUE_NAME))));
    if (lock) {
        _ = natsMutex_Lock(nc.*.mu);
    }
    if (natsConn_isClosed(nc)) {
        ...
        return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_subscribeImpl", @as(c_int, 3060), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    }
    if (natsConn_isDraining(nc)) {
        if (lock) {
            _ = natsMutex_Unlock(nc.*.mu);
        }
        return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_subscribeImpl", @as(c_int, 3068), @as(c_uint, @bitCast(NATS_DRAINING)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_DRAINING))));
    }
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        if (!natsConn_isReconnecting(nc)) {
            ...
            if (s != @as(c_uint, @bitCast(NATS_NO_MEMORY))) {
                s = @as(c_uint, @bitCast(NATS_OK));
            }
        }
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newSub.* = sub;
    } else if (sub != @as([*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        natsSub_close(sub, @as(c_int, 0) != 0);
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_subscribeImpl");
}
pub export fn natsConn_unsubscribe(arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription, arg_max: c_int, arg_drainMode: bool, arg_timeout: i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var sub = arg_sub;
    _ = &sub;
    ...
    if (natsConn_isClosed(nc)) {
        _ = natsMutex_Unlock(nc.*.mu);
        return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_unsubscribe", @as(c_int, 3156), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    }
    natsMutex_Lock(nc.*.subsMu);
    sub = @as([*c]natsSubscription, @ptrCast(@alignCast(natsHash_Get(nc.*.subs, sub.*.sid))));
    natsMutex_Unlock(nc.*.subsMu);
    if ((sub == @as([*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or !natsSubscription_IsValid(sub)) {
        _ = natsMutex_Unlock(nc.*.mu);
        return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_unsubscribe", @as(c_int, 3166), @as(c_uint, @bitCast(NATS_INVALID_SUBSCRIPTION)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_SUBSCRIPTION))));
    }
    if (max > @as(c_int, 0)) {
        if (!natsSub_setMax(sub, @as(u64, @bitCast(@as(c_long, max))))) {
            max = 0;
        }
    }
    if ((max == @as(c_int, 0)) and !drainMode) {
        natsConn_removeSubscription(nc, sub);
    }
    if (!drainMode and !natsConn_isReconnecting(nc)) {
        if (nc.*.opts.*.writeDeadline > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
            natsDeadline_Init(&nc.*.sockCtx.writeDeadline, nc.*.opts.*.writeDeadline);
        }
        ...
        if ((s != @as(c_uint, @bitCast(NATS_OK))) and (s != @as(c_uint, @bitCast(NATS_NO_MEMORY)))) {
            nats_clearLastError();
            s = @as(c_uint, @bitCast(NATS_OK));
        }
    } else if (drainMode) {
        if (natsConn_isDraining(nc)) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_unsubscribe", @as(c_int, 3202), @as(c_uint, @bitCast(NATS_DRAINING)), "%s", "Illegal to drain a subscription while its connection is draining");
        } else {
            s = natsSub_startDrain(sub, timeout);
        }
    }
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_unsubscribe");
}
pub export fn natsConn_enqueueUnsubProto(arg_nc: [*c]natsConnection, arg_sid: i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var sid = arg_sid;
    _ = &sid;
    ...
    var res: c_int = 0;
    _ = &res;
    res = asprintf(&proto, "UNSUB %ld \r\n", sid);
    if (res < @as(c_int, 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_enqueueUnsubProto", @as(c_int, 3133), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    } else {
        nc.*.dontSendInPlace = @as(c_int, 1) != 0;
        _ = natsConn_bufferWrite(nc, @as([*c]const u8, @ptrCast(@alignCast(proto))), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(proto))))));
        nc.*.dontSendInPlace = @as(c_int, 0) != 0;
        free(@as(?*anyopaque, @ptrCast(proto)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_enqueueUnsubProto");
}
pub extern fn natsConn_drainSub(nc: [*c]natsConnection, sub: [*c]natsSubscription, checkConnDrainStatus: bool) natsStatus;
pub export fn natsConn_isDraining(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    return (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_DRAINING_SUBS))) or (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_DRAINING_PUBS)));
}
pub export fn natsConn_isDrainingPubs(arg_nc: [*c]natsConnection) bool {
    var nc = arg_nc;
    _ = &nc;
    return nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_DRAINING_PUBS));
}
pub export fn natsConn_removeSubscription(arg_nc: [*c]natsConnection, arg_removedSub: [*c]natsSubscription) void {
    var nc = arg_nc;
    _ = &nc;
    var removedSub = arg_removedSub;
    _ = &removedSub;
    var sub: [*c]natsSubscription = null;
    _ = &sub;
    natsMutex_Lock(nc.*.subsMu);
    sub = @as([*c]natsSubscription, @ptrCast(@alignCast(natsHash_Remove(nc.*.subs, removedSub.*.sid))));
    if (sub != @as([*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        natsSub_close(sub, @as(c_int, 0) != 0);
    }
    natsMutex_Unlock(nc.*.subsMu);
...
pub export fn natsConn_processAsyncINFO(arg_nc: [*c]natsConnection, arg_buf: [*c]u8, arg_len: c_int) void {
    var nc = arg_nc;
    _ = &nc;
    var buf = arg_buf;
    _ = &buf;
    ...
    _ = &len;
    _ = natsMutex_Lock(nc.*.mu);
    _ = _processInfo(nc, buf, len);
    _ = natsMutex_Unlock(nc.*.mu);
}
pub export fn natsConn_addRespInfo(arg_newResp: [*c][*c]respInfo, arg_nc: [*c]natsConnection, arg_respInbox: [*c]u8, arg_respInboxSize: c_int) natsStatus {
    var newResp = arg_newResp;
    _ = &newResp;
    var nc = arg_nc;
    _ = &nc;
    var respInbox = arg_respInbox;
    _ = &respInbox;
    var respInboxSize = arg_respInboxSize;
    _ = &respInboxSize;
    var resp: [*c]respInfo = null;
    _ = &resp;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (nc.*.respPoolIdx > @as(c_int, 0)) {
        resp = (blk: {
            const tmp = blk_1: {
                const ref = &nc.*.respPoolIdx;
                ref.* -= 1;
                break :blk_1 ref.*;
            };
            if (tmp >= 0) break :blk nc.*.respPool + @as(usize, @intCast(tmp)) else break :blk nc.*.respPool - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*;
    } else {
        resp = @as([*c]respInfo, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(respInfo)))));
        if (resp == @as([*c]respInfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_addRespInfo", @as(c_int, 1337), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
        ...
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.respPoolSize < @as(c_int, 10))) {
            resp.*.pooled = @as(c_int, 1) != 0;
            nc.*.respPoolSize += 1;
        }
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.respId[@as(c_uint, @intCast(nc.*.respIdPos))] = @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, '0') + @as(c_int, @bitCast(@as(c_uint, nc.*.respIdVal)))))));
        nc.*.respId[@as(c_uint, @intCast(nc.*.respIdPos + @as(c_int, 1)))] = '\x00';
        _ = memcpy(@as(?*anyopaque, @ptrCast(respInbox)), @as(?*const anyopaque, @ptrCast(nc.*.respSub)), @as(c_ulong, @bitCast(@as(c_long, nc.*.reqIdOffset))));
        (blk: {
            const tmp = nc.*.reqIdOffset - @as(c_int, 1);
            if (tmp >= 0) break :blk respInbox + @as(usize, @intCast(tmp)) else break :blk respInbox - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).* = '.';
        _ = memcpy(@as(?*anyopaque, @ptrCast(respInbox + @as(usize, @bitCast(@as(isize, @intCast(nc.*.reqIdOffset)))))), @as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&nc.*.respId))))), @as(c_ulong, @bitCast(@as(c_long, nc.*.respIdPos + @as(c_int, 2)))));
        nc.*.respIdVal +%= 1;
        if (@as(c_int, @bitCast(@as(c_uint, nc.*.respIdVal))) == @as(c_int, 10)) {
            nc.*.respIdVal = 0;
            if (nc.*.respIdPos > @as(c_int, 0)) {
                var shift: bool = @as(c_int, 1) != 0;
                _ = &shift;
                ...
                _ = &j;
                {
                    i = nc.*.respIdPos - @as(c_int, 1);
                    while (i >= @as(c_int, 0)) : (i -= 1) {
                        if (@as(c_int, @bitCast(@as(c_uint, nc.*.respId[@as(c_uint, @intCast(i))]))) != @as(c_int, '9')) {
                            nc.*.respId[@as(c_uint, @intCast(i))] +%= 1;
                            {
                                j = i + @as(c_int, 1);
                                while (j <= (nc.*.respIdPos - @as(c_int, 1))) : (j += 1) {
                                    nc.*.respId[@as(c_uint, @intCast(j))] = '0';
                                }
                            }
                            shift = @as(c_int, 0) != 0;
                            break;
                ...
                if (shift) {
                    nc.*.respId[@as(c_uint, @intCast(@as(c_int, 0)))] = '1';
                    {
                        i = 1;
                        while (i <= nc.*.respIdPos) : (i += 1) {
                            nc.*.respId[@as(c_uint, @intCast(i))] = '0';
                        }
                    }
                    nc.*.respIdPos += 1;
                }
            ...
            if (nc.*.respIdPos == @as(c_int, 19)) {
                nc.*.respIdPos = 0;
            }
        }
        s = natsStrHash_SetEx(nc.*.respMap, respInbox + @as(usize, @bitCast(@as(isize, @intCast(nc.*.reqIdOffset)))), @as(c_int, 1) != 0, (if (@as(c_int, 1) != 0) @as(c_int, 1) else @as(c_int, 0)) != 0, @as(?*anyopaque, @ptrCast(resp)), null);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newResp.* = resp;
    } else {
        natsConn_disposeRespInfo(nc, resp, @as(c_int, 0) != 0);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_addRespInfo");
}
pub export fn natsConn_disposeRespInfo(arg_nc: [*c]natsConnection, arg_resp: [*c]respInfo, arg_needsLock: bool) void {
    var nc = arg_nc;
    _ = &nc;
    var resp = arg_resp;
    _ = &resp;
    var needsLock = arg_needsLock;
    _ = &needsLock;
    if (resp == @as([*c]respInfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    if (resp.*.msg != @as([*c]natsMsg, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    if (!resp.*.pooled) {
        ...
        free(@as(?*anyopaque, @ptrCast(resp)));
    } else {
        if (needsLock) {
            _ = natsMutex_Lock(nc.*.mu);
        }
        resp.*.closed = @as(c_int, 0) != 0;
        resp.*.closedSts = @as(c_uint, @bitCast(NATS_OK));
        resp.*.removed = @as(c_int, 0) != 0;
        (blk: {
            const tmp = blk_1: {
                const ref = &nc.*.respPoolIdx;
                const tmp_2 = ref.*;
...
pub export fn natsConn_initResp(arg_nc: [*c]natsConnection, arg_cb: natsMsgHandler) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var cb = arg_cb;
    _ = &cb;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    nc.*.respPool = @as([*c][*c]respInfo, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 10)))), @sizeOf([*c]respInfo)))));
    if (nc.*.respPool == @as([*c][*c]respInfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_initResp", @as(c_int, 1467), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsStrHash_Create(&nc.*.respMap, @as(c_int, 4));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsConn_newInbox(nc, @as([*c][*c]natsInbox, @ptrCast(@alignCast(&nc.*.respSub))));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        var inbox: [*c]u8 = null;
        _ = &inbox;
        if (asprintf(&inbox, "%s.*", nc.*.respSub) < @as(c_int, 0)) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_initResp", @as(c_int, 1477), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        } else {
            s = natsConn_subscribeImpl(&nc.*.respMux, nc, @as(c_int, 0) != 0, inbox, null, @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))), cb, @as(?*anyopaque, @ptrCast(nc)), @as(c_int, 1) != 0, null);
        }
        free(@as(?*anyopaque, @ptrCast(inbox)));
    }
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        ...
        nc.*.respMap = null;
        free(@as(?*anyopaque, @ptrCast(nc.*.respPool)));
        nc.*.respPool = null;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_initResp");
}
pub export fn natsConn_destroyRespPool(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var i: c_int = undefined;
    _ = &i;
    var info: [*c]respInfo = undefined;
    _ = &info;
    {
        i = 0;
        while (i < nc.*.respPoolSize) : (i += 1) {
            info = (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk nc.*.respPool + @as(usize, @intCast(tmp)) else break :blk nc.*.respPool - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            info.*.pooled = @as(c_int, 0) != 0;
            natsConn_disposeRespInfo(nc, info, @as(c_int, 0) != 0);
        }
    }
    free(@as(?*anyopaque, @ptrCast(nc.*.respPool)));
}
pub extern fn natsConn_publish(nc: [*c]natsConnection, msg: [*c]natsMsg, reply: [*c]const u8, directFlush: bool) natsStatus;
pub export fn natsConn_userCreds(arg_userJWT: [*c][*c]u8, arg_customErrTxt: [*c][*c]u8, arg_closure: ?*anyopaque) natsStatus {
    var userJWT = arg_userJWT;
    _ = &userJWT;
    var customErrTxt = arg_customErrTxt;
    _ = &customErrTxt;
    var closure = arg_closure;
    _ = &closure;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var uc: [*c]userCreds = @as([*c]userCreds, @ptrCast(@alignCast(closure)));
    _ = &uc;
    if (uc.*.jwtAndSeedContent != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_GetJWTOrSeed(userJWT, uc.*.jwtAndSeedContent, @as(c_int, 0));
    } else {
        s = _getJwtOrSeed(userJWT, uc.*.userOrChainedFile, @as(c_int, 0) != 0, @as(c_int, 0));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_userCreds");
}
pub export fn natsConn_signatureHandler(arg_customErrTxt: [*c][*c]u8, arg_psig: [*c][*c]u8, arg_sigLen: [*c]c_int, arg_nonce: [*c]const u8, arg_closure: ?*anyopaque) natsStatus {
    var customErrTxt = arg_customErrTxt;
    _ = &customErrTxt;
    var psig = arg_psig;
    _ = &psig;
    ...
    _ = &sig;
    psig.* = null;
    if (sigLen != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        sigLen.* = 0;
    }
    sig = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 64))))))));
    if (sig == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_signatureHandler", @as(c_int, 4331), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    s = _sign(uc, @as([*c]const u8, @ptrCast(@alignCast(nonce))), @as(c_int, 0), @as([*c]u8, @ptrCast(@alignCast(sig))));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        psig.* = @as([*c]u8, @ptrCast(@alignCast(sig)));
        if (sigLen != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            sigLen.* = 64;
        }
    } else {
        free(@as(?*anyopaque, @ptrCast(sig)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_signatureHandler");
}
pub export fn natsConn_sendSubProto(arg_nc: [*c]natsConnection, arg_subject: [*c]const u8, arg_queue: [*c]const u8, arg_sid: i64) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    ...
    var res: c_int = 0;
    _ = &res;
    res = asprintf(&proto, "SUB %s %s %ld\r\n", subject, if (queue == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "" else queue, sid);
    if (res < @as(c_int, 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_sendSubProto", @as(c_int, 1102), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    } else {
        s = natsConn_bufferWriteString(nc, proto);
        free(@as(?*anyopaque, @ptrCast(proto)));
        proto = null;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_sendSubProto");
}
pub export fn natsConn_sendUnsubProto(arg_nc: [*c]natsConnection, arg_subId: i64, arg_max: c_int) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var subId = arg_subId;
    _ = &subId;
    ...
    _ = &res;
    if (max > @as(c_int, 0)) {
        res = asprintf(&proto, "UNSUB %ld %d\r\n", subId, max);
    } else {
        res = asprintf(&proto, "UNSUB %ld \r\n", subId);
    }
    if (res < @as(c_int, 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_sendUnsubProto", @as(c_int, 1083), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    } else {
        s = natsConn_bufferWriteString(nc, proto);
        free(@as(?*anyopaque, @ptrCast(proto)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_sendUnsubProto");
}
pub export fn natsConn_setFilterWithClosure(arg_nc: [*c]natsConnection, arg_f: natsMsgFilter, arg_closure: ?*anyopaque) void {
    var nc = arg_nc;
    _ = &nc;
    var f = arg_f;
    _ = &f;
...
pub export fn natsConn_initInbox(arg_nc: [*c]natsConnection, arg_buf: [*c]u8, arg_bufSize: c_int, arg_newInbox: [*c][*c]u8, arg_allocated: [*c]bool) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var buf = arg_buf;
    _ = &buf;
    ...
    _ = &bufSize;
    var newInbox = arg_newInbox;
    _ = &newInbox;
    var allocated = arg_allocated;
    _ = &allocated;
    var needed: c_int = (nc.*.inboxPfxLen + (@as(c_int, 12) + @as(c_int, 10))) + @as(c_int, 1);
    _ = &needed;
    var inbox: [*c]u8 = buf;
    _ = &inbox;
    var created: bool = @as(c_int, 0) != 0;
    _ = &created;
    var s: natsStatus = undefined;
    _ = &s;
    if (needed > bufSize) {
        inbox = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, needed)))))));
        if (inbox == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_initInbox", @as(c_int, 1424), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        created = @as(c_int, 1) != 0;
    }
    _ = memcpy(@as(?*anyopaque, @ptrCast(inbox)), @as(?*const anyopaque, @ptrCast(nc.*.inboxPfx)), @as(c_ulong, @bitCast(@as(c_long, nc.*.inboxPfxLen))));
    s = natsNUID_Next(inbox + @as(usize, @bitCast(@as(isize, @intCast(nc.*.inboxPfxLen)))), (@as(c_int, 12) + @as(c_int, 10)) + @as(c_int, 1));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newInbox.* = inbox;
        if (allocated != @as([*c]bool, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            allocated.* = created;
        }
    } else if (created) {
        free(@as(?*anyopaque, @ptrCast(inbox)));
...
pub export fn natsConn_newInbox(arg_nc: [*c]natsConnection, arg_newInbox: [*c][*c]natsInbox) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var newInbox = arg_newInbox;
    _ = &newInbox;
    var s: natsStatus = undefined;
    _ = &s;
    var inboxLen: c_int = (nc.*.inboxPfxLen + (@as(c_int, 12) + @as(c_int, 10))) + @as(c_int, 1);
    _ = &inboxLen;
    var inbox: [*c]u8 = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, inboxLen)))))));
    _ = &inbox;
    if (inbox == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "natsConn_newInbox", @as(c_int, 1450), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    s = natsConn_initInbox(nc, inbox, inboxLen, @as([*c][*c]u8, @ptrCast(@alignCast(newInbox))), null);
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        free(@as(?*anyopaque, @ptrCast(inbox)));
    }
    return s;
}
pub export fn natsConn_srvVersionAtLeast(arg_nc: [*c]natsConnection, arg_major: c_int, arg_minor: c_int, arg_update: c_int) bool {
    var nc = arg_nc;
    _ = &nc;
    var major = arg_major;
    _ = &major;
    var minor = arg_minor;
    _ = &minor;
    var update = arg_update;
    _ = &update;
    var ok: bool = undefined;
    _ = &ok;
    _ = natsMutex_Lock(nc.*.mu);
    ok = (if (((nc.*.srvVersion.ma > major) or ((nc.*.srvVersion.ma == major) and (nc.*.srvVersion.mi > minor))) or (((nc.*.srvVersion.ma == major) and (nc.*.srvVersion.mi == minor)) and (nc.*.srvVersion.up >= update))) @as(c_int, 1) else @as(c_int, 0)) != 0;
    _ = natsMutex_Unlock(nc.*.mu);
    return ok;
}
pub export fn natsConn_defaultErrHandler(arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription, arg_err: natsStatus, arg_closure: ?*anyopaque) void {
    var nc = arg_nc;
    _ = &nc;
    var sub = arg_sub;
    _ = &sub;
    ...
    var cid: u64 = 0;
    _ = &cid;
    var lastErr: [*c]const u8 = null;
    _ = &lastErr;
    var errTxt: [*c]const u8 = null;
    _ = &errTxt;
    _ = natsMutex_Lock(nc.*.mu);
    cid = nc.*.info.CID;
    _ = natsMutex_Unlock(nc.*.mu);
    _ = natsConnection_GetLastError(nc, &lastErr);
    errTxt = if ((if ((lastErr == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, lastErr[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0) natsStatus_GetText(err) else lastErr;
    if (sub != @as([*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        var subject: [*c]u8 = null;
        _ = &subject;
        natsSub_Lock(sub);
        if ((sub.*.jsi != @as([*c]jsSub, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (sub.*.jsi.*.psubj != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            subject = sub.*.jsi.*.psubj;
        } else {
            subject = sub.*.subject;
        }
        _ = fprintf(stderr, "Error %d - %s on connection [%lu] on \"%s\"\n", err, errTxt, cid, subject);
        natsSub_Unlock(sub);
    } else {
        _ = fprintf(stderr, "Error %d - %s on connection [%lu]\n", err, errTxt, cid);
    }
    _ = fflush(stderr);
}
pub export fn natsConn_close(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    nats_doNotUpdateErrStack(@as(c_int, 1) != 0);
    _close(nc, @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED)), @as(c_int, 1) != 0, @as(c_int, 1) != 0);
    nats_doNotUpdateErrStack(@as(c_int, 0) != 0);
}
pub export fn natsConn_destroy(arg_nc: [*c]natsConnection, arg_fromPublicDestroy: bool) void {
    var nc = arg_nc;
    _ = &nc;
    var fromPublicDestroy = arg_fromPublicDestroy;
    _ = &fromPublicDestroy;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    nats_doNotUpdateErrStack(@as(c_int, 1) != 0);
    _close(nc, @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED)), fromPublicDestroy, @as(c_int, 1) != 0);
    nats_doNotUpdateErrStack(@as(c_int, 0) != 0);
    natsConn_release(nc);
}
...
pub const nats_JSONField = extern struct {
    name: [*c]u8 = @import("std").mem.zeroes([*c]u8),
    typ: c_int = @import("std").mem.zeroes(c_int),
    value: union_unnamed_76 = @import("std").mem.zeroes(union_unnamed_76),
...
pub extern fn natsSock_Init(ctx: [*c]natsSockCtx) natsStatus;
pub extern fn natsSock_WaitReady(waitMode: c_int, ctx: [*c]natsSockCtx) natsStatus;
pub extern fn natsSock_ShuffleIPs(ctx: [*c]natsSockCtx, tmp: [*c][*c]struct_addrinfo, tmpSize: c_int, ipListHead: [*c][*c]struct_addrinfo, count: c_int) void;
pub extern fn natsSock_ConnectTcp(ctx: [*c]natsSockCtx, host: [*c]const u8, port: c_int) natsStatus;
pub extern fn natsSock_SetBlocking(fd: natsSock, blocking: bool) natsStatus;
pub extern fn natsSock_IsConnected(fd: natsSock) bool;
pub extern fn natsSock_ReadLine(ctx: [*c]natsSockCtx, buffer: [*c]u8, maxBufferSize: usize) natsStatus;
pub extern fn natsSock_Read(ctx: [*c]natsSockCtx, buffer: [*c]u8, maxBufferSize: usize, n: [*c]c_int) natsStatus;
pub extern fn natsSock_Write(ctx: [*c]natsSockCtx, data: [*c]const u8, len: c_int, n: [*c]c_int) natsStatus;
pub extern fn natsSock_WriteFully(ctx: [*c]natsSockCtx, data: [*c]const u8, len: c_int) natsStatus;
pub extern fn natsSock_Flush(fd: natsSock) natsStatus;
pub extern fn natsSock_Close(fd: natsSock) void;
pub extern fn natsSock_SetCommonTcpOptions(fd: natsSock) natsStatus;
pub extern fn natsSock_Shutdown(fd: natsSock) void;
pub extern fn natsSock_ClearDeadline(ctx: [*c]natsSockCtx) void;
pub extern fn natsSock_InitDeadline(ctx: [*c]natsSockCtx, timeout: i64) void;
pub extern fn natsSock_GetLocalIPAndPort(ctx: [*c]natsSockCtx, ip: [*c][*c]u8, port: [*c]c_int) natsStatus;
pub extern fn natsKeys_Sign(encodedSeed: [*c]const u8, input: [*c]const u8, inputLen: c_int, signature: [*c]u8) natsStatus;
pub extern fn natsCrypto_Init() natsStatus;
pub extern fn natsCrypto_Sign(seed: [*c]const u8, input: [*c]const u8, inputLen: c_int, signature: [*c]u8) natsStatus;
pub extern fn natsCrypto_Clear(mem: ?*anyopaque, memLen: c_int) void;
...
pub fn _spinUpSocketWatchers(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    nc.*.pout = 0;
    nc.*.flusherStop = @as(c_int, 0) != 0;
    if (nc.*.opts.*.evLoop == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))) {
        _ = blk: {
            const ref = &nc.*.refs;
            const tmp = ref.*;
            ref.* += 1;
        ...
        s = natsThread_Create(&nc.*.readLoopThread, &_readLoop, @as(?*anyopaque, @ptrCast(nc)));
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            _ = blk: {
                const ref = &nc.*.refs;
                const tmp = ref.*;
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and !nc.*.opts.*.sendAsap) {
        _ = blk: {
            const ref = &nc.*.refs;
            const tmp = ref.*;
            ref.* += 1;
        ...
        s = natsThread_Create(&nc.*.flusherThread, &_flusher, @as(?*anyopaque, @ptrCast(nc)));
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            _ = blk: {
                const ref = &nc.*.refs;
                const tmp = ref.*;
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.opts.*.pingInterval > @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))))) {
        _ = blk: {
            const ref = &nc.*.refs;
            const tmp = ref.*;
            ref.* += 1;
        ...
        if (nc.*.ptmr == @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = natsTimer_Create(&nc.*.ptmr, &_processPingTimer, &_pingStopppedCb, nc.*.opts.*.pingInterval, @as(?*anyopaque, @ptrCast(nc)));
            if (s != @as(c_uint, @bitCast(NATS_OK))) {
                _ = blk: {
                    const ref = &nc.*.refs;
                    const tmp = ref.*;
...
pub fn _processConnInit(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTING));
    if ((@as(c_int, @intFromBool(nc.*.opts.*.secure)) != 0) and (@as(c_int, @intFromBool(nc.*.opts.*.tlsHandshakeFirst)) != 0)) {
        s = _makeTLSConn(nc);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.opts.*.writeDeadline <= @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))))) {
        s = natsSock_SetBlocking(nc.*.sockCtx.fd, @as(c_int, 1) != 0);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = _spinUpSocketWatchers(nc);
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.opts.*.evLoop != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))) {
        s = natsSock_SetBlocking(nc.*.sockCtx.fd, @as(c_int, 0) != 0);
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.el.buffer == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))) {
            nc.*.el.buffer = @as(?*anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, nc.*.opts.*.ioBufSize)))))))));
            if (nc.*.el.buffer == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_processConnInit", @as(c_int, 1999), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            }
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            nc.*.sockCtx.useEventLoop = @as(c_int, 1) != 0;
            s = nc.*.opts.*.evCbs.attach.?(&nc.*.el.data, nc.*.opts.*.evLoop, nc, @as(c_int, @bitCast(nc.*.sockCtx.fd)));
            if (s == @as(c_uint, @bitCast(NATS_OK))) {
                nc.*.el.attached = @as(c_int, 1) != 0;
            } else {
                nc.*.sockCtx.useEventLoop = @as(c_int, 0) != 0;
                _ = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_processConnInit", @as(c_int, 2021), s, "Error attaching to the event loop: %d - %s", s, natsStatus_GetText(s));
            }
        }
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_processConnInit");
}
pub const struct_threadsToJoin = extern struct {
    readLoop: [*c]natsThread = @import("std").mem.zeroes([*c]natsThread),
    flusher: [*c]natsThread = @import("std").mem.zeroes([*c]natsThread),
    reconnect: [*c]natsThread = @import("std").mem.zeroes([*c]natsThread),
    joinReconnect: bool = @import("std").mem.zeroes(bool),
};
pub fn _close(arg_nc: [*c]natsConnection, arg_status: natsConnStatus, arg_fromPublicClose: bool, arg_doCBs: bool) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var status = arg_status;
    _ = &status;
    var fromPublicClose = arg_fromPublicClose;
    _ = &fromPublicClose;
    var doCBs = arg_doCBs;
    _ = &doCBs;
    var ttj: struct_threadsToJoin = undefined;
    _ = &ttj;
    var sockWasActive: bool = @as(c_int, 0) != 0;
    _ = &sockWasActive;
    var detach: bool = @as(c_int, 0) != 0;
    _ = &detach;
    var postClosedCb: bool = @as(c_int, 0) != 0;
    _ = &postClosedCb;
    var postDisconnectedCb: bool = @as(c_int, 0) != 0;
    _ = &postDisconnectedCb;
    var sub: [*c]natsSubscription = null;
    _ = &sub;
    natsOptions_lock(nc.*.opts);
    postClosedCb = nc.*.opts.*.closedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    postDisconnectedCb = nc.*.opts.*.disconnectedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    natsOptions_unlock(nc.*.opts);
    natsConn_lockAndRetain(nc);
    if ((((@as(c_int, @intFromBool(fromPublicClose)) != 0) and (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED)))) and (@as(c_int, @intFromBool(nc.*.sockCtx.fdActive)) != 0)) and (nc.*.bw.*.len > @as(c_int, 0))) {
        _ = _flushTimeout(nc, @as(i64, @bitCast(@as(c_long, @as(c_int, 500)))));
    }
    if (natsConn_isClosed(nc)) {
        nc.*.status = status;
        natsConn_unlockAndRelease(nc);
    ...
    nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED));
    _initThreadsToJoin(&ttj, nc, @as(c_int, 1) != 0);
    _clearPendingFlushRequests(nc);
    _clearPendingRequestCalls(nc, @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)));
    if (nc.*.ptmr != @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    if (nc.*.sockCtx.fdActive) {
        if (ttj.readLoop == @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            if (nc.*.el.attached) {
                _ = _evStopPolling(nc);
            } else {
        ...
        nc.*.sockCtx.fdActive = @as(c_int, 0) != 0;
        sockWasActive = @as(c_int, 1) != 0;
    }
    if ((((@as(c_int, @intFromBool(doCBs)) != 0) and !nc.*.rle) and (@as(c_int, @intFromBool(postDisconnectedCb)) != 0)) and (@as(c_int, @intFromBool(sockWasActive)) != 0)) {
        natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_DISCONNECTED)));
    }
    sub = nc.*.respMux;
    nc.*.respMux = null;
    _ = natsMutex_Unlock(nc.*.mu);
    ...
    _joinThreads(&ttj);
    _ = natsMutex_Lock(nc.*.mu);
    if ((@as(c_int, @intFromBool(doCBs)) != 0) and (@as(c_int, @intFromBool(postClosedCb)) != 0)) {
        natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_CLOSED)));
    }
    nc.*.status = status;
    if (nc.*.el.attached) {
        nc.*.el.attached = @as(c_int, 0) != 0;
        detach = @as(c_int, 1) != 0;
        _ = blk: {
            const ref = &nc.*.refs;
            const tmp = ref.*;
    ...
    natsConn_unlockAndRelease(nc);
    if (detach) {
        _ = nc.*.opts.*.evCbs.detach.?(nc.*.el.data);
        natsConn_release(nc);
    }
}
pub fn _processOpError(arg_nc: [*c]natsConnection, arg_s: natsStatus, arg_initialConnect: bool) callconv(.c) bool {
    var nc = arg_nc;
    _ = &nc;
    var s = arg_s;
    _ = &s;
    var initialConnect = arg_initialConnect;
    _ = &initialConnect;
    _ = natsMutex_Lock(nc.*.mu);
    if (!initialConnect) {
        if (((@as(c_int, @intFromBool(_isConnecting(nc))) != 0) or (@as(c_int, @intFromBool(natsConn_isClosed(nc))) != 0)) or (nc.*.inReconnect > @as(c_int, 0))) {
            _ = natsMutex_Unlock(nc.*.mu);
            return @as(c_int, 0) != 0;
        }
    }
    if ((@as(c_int, @intFromBool(initialConnect)) != 0) or ((@as(c_int, @intFromBool(nc.*.opts.*.allowReconnect)) != 0) and (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))))) {
        var ls: natsStatus = @as(c_uint, @bitCast(NATS_OK));
        _ = &ls;
        nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_RECONNECTING));
        if (nc.*.ptmr != @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            natsTimer_Stop(nc.*.ptmr);
        }
        if (nc.*.sockCtx.fdActive) {
        ...
        if (ls == @as(c_uint, @bitCast(NATS_OK))) {
            _clearPendingFlushRequests(nc);
        }
        if ((ls == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(nc.*.opts.*.failRequestsOnDisconnect)) != 0)) {
            _clearPendingRequestCalls(nc, @as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED)));
        }
        if (ls == @as(c_uint, @bitCast(NATS_OK))) {
            ls = natsBuf_Create(&nc.*.pending, nc.*.opts.*.reconnectBufSize);
        }
        if (ls == @as(c_uint, @bitCast(NATS_OK))) {
            nc.*.usePending = @as(c_int, 1) != 0;
            ls = natsThread_Create(&nc.*.reconnectThread, &_doReconnect, @as(?*anyopaque, @ptrCast(nc)));
        }
        if (ls == @as(c_uint, @bitCast(NATS_OK))) {
            _ = blk: {
                const ref = &nc.*.refs;
    ...
    nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_DISCONNECTED));
    nc.*.err = s;
    _ = natsMutex_Unlock(nc.*.mu);
    _close(nc, @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED)), @as(c_int, 0) != 0, @as(c_int, 1) != 0);
    return @as(c_int, 0) != 0;
}
pub fn _flushTimeout(arg_nc: [*c]natsConnection, arg_timeout: i64) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var timeout = arg_timeout;
    _ = &timeout;
    ...
    var pong: [*c]natsPong = null;
    _ = &pong;
    if (nc.*.pongs.head == @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        pong = &nc.*.pongs.cached;
    } else {
        pong = @as([*c]natsPong, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsPong)))));
    }
    if (pong == @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_flushTimeout", @as(c_int, 3538), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        _sendPing(nc, pong);
        target = nats_setTargetTime(timeout);
        while (((s != @as(c_uint, @bitCast(NATS_TIMEOUT))) and !natsConn_isClosed(nc)) and (pong.*.id > @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))))) {
            s = natsCondition_AbsoluteTimedWait(nc.*.pongs.cond, nc.*.mu, target);
        }
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CLOSED)))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_flushTimeout", @as(c_int, 3560), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
        } else if ((s == @as(c_uint, @bitCast(NATS_OK))) and (pong.*.id == @as(i64, @bitCast(@as(c_long, -@as(c_int, 1)))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_flushTimeout", @as(c_int, 3566), @as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_DISCONNECTED))));
        }
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            _removePongFromList(nc, pong);
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_flushTimeout", @as(c_int, 3577), s, "%s", natsStatus_GetText(s));
        }
        _destroyPong(nc, pong);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_flushTimeout");
}
pub fn _processAuthError(arg_nc: [*c]natsConnection, arg_errCode: c_int, arg_error: [*c]u8) callconv(.c) bool {
    var nc = arg_nc;
    _ = &nc;
    ...
    var @"error" = arg_error;
    _ = &@"error";
    nc.*.err = @as(c_uint, @bitCast(NATS_CONNECTION_AUTH_FAILED));
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&nc.*.errStr))), @sizeOf([256]u8), "%s", @"error");
    if (!nc.*.initc) {
        natsAsyncCb_PostErrHandler(nc, null, nc.*.err, strdup(@"error"));
    }
...
pub fn _checkAuthError(arg_error: [*c]u8) callconv(.c) c_int {
    var @"error" = arg_error;
    _ = &@"error";
    if (strcasestr(@"error", "Authorization Violation") != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_int, 2) else if (strcasestr(@"error", "User Authentication Expired") != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_int, 1);
    return 0;
}
pub export var threadsToJoin: struct_threadsToJoin = @import("std").mem.zeroes(struct_threadsToJoin);
pub fn _initThreadsToJoin(arg_ttj: [*c]struct_threadsToJoin, arg_nc: [*c]natsConnection, arg_joinReconnect: bool) callconv(.c) void {
    var ttj = arg_ttj;
    _ = &ttj;
    var nc = arg_nc;
    _ = &nc;
    var joinReconnect = arg_joinReconnect;
    _ = &joinReconnect;
    _ = memset(@as(?*anyopaque, @ptrCast(ttj)), @as(c_int, 0), @sizeOf(struct_threadsToJoin));
    ttj.*.joinReconnect = joinReconnect;
    if (nc.*.readLoopThread != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        ttj.*.readLoop = nc.*.readLoopThread;
        nc.*.readLoopThread = null;
    }
    if ((@as(c_int, @intFromBool(joinReconnect)) != 0) and (nc.*.reconnectThread != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        ttj.*.reconnect = nc.*.reconnectThread;
        nc.*.reconnectThread = null;
    }
    if (nc.*.flusherThread != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        nc.*.flusherStop = @as(c_int, 1) != 0;
        natsCondition_Signal(nc.*.flusherCond);
        ttj.*.flusher = nc.*.flusherThread;
        nc.*.flusherThread = null;
    }
}
pub fn _joinThreads(arg_ttj: [*c]struct_threadsToJoin) callconv(.c) void {
    var ttj = arg_ttj;
    _ = &ttj;
    if (ttj.*.readLoop != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    if ((@as(c_int, @intFromBool(ttj.*.joinReconnect)) != 0) and (ttj.*.reconnect != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        natsThread_Join(ttj.*.reconnect);
        natsThread_Destroy(ttj.*.reconnect);
    }
    if (ttj.*.flusher != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        natsThread_Join(ttj.*.flusher);
        natsThread_Destroy(ttj.*.flusher);
    }
}
pub fn _clearServerInfo(arg_si: [*c]natsServerInfo) callconv(.c) void {
    var si = arg_si;
    _ = &si;
    var i: c_int = undefined;
    _ = &i;
    free(@as(?*anyopaque, @ptrCast(si.*.id)));
    free(@as(?*anyopaque, @ptrCast(si.*.host)));
    free(@as(?*anyopaque, @ptrCast(si.*.version)));
    {
        i = 0;
        while (i < si.*.connectURLsCount) : (i += 1) {
            free(@as(?*anyopaque, @ptrCast((blk: {
                const tmp = i;
                if (tmp >= 0) break :blk si.*.connectURLs + @as(usize, @intCast(tmp)) else break :blk si.*.connectURLs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*)));
        }
    }
    free(@as(?*anyopaque, @ptrCast(si.*.connectURLs)));
    free(@as(?*anyopaque, @ptrCast(si.*.nonce)));
    free(@as(?*anyopaque, @ptrCast(si.*.clientIP)));
    _ = memset(@as(?*anyopaque, @ptrCast(si)), @as(c_int, 0), @sizeOf(natsServerInfo));
}
pub fn _freeConn(arg_nc: [*c]natsConnection) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    ...
    if (nc.*.sockCtx.ssl != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        nc.*.sockCtx.ssl = null;
    }
    free(nc.*.el.buffer);
    ...
    free(@as(?*anyopaque, @ptrCast(nc.*.services)));
    free(@as(?*anyopaque, @ptrCast(nc)));
    natsLib_Release();
}
pub export fn natsConn_lockAndRetain(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    _ = natsMutex_Lock(nc.*.mu);
    nc.*.refs += 1;
}
pub export fn natsConn_unlockAndRelease(arg_nc: [*c]natsConnection) void {
    var nc = arg_nc;
    _ = &nc;
    var refs: c_int = 0;
    _ = &refs;
...
pub export fn natsConn_bufferWriteString(arg_nc: [*c]natsConnection, arg_string: [*c]const u8) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var string = arg_string;
    _ = &string;
    var s: natsStatus = natsConn_bufferWrite(nc, string, @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(string))))));
    _ = &s;
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_bufferWriteString");
}
pub fn _createConn(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.sockCtx.fdActive = @as(c_int, 1) != 0;
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) or (@as(c_int, @intFromBool(nc.*.opts.*.retryOnFailedConnect)) != 0)) {
        var ls: natsStatus = @as(c_uint, @bitCast(NATS_OK));
        _ = &ls;
        if (nc.*.bw == @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_createConn");
}
pub fn _clearControlContent(arg_control: [*c]natsControl) callconv(.c) void {
    var control = arg_control;
    _ = &control;
    free(@as(?*anyopaque, @ptrCast(control.*.op)));
    free(@as(?*anyopaque, @ptrCast(control.*.args)));
}
pub fn _initControlContent(arg_control: [*c]natsControl) callconv(.c) void {
    var control = arg_control;
    _ = &control;
    control.*.op = null;
    control.*.args = null;
}
pub fn _isConnecting(arg_nc: [*c]natsConnection) callconv(.c) bool {
    var nc = arg_nc;
    _ = &nc;
    return nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTING));
}
pub fn _isConnected(arg_nc: [*c]natsConnection) callconv(.c) bool {
    var nc = arg_nc;
    _ = &nc;
    return (nc.*.status == @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))) or (@as(c_int, @intFromBool(natsConn_isDraining(nc))) != 0);
}
pub fn _readOp(arg_nc: [*c]natsConnection, arg_control: [*c]natsControl) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var control = arg_control;
    _ = &control;
    ...
    _ = &buffer;
    buffer[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    s = natsSock_ReadLine(&nc.*.sockCtx, @as([*c]u8, @ptrCast(@alignCast(&buffer))), @sizeOf([32768]u8));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = nats_ParseControl(control, @as([*c]u8, @ptrCast(@alignCast(&buffer))));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_readOp");
}
pub fn _unpackSrvVersion(arg_nc: [*c]natsConnection) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    nc.*.srvVersion.ma = 0;
    nc.*.srvVersion.mi = 0;
    nc.*.srvVersion.up = 0;
    if ((if ((nc.*.info.version == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, nc.*.info.version[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0) return;
    _ = sscanf(nc.*.info.version, "%d.%d.%d", &nc.*.srvVersion.ma, &nc.*.srvVersion.mi, &nc.*.srvVersion.up);
}
pub fn _processInfo(arg_nc: [*c]natsConnection, arg_info: [*c]u8, arg_len: c_int) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var info = arg_info;
    _ = &info;
    ...
    _ = &json;
    var postDiscoveredServersCb: bool = @as(c_int, 0) != 0;
    _ = &postDiscoveredServersCb;
    var postLameDuckCb: bool = @as(c_int, 0) != 0;
    _ = &postLameDuckCb;
    if (info == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_uint, @bitCast(NATS_OK));
    natsOptions_lock(nc.*.opts);
    postDiscoveredServersCb = nc.*.opts.*.discoveredServersCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    postLameDuckCb = nc.*.opts.*.lameDuckCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    natsOptions_unlock(nc.*.opts);
    _clearServerInfo(&nc.*.info);
    s = nats_JSONParse(&json, info, len);
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_processInfo");
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = nats_JSONGetBool(json, "tls_available", &nc.*.info.tlsAvailable);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = nats_JSONGetLong(json, "max_payload", &nc.*.info.maxPayload);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = nats_JSONGetArrayStr(json, "connect_urls", &nc.*.info.connectURLs, &nc.*.info.connectURLsCount);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if (((s == @as(c_uint, @bitCast(NATS_OK))) and !nc.*.opts.*.ignoreDiscoveredServers) and (nc.*.info.connectURLsCount > @as(c_int, 0))) {
        var added: bool = @as(c_int, 0) != 0;
        _ = &added;
        var tlsName: [*c]const u8 = null;
        _ = &tlsName;
        if (((nc.*.cur != @as([*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (nc.*.cur.*.url != @as([*c]natsUrl, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) and !nats_HostIsIP(nc.*.cur.*.url.*.host)) {
            tlsName = @as([*c]const u8, @ptrCast(@alignCast(nc.*.cur.*.url.*.host)));
        }
        s = natsSrvPool_addNewURLs(nc.*.srvPool, nc.*.cur.*.url, nc.*.info.connectURLs, nc.*.info.connectURLsCount, tlsName, &added);
        if ((((s == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(added)) != 0)) and !nc.*.initc) and (@as(c_int, @intFromBool(postDiscoveredServersCb)) != 0)) {
            natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_DISCOVERED_SERVERS)));
        }
    }
    if (((s == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(nc.*.info.lameDuckMode)) != 0)) and (@as(c_int, @intFromBool(postLameDuckCb)) != 0)) {
        natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_LAME_DUCK_MODE)));
    }
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_processInfo", @as(c_int, 599), @as(c_uint, @bitCast(NATS_PROTOCOL_ERROR)), "Invalid protocol: %s", nats_GetLastError(null));
    }
    nats_JSONDestroy(json);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_processInfo");
}
pub fn _makeTLSConn(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_makeTLSConn", @as(c_int, 777), @as(c_uint, @bitCast(NATS_ILLEGAL_STATE)), "%s", "The library was built without SSL support!");
}
pub fn _checkForSecure(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (((@as(c_int, @intFromBool(nc.*.opts.*.secure)) != 0) and !nc.*.info.tlsRequired) and !nc.*.info.tlsAvailable) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_checkForSecure", @as(c_int, 791), @as(c_uint, @bitCast(NATS_SECURE_CONNECTION_WANTED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_SECURE_CONNECTION_WANTED))));
    } else if ((@as(c_int, @intFromBool(nc.*.info.tlsRequired)) != 0) and !nc.*.opts.*.secure) {
        s = natsOptions_SetSecure(nc.*.opts, @as(c_int, 1) != 0);
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(nc.*.opts.*.secure)) != 0)) {
        if (!nc.*.opts.*.tlsHandshakeFirst) {
            s = _makeTLSConn(nc);
        }
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_checkForSecure");
}
pub fn _processExpectedInfo(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var control: natsControl = undefined;
    _ = &control;
    var s: natsStatus = undefined;
    _ = &s;
    _initControlContent(&control);
    s = _readOp(nc, &control);
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_processExpectedInfo");
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and ((control.op == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (strcmp(control.op, "INFO") != @as(c_int, 0)))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_processExpectedInfo", @as(c_int, 828), @as(c_uint, @bitCast(NATS_PROTOCOL_ERROR)), "Unexpected protocol: got '%s' instead of '%s'", if (control.op == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "<null>" else control.op, "INFO");
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = _processInfo(nc, control.args, -@as(c_int, 1));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = _checkForSecure(nc);
    }
    _clearControlContent(&control);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_processExpectedInfo");
}
pub fn _escape(arg_origin: [*c]u8) callconv(.c) [*c]u8 {
    var origin = arg_origin;
    _ = &origin;
    var escChar: [8]u8 = [8]u8{
        '\x07',
        '\x08',
        '\x0c',
        '\n',
    ...
    _ = &escRepl;
    var l: c_int = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(origin)))));
    _ = &l;
    var ec: c_int = 0;
    _ = &ec;
    var dest: [*c]u8 = null;
        ...
        while (i < l) : (i += 1) {
            {
                ...
                while (j < @as(c_int, 8)) : (j += 1) {
                    if (@as(c_int, @bitCast(@as(c_uint, (blk: {
                        const tmp = i;
                        if (tmp >= 0) break :blk origin + @as(usize, @intCast(tmp)) else break :blk origin - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                    }).*))) == @as(c_int, @bitCast(@as(c_uint, escChar[@as(c_uint, @intCast(j))])))) {
                        ec += 1;
                        break;
                    }
                }
    ...
    if (ec == @as(c_int, 0)) return origin;
    dest = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, (l + ec) + @as(c_int, 1))))))));
    if (dest == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return null;
    ptr = dest;
    {
        i = 0;
        while (i < l) : (i += 1) {
...
pub fn _connectProto(arg_nc: [*c]natsConnection, arg_proto: [*c][*c]u8) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var proto = arg_proto;
    _ = &proto;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var opts: [*c]natsOptions = nc.*.opts;
    _ = &opts;
    var token: [*c]const u8 = null;
    _ = &token;
    var user: [*c]const u8 = null;
    _ = &user;
    var pwd: [*c]const u8 = null;
    _ = &pwd;
    ...
    _ = &res;
    var sigRaw: [*c]u8 = null;
    _ = &sigRaw;
    var sigRawLen: c_int = 0;
    _ = &sigRawLen;
    if ((@as(c_int, @intFromBool(opts.*.noEcho)) != 0) and (nc.*.info.proto < @as(c_int, 1))) return @as(c_uint, @bitCast(NATS_NO_SERVER_SUPPORT));
    if (nc.*.cur.*.url.*.username != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        user = nc.*.cur.*.url.*.username;
    }
    if (nc.*.cur.*.url.*.password != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        pwd = nc.*.cur.*.url.*.password;
    }
    if ((user != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (pwd == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        token = user;
        user = null;
    }
    if ((user == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (token == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        user = opts.*.user;
        pwd = opts.*.password;
        token = opts.*.token;
        nkey = opts.*.nkey;
        if ((((if ((user == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, user[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0) and ((if ((token == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, token[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0)) and (nc.*.srvPool.*.user != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            user = nc.*.srvPool.*.user;
            pwd = nc.*.srvPool.*.pwd;
            if (pwd == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                token = user;
                user = null;
            }
        }
    }
    if (opts.*.userJWTHandler != @as(natsUserJWTHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        var errTxt: [*c]u8 = null;
        _ = &errTxt;
        var userCb: bool = opts.*.userJWTHandler != &natsConn_userCreds;
        _ = &userCb;
        if (userCb) {
            _ = natsMutex_Unlock(nc.*.mu);
        }
        s = opts.*.userJWTHandler.?(&ujwt, &errTxt, opts.*.userJWTClosure);
        if (userCb) {
            _ = natsMutex_Lock(nc.*.mu);
            if ((@as(c_int, @intFromBool(natsConn_isClosed(nc))) != 0) and (s == @as(c_uint, @bitCast(NATS_OK)))) {
                s = @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED));
            }
        }
        if ((s != @as(c_uint, @bitCast(NATS_OK))) and (errTxt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 963), s, "%s", errTxt);
            free(@as(?*anyopaque, @ptrCast(errTxt)));
        }
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and !((if ((nkey == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, nkey[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0)) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 967), @as(c_uint, @bitCast(NATS_ILLEGAL_STATE)), "%s", "user JWT callback and NKey cannot be both specified");
        }
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (ujwt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            var tmp: [*c]u8 = _escape(ujwt);
            _ = &tmp;
            if (tmp == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 974), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            } else if (tmp != ujwt) {
                free(@as(?*anyopaque, @ptrCast(ujwt)));
                ujwt = tmp;
            }
        }
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (!((if ((ujwt == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, ujwt[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0) or !((if ((nkey == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, nkey[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0))) {
        var errTxt: [*c]u8 = null;
        _ = &errTxt;
        var userCb: bool = opts.*.sigHandler != &natsConn_signatureHandler;
        _ = &userCb;
        if (userCb) {
            _ = natsMutex_Unlock(nc.*.mu);
        }
        ...
        if ((s != @as(c_uint, @bitCast(NATS_OK))) and (errTxt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 1002), s, "%s", errTxt);
            free(@as(?*anyopaque, @ptrCast(errTxt)));
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            s = nats_Base64RawURL_EncodeString(@as([*c]const u8, @ptrCast(@alignCast(sigRaw))), sigRawLen, &sig);
        }
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (opts.*.tokenCb != @as(natsTokenHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        if (token != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 1012), @as(c_uint, @bitCast(NATS_ILLEGAL_STATE)), "%s", "Token and token handler options cannot be both set");
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            token = opts.*.tokenCb.?(opts.*.tokenCbClosure);
        }
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (opts.*.name != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        name = opts.*.name;
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        var hdrs: [*c]const u8 = nats_GetBoolStr(nc.*.info.headers);
        _ = &hdrs;
        var noResponders: [*c]const u8 = nats_GetBoolStr((@as(c_int, @intFromBool(nc.*.info.headers)) != 0) and !nc.*.opts.*.disableNoResponders);
        _ = &noResponders;
        res = asprintf(proto, "CONNECT {\"verbose\":%s,\"pedantic\":%s,%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\"tls_required\":%s,\"name\":\"%s\",\"lang\":\"%s\",\"version\":\"%s\",\"protocol\":%d,\"echo\":%s,\"headers\":%s,\"no_responders\":%s}%s", nats_GetBoolStr(opts.*.verbose), nats_GetBoolStr(opts.*.pedantic), if (nkey != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"nkey\":\"" else "", if (nkey != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) nkey else "", if (nkey != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", if (ujwt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"jwt\":\"" else "", if (ujwt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) ujwt else "", if (ujwt != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", if (sig != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"sig\":\"" else "", if (sig != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) sig else "", if (sig != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", if (user != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"user\":\"" else "", if (user != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) user else "", if (user != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", if (pwd != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"pass\":\"" else "", if (pwd != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) pwd else "", if (pwd != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", if (token != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"auth_token\":\"" else "", if (token != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) token else "", if (token != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) "\"," else "", nats_GetBoolStr(opts.*.secure), if (name != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) name else "", "C", "3.10.0-beta", @as(c_int, 1), nats_GetBoolStr(!opts.*.noEcho), hdrs, noResponders, "\r\n");
        if (res < @as(c_int, 0)) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connectProto", @as(c_int, 1060), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        }
    }
    free(@as(?*anyopaque, @ptrCast(ujwt)));
    free(@as(?*anyopaque, @ptrCast(sigRaw)));
    free(@as(?*anyopaque, @ptrCast(sig)));
    return s;
}
pub fn _resendSubscriptions(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var sub: [*c]natsSubscription = null;
    _ = &sub;
    var iter: natsHashIter = undefined;
    _ = &iter;
    var adjustedMax: c_int = undefined;
    _ = &adjustedMax;
    var subs: [*c][*c]natsSubscription = null;
    _ = &subs;
    var i: c_int = 0;
    _ = &i;
    var count: c_int = 0;
    _ = &count;
    natsMutex_Lock(nc.*.subsMu);
    if (nc.*.subs.*.used > @as(c_int, 0)) {
        subs = @as([*c][*c]natsSubscription, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, nc.*.subs.*.used))), @sizeOf([*c]natsSubscription)))));
        if (subs == @as([*c][*c]natsSubscription, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = @as(c_uint, @bitCast(NATS_NO_MEMORY));
        }
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            var p: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
            _ = &p;
            ...
            while (natsHashIter_Next(&iter, null, &p)) {
                (blk: {
                    ...
                    if (tmp >= 0) break :blk subs + @as(usize, @intCast(tmp)) else break :blk subs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).* = @as([*c]natsSubscription, @ptrCast(@alignCast(p)));
            }
    ...
    {
        i = 0;
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (i < count)) : (i += 1) {
            sub = (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk subs + @as(usize, @intCast(tmp)) else break :blk subs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            ...
            if ((sub.*.jsi != @as([*c]jsSub, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (@as(c_int, @intFromBool(sub.*.jsi.*.ordered)) != 0)) {
                _ = jsSub_resetOrderedConsumer(sub, sub.*.jsi.*.sseq +% @as(u64, @bitCast(@as(c_long, @as(c_int, 1)))));
                nats_unlockSubAndDispatcher(sub);
                continue;
            }
            if ((@as(c_int, @bitCast(@as(c_uint, sub.*.drainState))) & @as(c_int, @bitCast(@as(c_uint, @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 1))))))))) != @as(c_int, 0)) {
                nats_unlockSubAndDispatcher(sub);
                continue;
            }
            if (sub.*.max > @as(u64, @bitCast(@as(c_long, @as(c_int, 0))))) {
                if (sub.*.delivered < sub.*.max) {
                    adjustedMax = @as(c_int, @bitCast(@as(c_uint, @truncate(sub.*.max -% sub.*.delivered))));
                }
                if (adjustedMax == @as(c_int, 0)) {
                    nats_unlockSubAndDispatcher(sub);
                    s = natsConn_sendUnsubProto(nc, sub.*.sid, @as(c_int, 0));
                    continue;
                }
            }
            s = natsConn_sendSubProto(nc, sub.*.subject, sub.*.queue, sub.*.sid);
            if ((s == @as(c_uint, @bitCast(NATS_OK))) and (adjustedMax > @as(c_int, 0))) {
                s = natsConn_sendUnsubProto(nc, sub.*.sid, adjustedMax);
            }
            nats_unlockSubAndDispatcher(sub);
        }
    }
    free(@as(?*anyopaque, @ptrCast(subs)));
    return s;
}
pub fn _flushReconnectPendingItems(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (nc.*.pending == @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(c_uint, @bitCast(NATS_OK));
    if (nc.*.pending.*.len > @as(c_int, 0)) {
        s = natsConn_bufferWrite(nc, nc.*.pending.*.data, nc.*.pending.*.len);
        natsBuf_Reset(nc.*.pending);
    }
    return s;
}
pub fn _removePongFromList(arg_nc: [*c]natsConnection, arg_pong: [*c]natsPong) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var pong = arg_pong;
    _ = &pong;
    if (pong.*.prev != @as([*c]struct___natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        pong.*.prev.*.next = pong.*.next;
    }
    if (pong.*.next != @as([*c]struct___natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub fn _clearPendingFlushRequests(arg_nc: [*c]natsConnection) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var pong: [*c]natsPong = null;
    _ = &pong;
    while ((blk: {
        ...
        pong = tmp;
        break :blk tmp;
    }) != @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _removePongFromList(nc, pong);
        pong.*.id = @as(i64, @bitCast(@as(c_long, -@as(c_int, 1))));
        natsCondition_Broadcast(nc.*.pongs.cond);
    }
    nc.*.pongs.incoming = 0;
    nc.*.pongs.outgoingPings = 0;
}
pub fn _clearPendingRequestCalls(arg_nc: [*c]natsConnection, arg_reason: natsStatus) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var reason = arg_reason;
    _ = &reason;
    var iter: natsStrHashIter = undefined;
    _ = &iter;
    var p: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &p;
    if (nc.*.respMap == @as([*c]natsStrHash, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    natsStrHashIter_Init(&iter, nc.*.respMap);
    while (natsStrHashIter_Next(&iter, null, &p)) {
        var val: [*c]respInfo = @as([*c]respInfo, @ptrCast(@alignCast(p)));
        _ = &val;
        natsMutex_Lock(val.*.mu);
        val.*.closed = @as(c_int, 1) != 0;
        val.*.closedSts = reason;
        val.*.removed = @as(c_int, 1) != 0;
...
pub fn _clearSSL(arg_nc: [*c]natsConnection) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    if (nc.*.sockCtx.ssl == @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    {
        nc.*.sockCtx.ssl = null;
    }
    nc.*.sockCtx.ssl = null;
}
pub fn _doReconnect(arg_arg: ?*anyopaque) callconv(.c) void {
    var arg = arg_arg;
    _ = &arg;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var nc: [*c]natsConnection = @as([*c]natsConnection, @ptrCast(@alignCast(arg)));
    _ = &nc;
    var pool: [*c]natsSrvPool = null;
    _ = &pool;
    var sleepTime: i64 = 0;
    _ = &sleepTime;
    var ttj: struct_threadsToJoin = undefined;
    _ = &ttj;
    var rt: [*c]natsThread = null;
    _ = &rt;
    var wlf: c_int = 0;
    _ = &wlf;
    var doSleep: bool = @as(c_int, 0) != 0;
    _ = &doSleep;
    var jitter: i64 = 0;
    _ = &jitter;
    var i: c_int = 0;
    _ = &i;
    var crd: natsCustomReconnectDelayHandler = null;
    _ = &crd;
    var crdClosure: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &crdClosure;
    var postDisconnectedCb: bool = @as(c_int, 0) != 0;
    _ = &postDisconnectedCb;
    var postReconnectedCb: bool = @as(c_int, 0) != 0;
    _ = &postReconnectedCb;
    var postConnectedCb: bool = @as(c_int, 0) != 0;
    _ = &postConnectedCb;
    natsOptions_lock(nc.*.opts);
    postDisconnectedCb = nc.*.opts.*.disconnectedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    postReconnectedCb = nc.*.opts.*.reconnectedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    postConnectedCb = nc.*.opts.*.connectedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    natsOptions_unlock(nc.*.opts);
    _ = natsMutex_Lock(nc.*.mu);
    _initThreadsToJoin(&ttj, nc, @as(c_int, 0) != 0);
    _ = natsMutex_Unlock(nc.*.mu);
    _joinThreads(&ttj);
    _ = natsMutex_Lock(nc.*.mu);
    nc.*.err = @as(c_uint, @bitCast(NATS_OK));
    nc.*.errStr[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
    pool = nc.*.srvPool;
    if (!nc.*.initc and (@as(c_int, @intFromBool(postDisconnectedCb)) != 0)) {
        natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_DISCONNECTED)));
    }
    crd = nc.*.opts.*.customReconnectDelayCB;
    if (crd == @as(natsCustomReconnectDelayHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        jitter = nc.*.opts.*.reconnectJitter;
        if ((@as(c_int, @intFromBool(nc.*.opts.*.secure)) != 0) or (nc.*.opts.*.sslCtx != @as([*c]natsSSLCtx, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
    ...
    {
        i = 0;
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (pool.*.size > @as(c_int, 0))) {
            nc.*.cur = natsSrvPool_GetNextServer(pool, nc.*.opts, nc.*.cur);
            if (nc.*.cur == @as([*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                nc.*.err = @as(c_uint, @bitCast(NATS_NO_SERVER));
                break;
            }
            doSleep = (i + @as(c_int, 1)) >= pool.*.size;
            if (doSleep) {
                i = 0;
                if (crd != @as(natsCustomReconnectDelayHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    wlf += 1;
                    ...
                    if (jitter > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
                        sleepTime += @import("std").zig.c_translation.signedRemainder(@as(i64, @bitCast(@as(c_long, rand()))), jitter);
                    }
                }
                if (natsConn_isClosed(nc)) break;
                _ = natsCondition_TimedWait(nc.*.reconnectCond, nc.*.mu, sleepTime);
            } else {
                i += 1;
                _ = natsMutex_Unlock(nc.*.mu);
                natsThread_Yield();
            ...
            nc.*.stats.reconnects +%= @as(u64, @bitCast(@as(c_long, @as(c_int, 1))));
            s = _processConnInit(nc);
            if ((@as(c_int, @intFromBool(natsConn_isClosed(nc))) != 0) or (@as(c_int, @intFromBool(nc.*.ar)) != 0)) {
                if (s == @as(c_uint, @bitCast(NATS_OK))) {
                    s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_doReconnect", @as(c_int, 1668), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", "connection has been closed/destroyed while reconnecting");
                }
                break;
            }
            nc.*.usePending = @as(c_int, 0) != 0;
            if (s == @as(c_uint, @bitCast(NATS_OK))) {
            ...
            nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED));
            nc.*.err = @as(c_uint, @bitCast(NATS_OK));
            nc.*.errStr[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
            nc.*.cur.*.lastAuthErrCode = 0;
            nc.*.cur.*.didConnect = @as(c_int, 1) != 0;
            nc.*.cur.*.reconnects = 0;
            natsBuf_Destroy(nc.*.pending);
            nc.*.pending = null;
            nc.*.usePending = @as(c_int, 0) != 0;
            if (nc.*.initc) {
                nc.*.initc = @as(c_int, 0) != 0;
                if (postConnectedCb) {
                    natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_CONNECTED)));
                }
            } else {
                if (postReconnectedCb) {
                    natsAsyncCb_PostConnHandler(nc, @as(c_uint, @bitCast(ASYNC_RECONNECTED)));
                }
            }
            nc.*.inReconnect -= 1;
            if (nc.*.reconnectThread != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                rt = nc.*.reconnectThread;
                nc.*.reconnectThread = null;
            }
            _ = natsMutex_Unlock(nc.*.mu);
            _ = natsConnection_Flush(nc);
            natsConn_release(nc);
            if (rt != @as([*c]natsThread, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    if (nc.*.err == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.err = @as(c_uint, @bitCast(NATS_NO_SERVER));
    }
    nc.*.inReconnect -= 1;
    nc.*.rle = @as(c_int, 1) != 0;
...
pub fn _readProto(arg_nc: [*c]natsConnection, arg_proto: [*c][*c]natsBuffer) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var proto = arg_proto;
    _ = &proto;
    ...
    _ = &buf;
    var oneChar: [1]u8 = [1]u8{
        '\x00',
    };
    _ = &oneChar;
    s = natsBuf_Create(&buf, @as(c_int, 10));
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_readProto");
    while (true) {
        s = natsSock_Read(&nc.*.sockCtx, @as([*c]u8, @ptrCast(@alignCast(&oneChar))), @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))), null);
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            natsBuf_Destroy(buf);
            return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_readProto");
        }
        s = natsBuf_AppendByte(buf, oneChar[@as(c_uint, @intCast(@as(c_int, 0)))]);
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            natsBuf_Destroy(buf);
            return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_readProto");
        }
        if (@as(c_int, @bitCast(@as(c_uint, oneChar[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, @bitCast(@as(c_uint, protoEnd)))) break;
    }
    s = natsBuf_AppendByte(buf, @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, '\x00'))))));
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        natsBuf_Destroy(buf);
        return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_readProto");
    }
...
pub fn _sendConnect(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var cProto: [*c]u8 = null;
    _ = &cProto;
    var proto: [*c]natsBuffer = null;
    _ = &proto;
    var rup: bool = nc.*.pending != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    _ = &rup;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsConn_bufferWrite(nc, "PING", @as(c_int, 4));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsConn_bufferWrite(nc, "\r\n", @as(c_int, 2));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsConn_bufferFlush(nc);
    }
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(nc.*.opts.*.verbose)) != 0)) {
        if (strncmp(proto.*.data, "+OK", @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 3))))) != @as(c_int, 0)) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_sendConnect", @as(c_int, 1901), @as(c_uint, @bitCast(NATS_PROTOCOL_ERROR)), "Expected '%s', got '%s'", "+OK", proto.*.data);
        }
    ...
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (strncmp(proto.*.data, "PONG", @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 4))))) != @as(c_int, 0))) {
        if (strncmp(proto.*.data, "-ERR", @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 4))))) == @as(c_int, 0)) {
            var buffer: [256]u8 = undefined;
            _ = &buffer;
            var authErrCode: c_int = 0;
            _ = &authErrCode;
            buffer[@as(c_uint, @intCast(@as(c_int, 0)))] = '\x00';
            if (snprintf(@as([*c]u8, @ptrCast(@alignCast(&buffer))), @sizeOf([256]u8), "%s", proto.*.data) >= @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([256]u8)))))) {
                var offset: c_int = @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([256]u8))))) - @as(c_int, 2);
                _ = &offset;
                if (offset > @as(c_int, 0)) {
            ...
            nats_NormalizeErr(@as([*c]u8, @ptrCast(@alignCast(&buffer))));
            if ((blk: {
                const tmp = _checkAuthError(@as([*c]u8, @ptrCast(@alignCast(&buffer))));
                authErrCode = tmp;
                break :blk tmp;
            }) != @as(c_int, 0)) {
                _ = _processAuthError(nc, authErrCode, @as([*c]u8, @ptrCast(@alignCast(&buffer))));
                s = nc.*.err;
            } else {
                s = @as(c_uint, @bitCast(NATS_ERR));
            }
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_sendConnect", @as(c_int, 1939), s, "%s", @as([*c]u8, @ptrCast(@alignCast(&buffer))));
        } else {
            s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_sendConnect", @as(c_int, 1945), @as(c_uint, @bitCast(NATS_PROTOCOL_ERROR)), "Expected '%s', got '%s'", "PONG", proto.*.data);
        }
    }
    natsBuf_Destroy(proto);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    free(@as(?*anyopaque, @ptrCast(cProto)));
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_sendConnect");
}
pub fn _connect(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var retSts: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &retSts;
    var pool: [*c]natsSrvPool = null;
    _ = &pool;
    var i: c_int = 0;
    ...
    var wtime: i64 = 0;
    _ = &wtime;
    var retry: bool = @as(c_int, 0) != 0;
    _ = &retry;
    var retryOnFailedConnect: bool = @as(c_int, 0) != 0;
    _ = &retryOnFailedConnect;
    var hasConnectedCb: bool = @as(c_int, 0) != 0;
    _ = &hasConnectedCb;
    natsOptions_lock(nc.*.opts);
    hasConnectedCb = nc.*.opts.*.connectedCb != @as(natsConnectionHandler, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))));
    retryOnFailedConnect = nc.*.opts.*.retryOnFailedConnect;
    natsOptions_unlock(nc.*.opts);
    _ = natsMutex_Lock(nc.*.mu);
    nc.*.initc = @as(c_int, 1) != 0;
    pool = nc.*.srvPool;
    if ((@as(c_int, @intFromBool(retryOnFailedConnect)) != 0) and !hasConnectedCb) {
        retry = @as(c_int, 1) != 0;
    ...
    while (true) {
        {
            i = 0;
            while (i < pool.*.size) : (i += 1) {
                nc.*.cur = (blk: {
                    const tmp = i;
                    if (tmp >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*;
                s = _createConn(nc);
                if (s == @as(c_uint, @bitCast(NATS_OK))) {
                    s = _processConnInit(nc);
                    if (s == @as(c_uint, @bitCast(NATS_OK))) {
                        ...
                        _ = blk: {
                            ...
                            (blk_1: {
                                const tmp_2 = i;
                                if (tmp_2 >= 0) break :blk_1 pool.*.srvrs + @as(usize, @intCast(tmp_2)) else break :blk_1 pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
                            }).*.*.didConnect = tmp;
                            break :blk tmp;
                        };
                        _ = blk: {
                        ...
                        retSts = @as(c_uint, @bitCast(NATS_OK));
                        retry = @as(c_int, 0) != 0;
                        break;
                    } else {
                        retSts = s;
                        _ = natsMutex_Unlock(nc.*.mu);
                        _close(nc, @as(c_uint, @bitCast(NATS_CONN_STATUS_DISCONNECTED)), @as(c_int, 0) != 0, @as(c_int, 0) != 0);
                        _ = natsMutex_Lock(nc.*.mu);
                        nc.*.cur = null;
                    }
                } else {
                    ...
                    if (s == @as(c_uint, @bitCast(NATS_IO_ERROR))) {
                        retSts = @as(c_uint, @bitCast(NATS_OK));
                    }
                }
            }
        ...
        if (!retry) break;
        l += 1;
        if ((max > @as(c_int, 0)) and (l > max)) break;
        if (wtime > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
            nats_Sleep(wtime);
        }
    }
    if (((nc.*.status != @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))) and (@as(c_int, @intFromBool(retryOnFailedConnect)) != 0)) and (@as(c_int, @intFromBool(hasConnectedCb)) != 0)) {
        _ = natsMutex_Unlock(nc.*.mu);
        if (_processOpError(nc, retSts, @as(c_int, 1) != 0)) {
            nats_clearLastError();
            return @as(c_uint, @bitCast(NATS_NOT_YET_CONNECTED));
        }
        _ = natsMutex_Lock(nc.*.mu);
    }
    if ((retSts == @as(c_uint, @bitCast(NATS_OK))) and (nc.*.status != @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED)))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_connect", @as(c_int, 2136), @as(c_uint, @bitCast(NATS_NO_SERVER)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_SERVER))));
    }
    nc.*.initc = @as(c_int, 0) != 0;
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_connect");
}
pub fn _evStopPolling(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = undefined;
    _ = &s;
    ...
    s = nc.*.opts.*.evCbs.write.?(nc.*.el.data, @as(c_int, 0) != 0);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = nc.*.opts.*.evCbs.read.?(nc.*.el.data, @as(c_int, 0) != 0);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.sockCtx.fd = -@as(c_int, 1);
        _clearSSL(nc);
...
pub fn _readLoop(arg_arg: ?*anyopaque) callconv(.c) void {
    var arg = arg_arg;
    _ = &arg;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    ...
    _ = &n;
    var bufSize: c_int = undefined;
    _ = &bufSize;
    var nc: [*c]natsConnection = @as([*c]natsConnection, @ptrCast(@alignCast(arg)));
    _ = &nc;
    _ = natsMutex_Lock(nc.*.mu);
    bufSize = nc.*.opts.*.ioBufSize;
    buffer = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, bufSize)))))));
    if (buffer == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_readLoop", @as(c_int, 2277), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    if (nc.*.sockCtx.ssl != @as([*c]?*anyopaque, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    while (((s == @as(c_uint, @bitCast(NATS_OK))) and !natsConn_isClosed(nc)) and !natsConn_isReconnecting(nc)) {
        _ = natsMutex_Unlock(nc.*.mu);
        n = 0;
        s = natsSock_Read(&nc.*.sockCtx, buffer, @as(usize, @bitCast(@as(c_long, bufSize))), &n);
        if ((s == @as(c_uint, @bitCast(NATS_IO_ERROR))) and (__errno_location().* == @as(c_int, 11))) {
            s = @as(c_uint, @bitCast(NATS_OK));
        }
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (n > @as(c_int, 0))) {
            s = natsParser_Parse(nc, buffer, n);
        }
        if (s != @as(c_uint, @bitCast(NATS_OK))) {
            _ = _processOpError(nc, s, @as(c_int, 0) != 0);
...
pub fn _flusher(arg_arg: ?*anyopaque) callconv(.c) void {
    var arg = arg_arg;
    _ = &arg;
    var nc: [*c]natsConnection = @as([*c]natsConnection, @ptrCast(@alignCast(arg)));
    _ = &nc;
    ...
    while (true) {
        _ = natsMutex_Lock(nc.*.mu);
        while (!nc.*.flusherSignaled and !nc.*.flusherStop) {
        ...
        _ = natsCondition_TimedWait(nc.*.flusherCond, nc.*.mu, @as(i64, @bitCast(@as(c_long, @as(c_int, 1)))));
        nc.*.flusherSignaled = @as(c_int, 0) != 0;
        if ((!_isConnected(nc) or (@as(c_int, @intFromBool(natsConn_isClosed(nc))) != 0)) or (@as(c_int, @intFromBool(natsConn_isReconnecting(nc))) != 0)) {
            _ = natsMutex_Unlock(nc.*.mu);
            break;
        }
        if ((@as(c_int, @intFromBool(nc.*.sockCtx.fdActive)) != 0) and (nc.*.bw.*.len > @as(c_int, 0))) {
            if (nc.*.opts.*.writeDeadline > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
                natsDeadline_Init(&nc.*.sockCtx.writeDeadline, nc.*.opts.*.writeDeadline);
            }
            s = natsConn_bufferFlush(nc);
            if ((s != @as(c_uint, @bitCast(NATS_OK))) and (nc.*.err == @as(c_uint, @bitCast(NATS_OK)))) {
                nc.*.err = s;
            }
        }
        _ = natsMutex_Unlock(nc.*.mu);
...
pub fn _sendPing(arg_nc: [*c]natsConnection, arg_pong: [*c]natsPong) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var pong = arg_pong;
    _ = &pong;
    ...
    s = natsConn_bufferWrite(nc, "PING\r\n", @as(c_int, 6));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsConn_bufferFlush(nc);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.pongs.outgoingPings += 1;
        if (pong != @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            pong.*.id = nc.*.pongs.outgoingPings;
            pong.*.next = null;
            pong.*.prev = nc.*.pongs.tail;
            if (nc.*.pongs.tail != @as([*c]natsPong, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub fn _processPingTimer(arg_timer: [*c]natsTimer, arg_arg: ?*anyopaque) callconv(.c) void {
    var timer = arg_timer;
    _ = &timer;
    var arg = arg_arg;
    _ = &arg;
    ...
    _ = &nc;
    _ = natsMutex_Lock(nc.*.mu);
    if (nc.*.status != @as(c_uint, @bitCast(NATS_CONN_STATUS_CONNECTED))) {
        _ = natsMutex_Unlock(nc.*.mu);
        return;
    }
    if ((blk: {
...
pub fn _pingStopppedCb(arg_timer: [*c]natsTimer, arg_closure: ?*anyopaque) callconv(.c) void {
    var timer = arg_timer;
    _ = &timer;
    var closure = arg_closure;
    _ = &closure;
    var nc: [*c]natsConnection = @as([*c]natsConnection, @ptrCast(@alignCast(closure)));
    _ = &nc;
    natsConn_release(nc);
}
pub fn _removeAllSubscriptions(arg_nc: [*c]natsConnection) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var iter: natsHashIter = undefined;
    _ = &iter;
    ...
    natsHashIter_Init(&iter, nc.*.subs);
    while (natsHashIter_Next(&iter, null, &p)) {
        var sub: [*c]natsSubscription = @as([*c]natsSubscription, @ptrCast(@alignCast(p)));
        _ = &sub;
        _ = natsHashIter_RemoveCurrent(&iter);
        natsSub_close(sub, @as(c_int, 1) != 0);
        natsSub_release(sub);
    }
    natsHashIter_Done(&iter);
    natsMutex_Unlock(nc.*.subsMu);
}
pub fn _createMsg(arg_newMsg: [*c][*c]natsMsg, arg_nc: [*c]natsConnection, arg_buf: [*c]u8, arg_bufLen: c_int, arg_hdrLen: c_int) callconv(.c) natsStatus {
    var newMsg = arg_newMsg;
    _ = &newMsg;
    var nc = arg_nc;
    _ = &nc;
    var buf = arg_buf;
    ...
    var reply: [*c]u8 = null;
    _ = &reply;
    var replyLen: c_int = 0;
    _ = &replyLen;
    subjLen = nc.*.ps.*.ma.subject.*.len;
    if (nc.*.ps.*.ma.reply != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        reply = nc.*.ps.*.ma.reply.*.data;
        replyLen = nc.*.ps.*.ma.reply.*.len;
    }
    s = natsMsg_createWithPadding(newMsg, @as([*c]const u8, @ptrCast(@alignCast(nc.*.ps.*.ma.subject.*.data))), subjLen, @as([*c]const u8, @ptrCast(@alignCast(reply))), replyLen, @as([*c]const u8, @ptrCast(@alignCast(buf))), bufLen, nc.*.opts.*.payloadPaddingSize, hdrLen);
    return s;
}
pub fn _processPermissionViolation(arg_nc: [*c]natsConnection, arg_error: [*c]u8) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var @"error" = arg_error;
    _ = &@"error";
    _ = natsMutex_Lock(nc.*.mu);
    nc.*.err = @as(c_uint, @bitCast(NATS_NOT_PERMITTED));
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&nc.*.errStr))), @sizeOf([256]u8), "%s", @"error");
    natsAsyncCb_PostErrHandler(nc, null, nc.*.err, strdup(@"error"));
    _ = natsMutex_Unlock(nc.*.mu);
}
pub export fn natsConn_addSubcription(arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var sub = arg_sub;
    _ = &sub;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var oldSub: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &oldSub;
    s = natsHash_Set(nc.*.subs, sub.*.sid, @as(?*anyopaque, @ptrCast(sub)), &oldSub);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        _ = blk: {
            _ = @sizeOf(c_int);
            break :blk blk_1: {
                break :blk_1 if (oldSub == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))) {} else {
                    __assert_fail("oldSub == NULL", "./src/submodules/nats.c/src/conn.c", @as(c_uint, @bitCast(@as(c_int, 2987))), "natsStatus natsConn_addSubcription(natsConnection *, natsSubscription *)");
                };
            };
        };
        natsSub_retain(sub);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsConn_addSubcription");
}
pub fn _isQueueNameValid(arg_name: [*c]const u8) callconv(.c) bool {
    var name = arg_name;
    _ = &name;
    var i: c_int = undefined;
    _ = &i;
    var len: c_int = undefined;
    _ = &len;
    if ((if ((name == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, name[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0) return @as(c_int, 0) != 0;
    len = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(name)))));
    {
        i = 0;
        while (i < len) : (i += 1) {
            if ((@as(c_int, @bitCast(@as(c_uint, (blk: {
                const tmp = @as(c_int, @bitCast(@as(c_uint, @as(u8, @bitCast((blk_1: {
                    const tmp_2 = i;
                    if (tmp_2 >= 0) break :blk_1 name + @as(usize, @intCast(tmp_2)) else break :blk_1 name - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
                }).*)))));
...
pub fn _setupServerPool(arg_nc: [*c]natsConnection) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var s: natsStatus = undefined;
    _ = &s;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nc.*.cur = (blk: {
            const tmp = @as(c_int, 0);
            if (tmp >= 0) break :blk nc.*.srvPool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk nc.*.srvPool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_setupServerPool");
}
pub fn _processUrlString(arg_opts: [*c]natsOptions, arg_urls: [*c]const u8) callconv(.c) natsStatus {
    var opts = arg_opts;
    _ = &opts;
    var urls = arg_urls;
    _ = &urls;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var serverUrls: [*c][*c]u8 = null;
    _ = &serverUrls;
    var urlsCopy: [*c]u8 = null;
    _ = &urlsCopy;
    var commaPos: [*c]u8 = null;
    _ = &commaPos;
    var ptr: [*c]u8 = null;
    _ = &ptr;
    if (urls != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        ptr = @as([*c]u8, @ptrCast(@volatileCast(@constCast(urls))));
        while ((blk: {
            const tmp = strchr(ptr, @as(c_int, ','));
            ptr = tmp;
            break :blk tmp;
        }) != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            ptr += 1;
    ...
    if (count == @as(c_int, 0)) return natsOptions_SetURL(opts, urls);
    serverUrls = @as([*c][*c]u8, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, count + @as(c_int, 1)))), @sizeOf([*c]u8)))));
    if (serverUrls == @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = @as(c_uint, @bitCast(NATS_NO_MEMORY));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        urlsCopy = strdup(urls);
        if (urlsCopy == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            free(@as(?*anyopaque, @ptrCast(serverUrls)));
            return @as(c_uint, @bitCast(NATS_NO_MEMORY));
        }
    }
    count = 0;
    ptr = urlsCopy;
    while (true) {
        (blk: {
            ...
            if (tmp >= 0) break :blk serverUrls + @as(usize, @intCast(tmp)) else break :blk serverUrls - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).* = ptr;
        commaPos = strchr(ptr, @as(c_int, ','));
        if (commaPos != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            ptr = commaPos + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1)))));
            commaPos.* = '\x00';
        }
        if (!(commaPos != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) break;
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsOptions_SetServers(opts, @as([*c][*c]const u8, @ptrCast(@alignCast(serverUrls))), count);
    }
    free(@as(?*anyopaque, @ptrCast(urlsCopy)));
    free(@as(?*anyopaque, @ptrCast(serverUrls)));
    return s;
}
pub fn _destroyPong(arg_nc: [*c]natsConnection, arg_pong: [*c]natsPong) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var pong = arg_pong;
    _ = &pong;
    if (pong == (&nc.*.pongs.cached)) {
        _ = memset(@as(?*anyopaque, @ptrCast(pong)), @as(c_int, 0), @sizeOf(natsPong));
    } else {
        free(@as(?*anyopaque, @ptrCast(pong)));
    }
}
pub fn _pushDrainErr(arg_nc: [*c]natsConnection, arg_s: natsStatus, arg_errTxt: [*c]const u8) callconv(.c) void {
    var nc = arg_nc;
    _ = &nc;
    var s = arg_s;
    _ = &s;
    ...
    _ = &tmp;
    _ = natsMutex_Lock(nc.*.mu);
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&tmp))), @sizeOf([256]u8), "Drain error: %s: %u (%s)", errTxt, s, natsStatus_GetText(s));
    natsAsyncCb_PostErrHandler(nc, null, s, strdup(@as([*c]u8, @ptrCast(@alignCast(&tmp)))));
    _ = natsMutex_Unlock(nc.*.mu);
}
pub const subIterFunc = ?*const fn (natsStatus, [*c]natsConnection, [*c]natsSubscription) callconv(.c) natsStatus;
pub fn _iterateSubsAndInvokeFunc(arg_callerSts: natsStatus, arg_nc: [*c]natsConnection, arg_f: subIterFunc) callconv(.c) natsStatus {
    var callerSts = arg_callerSts;
    _ = &callerSts;
    var nc = arg_nc;
    _ = &nc;
    var f = arg_f;
    ...
    natsHashIter_Init(&iter, nc.*.subs);
    while (natsHashIter_Next(&iter, null, &p)) {
        sub = @as([*c]natsSubscription, @ptrCast(@alignCast(p)));
        ls = f.?(callerSts, nc, sub);
        s = if (s == @as(c_uint, @bitCast(NATS_OK))) ls else s;
    }
    natsHashIter_Done(&iter);
    natsMutex_Unlock(nc.*.subsMu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_iterateSubsAndInvokeFunc");
}
pub fn _enqueUnsubProto(arg_callerSts: natsStatus, arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription) callconv(.c) natsStatus {
    var callerSts = arg_callerSts;
    _ = &callerSts;
    var nc = arg_nc;
    _ = &nc;
    ...
    natsSub_Unlock(sub);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_enqueUnsubProto");
}
pub fn _initSubDrain(arg_callerSts: natsStatus, arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription) callconv(.c) natsStatus {
    var callerSts = arg_callerSts;
    _ = &callerSts;
    var nc = arg_nc;
    _ = &nc;
...
pub fn _startSubDrain(arg_callerSts: natsStatus, arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription) callconv(.c) natsStatus {
    var callerSts = arg_callerSts;
    _ = &callerSts;
    var nc = arg_nc;
    _ = &nc;
    var sub = arg_sub;
    _ = &sub;
    if (callerSts != @as(c_uint, @bitCast(NATS_OK))) {
        natsSub_setDrainSkip(sub, callerSts);
    }
    natsSub_drain(sub);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub fn _setSubDrainStatus(arg_callerSts: natsStatus, arg_nc: [*c]natsConnection, arg_sub: [*c]natsSubscription) callconv(.c) natsStatus {
    var callerSts = arg_callerSts;
    _ = &callerSts;
    var nc = arg_nc;
    _ = &nc;
...
pub fn _flushAndDrain(arg_closure: ?*anyopaque) callconv(.c) void {
    var closure = arg_closure;
    _ = &closure;
    var nc: [*c]natsConnection = @as([*c]natsConnection, @ptrCast(@alignCast(closure)));
    _ = &nc;
    ...
    var deadline: i64 = 0;
    _ = &deadline;
    var doSubs: bool = @as(c_int, 0) != 0;
    _ = &doSubs;
    var subsDone: bool = @as(c_int, 0) != 0;
    _ = &subsDone;
    var timedOut: bool = @as(c_int, 0) != 0;
    _ = &timedOut;
    var closed: bool = @as(c_int, 0) != 0;
    _ = &closed;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var start: i64 = undefined;
    ...
    closed = natsConn_isClosed(nc);
    natsMutex_Lock(nc.*.subsMu);
    doSubs = (if (nc.*.subs.*.used > @as(c_int, 0)) @as(c_int, 1) else @as(c_int, 0)) != 0;
    natsMutex_Unlock(nc.*.subsMu);
    _ = natsMutex_Unlock(nc.*.mu);
    ...
    start = nats_Now();
    if (!closed and (@as(c_int, @intFromBool(doSubs)) != 0)) {
        if (timeout == @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
        ...
        _ = natsMutex_Lock(nc.*.mu);
        _ = _iterateSubsAndInvokeFunc(s, nc, &_startSubDrain);
        _ = natsMutex_Unlock(nc.*.mu);
        s = @as(c_uint, @bitCast(NATS_OK));
        while ((timeout == @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) or ((deadline - nats_Now()) > @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))))) {
            _ = natsMutex_Lock(nc.*.mu);
            if (!(blk: {
                const tmp = natsConn_isClosed(nc);
                closed = tmp;
                break :blk tmp;
            })) {
                natsMutex_Lock(nc.*.subsMu);
                subsDone = (if (nc.*.subs.*.used == @as(c_int, 0)) @as(c_int, 1) else @as(c_int, 0)) != 0;
                natsMutex_Unlock(nc.*.subsMu);
            }
            _ = natsMutex_Unlock(nc.*.mu);
            if ((@as(c_int, @intFromBool(closed)) != 0) or (@as(c_int, @intFromBool(subsDone)) != 0)) break;
            nats_Sleep(@as(i64, @bitCast(@as(c_long, @as(c_int, 100)))));
        }
        if (!closed and !subsDone) {
            _ = _iterateSubsAndInvokeFunc(@as(c_uint, @bitCast(NATS_TIMEOUT)), nc, &_setSubDrainStatus);
            _pushDrainErr(nc, @as(c_uint, @bitCast(NATS_TIMEOUT)), "timeout waiting for subscriptions to drain");
            timedOut = @as(c_int, 1) != 0;
        }
    }
    _ = natsMutex_Lock(nc.*.mu);
    if (!(blk: {
        ...
        nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_DRAINING_PUBS));
    }
    _ = natsMutex_Unlock(nc.*.mu);
    if (!closed and !timedOut) {
        s = @as(c_uint, @bitCast(NATS_OK));
        if (timeout == @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
            s = natsConnection_Flush(nc);
        } else {
            var elapsed: i64 = nats_Now() - start;
            _ = &elapsed;
            if (elapsed < timeout) {
...
pub fn _drain(arg_nc: [*c]natsConnection, arg_timeout: i64) callconv(.c) natsStatus {
    var nc = arg_nc;
    _ = &nc;
    var timeout = arg_timeout;
    _ = &timeout;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (nc == @as([*c]natsConnection, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_drain", @as(c_int, 3817), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    _ = natsMutex_Lock(nc.*.mu);
    if (natsConn_isClosed(nc)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_drain", @as(c_int, 3821), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
    } else if (nc.*.stanOwned) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_drain", @as(c_int, 3823), @as(c_uint, @bitCast(NATS_ILLEGAL_STATE)), "%s", "Illegal to call Drain for connection owned by a streaming connection");
    } else if ((@as(c_int, @intFromBool(_isConnecting(nc))) != 0) or (@as(c_int, @intFromBool(natsConn_isReconnecting(nc))) != 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_drain", @as(c_int, 3825), @as(c_uint, @bitCast(NATS_ILLEGAL_STATE)), "%s", "Illegal to call Drain while the connection is reconnecting");
    } else if (!natsConn_isDraining(nc)) {
        s = _iterateSubsAndInvokeFunc(@as(c_uint, @bitCast(NATS_OK)), nc, &_enqueUnsubProto);
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            nc.*.drainTimeout = timeout;
            s = natsThread_Create(&nc.*.drainThread, &_flushAndDrain, @as(?*anyopaque, @ptrCast(nc)));
            if (s == @as(c_uint, @bitCast(NATS_OK))) {
                nc.*.status = @as(c_uint, @bitCast(NATS_CONN_STATUS_DRAINING_SUBS));
                _ = _iterateSubsAndInvokeFunc(@as(c_uint, @bitCast(NATS_OK)), nc, &_initSubDrain);
                _ = blk: {
                    const ref = &nc.*.refs;
                    const tmp = ref.*;
                    ref.* += 1;
    ...
    _ = natsMutex_Unlock(nc.*.mu);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_drain");
}
pub fn _getJwtOrSeed(arg_val: [*c][*c]u8, arg_fn: [*c]const u8, arg_seed: bool, arg_item: c_int) callconv(.c) natsStatus {
    var val = arg_val;
    _ = &val;
    var @"fn" = arg_fn;
    _ = &@"fn";
    ...
    var buf: [*c]natsBuffer = null;
    _ = &buf;
    s = nats_ReadFile(&buf, @as(c_int, 1024), @"fn");
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_getJwtOrSeed");
    s = nats_GetJWTOrSeed(val, @as([*c]const u8, @ptrCast(@alignCast(buf.*.data))), item);
    if (s == @as(c_uint, @bitCast(NATS_NOT_FOUND))) {
        s = @as(c_uint, @bitCast(NATS_OK));
        if (!seed) {
            val.* = strdup(buf.*.data);
            if (val.* == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_getJwtOrSeed", @as(c_int, 4242), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            }
        } else {
            var nt: [*c]u8 = null;
            _ = &nt;
            var pch: [*c]u8 = strtok_r(buf.*.data, "\n", &nt);
            _ = &pch;
            while (pch != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                var ptr: [*c]u8 = pch;
                _ = &ptr;
                while (((@as(c_int, @bitCast(@as(c_uint, ptr.*))) == @as(c_int, ' ')) or (@as(c_int, @bitCast(@as(c_uint, ptr.*))) == @as(c_int, '\t'))) and (@as(c_int, @bitCast(@as(c_uint, ptr.*))) != @as(c_int, '\x00'))) {
                    ptr += 1;
                }
                if (((@as(c_int, @bitCast(@as(c_uint, ptr.*))) != @as(c_int, '\x00')) and (@as(c_int, @bitCast(@as(c_uint, ptr.*))) == @as(c_int, 'S'))) and (@as(c_int, @bitCast(@as(c_uint, (ptr + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1)))))).*))) == @as(c_int, 'U'))) {
                    val.* = strdup(ptr);
                    if (val.* == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                        s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_getJwtOrSeed", @as(c_int, 4261), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
                    }
                    break;
                }
                pch = strtok_r(null, "\n", &nt);
            }
            if ((s == @as(c_uint, @bitCast(NATS_OK))) and (val.* == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/conn.c", "_getJwtOrSeed", @as(c_int, 4268), @as(c_uint, @bitCast(NATS_ERR)), "no nkey user seed found in '%s'", @"fn");
            }
        }
    }
    if (buf != @as([*c]natsBuffer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _ = memset(@as(?*anyopaque, @ptrCast(buf.*.data)), @as(c_int, 0), @as(c_ulong, @bitCast(@as(c_long, buf.*.capacity))));
        natsBuf_Destroy(buf);
        buf = null;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_getJwtOrSeed");
}
pub fn _sign(arg_uc: [*c]userCreds, arg_input: [*c]const u8, arg_inputLen: c_int, arg_sig: [*c]u8) callconv(.c) natsStatus {
    var uc = arg_uc;
    _ = &uc;
    var input = arg_input;
    _ = &input;
    var inputLen = arg_inputLen;
    _ = &inputLen;
    var sig = arg_sig;
    _ = &sig;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var encodedSeed: [*c]u8 = null;
    _ = &encodedSeed;
    if (uc.*.jwtAndSeedContent != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = nats_GetJWTOrSeed(&encodedSeed, uc.*.jwtAndSeedContent, @as(c_int, 1));
    } else if (uc.*.seedFile != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        s = _getJwtOrSeed(&encodedSeed, uc.*.seedFile, @as(c_int, 1) != 0, @as(c_int, 0));
    } else {
        s = _getJwtOrSeed(&encodedSeed, uc.*.userOrChainedFile, @as(c_int, 1) != 0, @as(c_int, 1));
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = natsKeys_Sign(@as([*c]const u8, @ptrCast(@alignCast(encodedSeed))), input, inputLen, sig);
    }
    if (encodedSeed != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        natsCrypto_Clear(@as(?*anyopaque, @ptrCast(encodedSeed)), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(encodedSeed))))));
        free(@as(?*anyopaque, @ptrCast(encodedSeed)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_sign");
}
...
pub const assert_perror = @compileError("unable to translate macro: undefined identifier `__FILE__`");
// /usr/include/assert.h:125:11
...
pub const SOCK_H_ = "";
pub const NKEYS_H_ = "";
pub const NKEYS_INVALID_ENCODED_KEY = "invalid encoded key";
pub const NKEYS_INVALID_CHECKSUM = "invalid checksum";
pub const NKEYS_INVALID_SEED = "invalid seed";
pub const NKEYS_INVALID_PREFIX = "invalid prefix byte";
pub const CRYPTO_H_ = "";
pub const NATS_CRYPTO_SECRET_BYTES = @as(c_int, 64);
pub const NATS_CRYPTO_SIGN_BYTES = @as(c_int, 64);
...
pub const GLIB_H_ = "";
pub const DEFAULT_SCRATCH_SIZE = @as(c_int, 512);
pub const MAX_INFO_MESSAGE_SIZE = @import("std").zig.c_translation.promoteIntLiteral(c_int, 32768, .decimal);
pub const DEFAULT_FLUSH_TIMEOUT = @as(c_int, 10000);
pub const NATS_EVENT_ACTION_ADD = @"true";
pub const NATS_EVENT_ACTION_REMOVE = @"false";
pub const _retain = @compileError("TODO postfix inc/dec expr");
// ./src/submodules/nats.c/src/conn.c:59:9
pub const _release = @compileError("TODO postfix inc/dec expr");
// ./src/submodules/nats.c/src/conn.c:60:9
pub const CLIENT_PROTO_ZERO = @as(c_int, 0);
pub const CLIENT_PROTO_INFO = @as(c_int, 1);

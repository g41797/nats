...
pub export fn natsSrvPool_Create(arg_newPool: [*c][*c]natsSrvPool, arg_opts: [*c]struct___natsOptions) natsStatus {
    var newPool = arg_newPool;
    _ = &newPool;
    var opts = arg_opts;
    _ = &opts;
    ...
    _ = &pool;
    var poolSize: c_int = undefined;
    _ = &poolSize;
    var i: c_int = undefined;
    _ = &i;
    poolSize = if (opts.*.url != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) @as(c_int, 1) else @as(c_int, 0);
    poolSize += opts.*.serversCount;
    if (poolSize == @as(c_int, 0)) {
        poolSize = 1;
    }
    pool = @as([*c]natsSrvPool, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsSrvPool)))));
    if (pool == @as([*c]natsSrvPool, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "natsSrvPool_Create", @as(c_int, 368), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    pool.*.srvrs = @as([*c][*c]natsSrv, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, poolSize))), @sizeOf([*c]natsSrv)))));
    if (pool.*.srvrs == @as([*c][*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        free(@as(?*anyopaque, @ptrCast(pool)));
        return nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "natsSrvPool_Create", @as(c_int, 374), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    }
    pool.*.cap = poolSize;
    ...
    {
        i = 0;
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (i < opts.*.serversCount)) : (i += 1) {
            s = _addURLToPool(pool, (blk: {
                const tmp = i;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        if (pool.*.randomize) {
            _shufflePool(pool, @as(c_int, 0));
        }
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (opts.*.url != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        s = _addURLToPool(pool, opts.*.url, @as(c_int, 0) != 0, null);
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and (pool.*.size > @as(c_int, 1))) {
            var opstUrl: [*c]natsSrv = (blk: {
                const tmp = pool.*.size - @as(c_int, 1);
            ...
            (blk: {
                const tmp = pool.*.size - @as(c_int, 1);
                if (tmp >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = pool.*.srvrs[@as(c_uint, @intCast(@as(c_int, 0)))];
            pool.*.srvrs[@as(c_uint, @intCast(@as(c_int, 0)))] = opstUrl;
        }
    } else if ((s == @as(c_uint, @bitCast(NATS_OK))) and (pool.*.size == @as(c_int, 0))) {
        s = _addURLToPool(pool, @as([*c]u8, @ptrCast(@volatileCast(@constCast("nats://localhost:4222")))), @as(c_int, 0) != 0, null);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newPool.* = pool;
    } else {
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSrvPool_Create");
}
pub export fn natsSrvPool_GetCurrentServer(arg_pool: [*c]natsSrvPool, arg_cur: [*c]const natsSrv, arg_index_1: [*c]c_int) [*c]natsSrv {
    var pool = arg_pool;
    _ = &pool;
    var cur = arg_cur;
    _ = &cur;
    ...
    _ = &i;
    {
        ...
        while (i < pool.*.size) : (i += 1) {
            ...
            if (s == @as([*c]natsSrv, @ptrCast(@volatileCast(@constCast(cur))))) {
                if (index_1 != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    index_1.* = i;
                }
                return s;
            }
...
pub export fn natsSrvPool_GetNextServer(arg_pool: [*c]natsSrvPool, arg_opts: [*c]struct___natsOptions, arg_cur: [*c]const natsSrv) [*c]natsSrv {
    var pool = arg_pool;
    _ = &pool;
    var opts = arg_opts;
    _ = &opts;
    ...
    if (i < @as(c_int, 0)) return null;
    {
        j = i;
        while (j < (pool.*.size - @as(c_int, 1))) : (j += 1) {
            (blk: {
                const tmp = j;
                if (tmp >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = (blk: {
    ...
    if ((opts.*.maxReconnect < @as(c_int, 0)) or (s.*.reconnects < opts.*.maxReconnect)) {
        (blk: {
            const tmp = pool.*.size - @as(c_int, 1);
            if (tmp >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).* = s;
    ...
    if (pool.*.size <= @as(c_int, 0)) return null;
    return pool.*.srvrs[@as(c_uint, @intCast(@as(c_int, 0)))];
}
pub export fn natsSrvPool_addNewURLs(arg_pool: [*c]natsSrvPool, arg_curUrl: [*c]const natsUrl, arg_urls: [*c][*c]u8, arg_urlCount: c_int, arg_tlsName: [*c]const u8, arg_added: [*c]bool) natsStatus {
    var pool = arg_pool;
    _ = &pool;
    var curUrl = arg_curUrl;
    _ = &curUrl;
    var urls = arg_urls;
    _ = &urls;
    var urlCount = arg_urlCount;
    _ = &urlCount;
    var tlsName = arg_tlsName;
    _ = &tlsName;
    var added = arg_added;
    _ = &added;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    ...
    var j: c_int = undefined;
    _ = &j;
    var sport: [*c]u8 = undefined;
    _ = &sport;
    var portPos: c_int = undefined;
    _ = &portPos;
    var found: bool = undefined;
    _ = &found;
    var isLH: bool = undefined;
    _ = &isLH;
    var tmp: [*c]natsStrHash = null;
    _ = &tmp;
    var srv: [*c]natsSrv = null;
    _ = &srv;
    added.* = @as(c_int, 0) != 0;
    s = natsStrHash_Create(&tmp, urlCount);
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSrvPool_addNewURLs");
    {
        i = 0;
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (i < urlCount)) : (i += 1) {
            s = natsStrHash_SetEx(tmp, (blk: {
                const tmp_1 = i;
                if (tmp_1 >= 0) break :blk urls + @as(usize, @intCast(tmp_1)) else break :blk urls - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
            }).*, @as(c_int, 0) != 0, (if (@as(c_int, 0) != 0) @as(c_int, 1) else @as(c_int, 0)) != 0, @as(?*anyopaque, @ptrFromInt(@as(c_int, 1))), null);
        }
    }
    {
        i = 0;
        while (i < pool.*.size) : (i += 1) {
            var inInfo: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
            _ = &inInfo;
            srv = (blk: {
                const tmp_1 = i;
                if (tmp_1 >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp_1)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
            }).*;
            _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "%s:%d", srv.*.url.*.host, srv.*.url.*.port);
            inInfo = natsStrHash_GetEx(tmp, @as([*c]u8, @ptrCast(@alignCast(&url))), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&url)))))))));
            _ = natsStrHash_Remove(tmp, @as([*c]u8, @ptrCast(@alignCast(&url))));
            if (!srv.*.isImplicit or (srv.*.url == @as([*c]natsUrl, @ptrCast(@volatileCast(@constCast(curUrl)))))) {
                continue;
            }
            if (!(inInfo != null)) {
                {
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        ...
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (@as(c_int, @intFromBool(natsStrHashIter_Next(&iter, &curl, null))) != 0)) {
            isLH = @as(c_int, 0) != 0;
            found = @as(c_int, 0) != 0;
            sport = strrchr(curl, @as(c_int, ':'));
            portPos = @as(c_int, @bitCast(@as(c_int, @truncate(@divExact(@as(c_long, @bitCast(@intFromPtr(sport) -% @intFromPtr(curl))), @sizeOf(u8))))));
            if ((((strcasestr(curl, "localhost") == curl) and (portPos == @as(c_int, 9))) or (strncmp(curl, "127.0.0.1", @as(c_ulong, @bitCast(@as(c_long, portPos)))) == @as(c_int, 0))) or (strncmp(curl, "[::1]", @as(c_ulong, @bitCast(@as(c_long, portPos)))) == @as(c_int, 0))) {
                isLH = (@as(c_int, @bitCast(@as(c_uint, curl[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, 'l')) or (@as(c_int, @bitCast(@as(c_uint, curl[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, 'L'));
                _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "localhost%s", sport);
                found = natsStrHash_GetEx(pool.*.urls, @as([*c]u8, @ptrCast(@alignCast(&url))), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&url))))))))) != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
                if (!found) {
                    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "127.0.0.1%s", sport);
                    found = natsStrHash_GetEx(pool.*.urls, @as([*c]u8, @ptrCast(@alignCast(&url))), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&url))))))))) != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
                }
                if (!found) {
                    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "[::1]%s", sport);
                    found = natsStrHash_GetEx(pool.*.urls, @as([*c]u8, @ptrCast(@alignCast(&url))), @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(@as([*c]u8, @ptrCast(@alignCast(&url))))))))) != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
                }
            } else {
                found = natsStrHash_GetEx(pool.*.urls, curl, @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(curl)))))) != @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
            }
            _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "nats://%s", curl);
            if (!found) {
                if (isLH) {
                    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&url))), @sizeOf([256]u8), "nats://localhost%s", sport);
                }
                added.* = @as(c_int, 1) != 0;
            }
            s = _addURLToPool(pool, @as([*c]u8, @ptrCast(@alignCast(&url))), @as(c_int, 1) != 0, tlsName);
        }
        natsStrHashIter_Done(&iter);
        if (((s == @as(c_uint, @bitCast(NATS_OK))) and (added != null)) and (@as(c_int, @intFromBool(pool.*.randomize)) != 0)) {
            _shufflePool(pool, @as(c_int, 1));
        }
    }
    natsStrHash_Destroy(tmp);
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSrvPool_addNewURLs");
}
pub export fn natsSrvPool_GetServers(arg_pool: [*c]natsSrvPool, arg_implicitOnly: bool, arg_servers: [*c][*c][*c]u8, arg_count: [*c]c_int) natsStatus {
    var pool = arg_pool;
    _ = &pool;
    var implicitOnly = arg_implicitOnly;
    _ = &implicitOnly;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var srvrs: [*c][*c]u8 = null;
    _ = &srvrs;
    var srv: [*c]natsSrv = undefined;
    _ = &srv;
    var url: [*c]natsUrl = undefined;
    _ = &url;
    var i: c_int = undefined;
    _ = &i;
    var discovered: c_int = 0;
    _ = &discovered;
    if (pool.*.size == @as(c_int, 0)) {
        servers.* = null;
        count.* = 0;
        return @as(c_uint, @bitCast(NATS_OK));
    }
    srvrs = @as([*c][*c]u8, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, pool.*.size))), @sizeOf([*c]u8)))));
    if (srvrs == @as([*c][*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "natsSrvPool_GetServers", @as(c_int, 443), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    {
        i = 0;
        while ((s == @as(c_uint, @bitCast(NATS_OK))) and (i < pool.*.size)) : (i += 1) {
            srv = (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            if ((@as(c_int, @intFromBool(implicitOnly)) != 0) and !srv.*.isImplicit) continue;
            url = srv.*.url;
            if (asprintf(&(blk: {
                const tmp = discovered;
                if (tmp >= 0) break :blk srvrs + @as(usize, @intCast(tmp)) else break :blk srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*, "nats://%s:%d", url.*.host, url.*.port) == -@as(c_int, 1)) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "natsSrvPool_GetServers", @as(c_int, 452), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            } else {
                discovered += 1;
            }
        }
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        ...
        free(@as(?*anyopaque, @ptrCast(srvrs)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSrvPool_GetServers");
}
pub export fn natsSrvPool_Destroy(arg_pool: [*c]natsSrvPool) void {
    var pool = arg_pool;
    _ = &pool;
    var srv: [*c]natsSrv = undefined;
    _ = &srv;
    var i: c_int = undefined;
    _ = &i;
    if (pool == @as([*c]natsSrvPool, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    {
        i = 0;
        while (i < pool.*.size) : (i += 1) {
            srv = (blk: {
    ...
    pool.*.urls = null;
    free(@as(?*anyopaque, @ptrCast(pool.*.srvrs)));
    pool.*.srvrs = null;
    pool.*.size = 0;
    free(@as(?*anyopaque, @ptrCast(pool.*.user)));
    free(@as(?*anyopaque, @ptrCast(pool.*.pwd)));
    free(@as(?*anyopaque, @ptrCast(pool)));
}
...
pub fn _freeSrv(arg_srv: [*c]natsSrv) callconv(.c) void {
    var srv = arg_srv;
    _ = &srv;
    if (srv == @as([*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    natsUrl_Destroy(srv.*.url);
    free(@as(?*anyopaque, @ptrCast(srv.*.tlsName)));
    free(@as(?*anyopaque, @ptrCast(srv)));
}
pub fn _createSrv(arg_newSrv: [*c][*c]natsSrv, arg_url: [*c]u8, arg_implicit: bool, arg_tlsName: [*c]const u8) callconv(.c) natsStatus {
    var newSrv = arg_newSrv;
    _ = &newSrv;
    var url = arg_url;
    _ = &url;
    var implicit = arg_implicit;
    _ = &implicit;
    var tlsName = arg_tlsName;
    _ = &tlsName;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var srv: [*c]natsSrv = @as([*c]natsSrv, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsSrv)))));
    _ = &srv;
    if (srv == @as([*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "_createSrv", @as(c_int, 37), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    srv.*.isImplicit = implicit;
    s = natsUrl_Create(&srv.*.url, url);
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (tlsName != @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        srv.*.tlsName = strdup(tlsName);
        if (srv.*.tlsName == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            s = nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "_createSrv", @as(c_int, 46), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
        }
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newSrv.* = srv;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_createSrv");
}
pub fn _addURLToPool(arg_pool: [*c]natsSrvPool, arg_sURL: [*c]u8, arg_implicit: bool, arg_tlsName: [*c]const u8) callconv(.c) natsStatus {
    var pool = arg_pool;
    _ = &pool;
    var sURL = arg_sURL;
    _ = &sURL;
    ...
    var srv: [*c]natsSrv = null;
    _ = &srv;
    var addedToMap: bool = @as(c_int, 0) != 0;
    _ = &addedToMap;
    var bareURL: [256]u8 = undefined;
    _ = &bareURL;
    s = _createSrv(&srv, sURL, implicit, tlsName);
    if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_addURLToPool");
    if ((!implicit and (pool.*.user == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) and (srv.*.url.*.username != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) {
        {
            pool.*.user = strdup(srv.*.url.*.username);
            if (pool.*.user == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "_addURLToPool", @as(c_int, 156), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            }
        }
        if ((s == @as(c_uint, @bitCast(NATS_OK))) and !((if ((srv.*.url.*.password == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (@as(c_int, @bitCast(@as(c_uint, srv.*.url.*.password[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '\x00'))) @as(c_int, 1) else @as(c_int, 0)) != 0)) {
            pool.*.pwd = strdup(srv.*.url.*.password);
            if (pool.*.pwd == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "_addURLToPool", @as(c_int, 158), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            }
        }
        if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_addURLToPool");
    }
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&bareURL))), @sizeOf([256]u8), "%s:%d", srv.*.url.*.host, srv.*.url.*.port);
    s = natsStrHash_SetEx(pool.*.urls, @as([*c]u8, @ptrCast(@alignCast(&bareURL))), @as(c_int, 1) != 0, (if (@as(c_int, 1) != 0) @as(c_int, 1) else @as(c_int, 0)) != 0, @as(?*anyopaque, @ptrFromInt(@as(c_int, 1))), null);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        addedToMap = @as(c_int, 1) != 0;
        if ((pool.*.size + @as(c_int, 1)) > pool.*.cap) {
            var newArray: [*c][*c]natsSrv = null;
            _ = &newArray;
            var newCap: c_int = @as(c_int, 2) * pool.*.cap;
            _ = &newCap;
            newArray = @as([*c][*c]natsSrv, @ptrCast(@alignCast(realloc(@as(?*anyopaque, @ptrCast(pool.*.srvrs)), @as(c_ulong, @bitCast(@as(c_long, newCap))) *% @sizeOf([*c]u8)))));
            if (newArray == @as([*c][*c]natsSrv, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/srvpool.c", "_addURLToPool", @as(c_int, 176), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
            }
            if (s == @as(c_uint, @bitCast(NATS_OK))) {
                pool.*.cap = newCap;
                pool.*.srvrs = newArray;
    ...
    if (s != @as(c_uint, @bitCast(NATS_OK))) {
        if (addedToMap) {
            _ = natsStrHash_Remove(pool.*.urls, sURL);
        }
        _freeSrv(srv);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_addURLToPool");
}
pub fn _shufflePool(arg_pool: [*c]natsSrvPool, arg_offset: c_int) callconv(.c) void {
    var pool = arg_pool;
    _ = &pool;
    ...
    var j: c_int = undefined;
    _ = &j;
    var tmp: [*c]natsSrv = undefined;
    _ = &tmp;
    if (pool.*.size <= (offset + @as(c_int, 1))) return;
    srand(@as(c_uint, @bitCast(@as(c_int, @truncate(nats_NowInNanoSeconds())))));
    {
        i = offset;
        while (i < pool.*.size) : (i += 1) {
            j = offset + @import("std").zig.c_translation.signedRemainder(rand(), (i + @as(c_int, 1)) - offset);
            tmp = (blk: {
                const tmp_1 = i;
                if (tmp_1 >= 0) break :blk pool.*.srvrs + @as(usize, @intCast(tmp_1)) else break :blk pool.*.srvrs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
            }).*;

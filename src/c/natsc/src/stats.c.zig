...
pub export fn natsStatistics_Create(arg_newStats: [*c][*c]natsStatistics) natsStatus {
    var newStats = arg_newStats;
    _ = &newStats;
    var stats: [*c]natsStatistics = null;
    _ = &stats;
    stats = @as([*c]natsStatistics, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsStatistics)))));
    if (stats == @as([*c]natsStatistics, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/stats.c", "natsStatistics_Create", @as(c_int, 28), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    newStats.* = stats;
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsStatistics_GetCounts(arg_stats: [*c]const natsStatistics, arg_inMsgs: [*c]u64, arg_inBytes: [*c]u64, arg_outMsgs: [*c]u64, arg_outBytes: [*c]u64, arg_reconnects: [*c]u64) natsStatus {
    var stats = arg_stats;
    _ = &stats;
    var inMsgs = arg_inMsgs;
    _ = &inMsgs;
    var inBytes = arg_inBytes;
    _ = &inBytes;
    var outMsgs = arg_outMsgs;
    _ = &outMsgs;
    var outBytes = arg_outBytes;
    _ = &outBytes;
    var reconnects = arg_reconnects;
    _ = &reconnects;
    if (stats == @as([*c]const natsStatistics, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/stats.c", "natsStatistics_GetCounts", @as(c_int, 42), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_INVALID_ARG))));
    if (inMsgs != @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        inMsgs.* = stats.*.inMsgs;
    }
    if (inBytes != @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        inBytes.* = stats.*.inBytes;
    }
    if (outMsgs != @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        outMsgs.* = stats.*.outMsgs;
    }
    if (outBytes != @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        outBytes.* = stats.*.outBytes;
    }
    if (reconnects != @as([*c]u64, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        reconnects.* = stats.*.reconnects;
    }
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsStatistics_Destroy(arg_stats: [*c]natsStatistics) void {
    var stats = arg_stats;
    _ = &stats;
    if (stats == @as([*c]natsStatistics, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    free(@as(?*anyopaque, @ptrCast(stats)));
}

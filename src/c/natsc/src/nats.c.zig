...
pub export fn nats_OpenWithConfig(arg_config: [*c]natsClientConfig) natsStatus {
    var config = arg_config;
    _ = &config;
    return nats_openLib(config);
}
pub export fn nats_Open(arg_lockSpinCount: i64) natsStatus {
    var lockSpinCount = arg_lockSpinCount;
    _ = &lockSpinCount;
    var defaultToSharedDispatchers: bool = (if (getenv("NATS_DEFAULT_TO_LIB_MSG_DELIVERY") != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) @as(c_int, 1) else @as(c_int, 0)) != 0;
    _ = &defaultToSharedDispatchers;
    var config: natsClientConfig = natsClientConfig{
        .DefaultWriteDeadline = @import("std").mem.zeroes(i64),
        .LockSpinCount = lockSpinCount,
        .DefaultToThreadPool = defaultToSharedDispatchers,
        .ThreadPoolMax = @as(c_int, 1),
        .DefaultRepliesToThreadPool = @as(c_int, 0) != 0,
        .UseSeparatePoolForReplies = false,
        .ReplyThreadPoolMax = @as(c_int, 0),
    };
    _ = &config;
    return nats_openLib(&config);
}
pub export fn nats_GetVersion() [*c]const u8 {
    return "3.10.0-beta";
}
pub export fn nats_GetVersionNumber() u32 {
    return @as(u32, @bitCast(((@as(c_int, 3) << @intCast(16)) | (@as(c_int, 10) << @intCast(8))) | @as(c_int, 0)));
}
pub export fn nats_CheckCompatibilityImpl(arg_headerReqVerNumber: u32, arg_headerVerNumber: u32, arg_headerVerString: [*c]const u8) bool {
    var headerReqVerNumber = arg_headerReqVerNumber;
    _ = &headerReqVerNumber;
    var headerVerNumber = arg_headerVerNumber;
    _ = &headerVerNumber;
    var headerVerString = arg_headerVerString;
    _ = &headerVerString;
    if ((headerVerNumber < @as(u32, @bitCast(@as(c_int, 198912)))) or (headerReqVerNumber > @as(u32, @bitCast(((@as(c_int, 3) << @intCast(16)) | (@as(c_int, 10) << @intCast(8))) | @as(c_int, 0))))) {
        var reqVerString: [10]u8 = undefined;
        _ = &reqVerString;
        var libReqVerString: [10]u8 = undefined;
        _ = &libReqVerString;
        _versionGetString(@as([*c]u8, @ptrCast(@alignCast(&reqVerString))), @sizeOf([10]u8), headerReqVerNumber);
        _versionGetString(@as([*c]u8, @ptrCast(@alignCast(&libReqVerString))), @sizeOf([10]u8), @as(u32, @bitCast(@as(c_int, 198912))));
        _ = printf("Incompatible versions:\nHeader : %s (requires %s)\nLibrary: %s (requires %s)\n", headerVerString, @as([*c]u8, @ptrCast(@alignCast(&reqVerString))), "3.10.0-beta", @as([*c]u8, @ptrCast(@alignCast(&libReqVerString))));
        exit(@as(c_int, 1));
    }
    return @as(c_int, 1) != 0;
}
...
pub export fn nats_SetMessageDeliveryPoolSize(arg_max: c_int) natsStatus {
    var max = arg_max;
    _ = &max;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "nats_SetMessageDeliveryPoolSize");
}
...
pub export fn nats_Close() void {
    _ = nats_closeLib(@as(c_int, 0) != 0, @as(i64, @bitCast(@as(c_long, @as(c_int, 1)))));
}
pub export fn nats_CloseAndWait(arg_timeout: i64) natsStatus {
    var timeout = arg_timeout;
    _ = &timeout;
    return nats_closeLib(@as(c_int, 1) != 0, timeout);
}
...
pub export fn natsInbox_Create(arg_newInbox: [*c][*c]natsInbox) natsStatus {
    ...
    var inbox: [*c]u8 = null;
    _ = &inbox;
    const size: c_int = (@as(c_int, 7) + (@as(c_int, 12) + @as(c_int, 10))) + @as(c_int, 1);
    _ = &size;
    s = nats_Open(@as(i64, @bitCast(@as(c_long, -@as(c_int, 1)))));
    if (s != @as(c_uint, @bitCast(NATS_OK))) return s;
    inbox = @as([*c]u8, @ptrCast(@alignCast(malloc(@as(c_ulong, @bitCast(@as(c_long, size)))))));
    if (inbox == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/nats.c", "natsInbox_Create", @as(c_int, 123), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    _ = memcpy(@as(?*anyopaque, @ptrCast(inbox)), @as(?*const anyopaque, @ptrCast("_INBOX.")), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 7)))));
    s = natsNUID_Next(inbox + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 7))))), (@as(c_int, 12) + @as(c_int, 10)) + @as(c_int, 1));
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        (blk: {
            const tmp = size - @as(c_int, 1);
            if (tmp >= 0) break :blk inbox + @as(usize, @intCast(tmp)) else break :blk inbox - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).* = '\x00';
        newInbox.* = @as([*c]natsInbox, @ptrCast(@alignCast(inbox)));
    } else {
        free(@as(?*anyopaque, @ptrCast(inbox)));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsInbox_Create");
}
pub export fn natsInbox_Destroy(arg_inbox: [*c]natsInbox) void {
    var inbox = arg_inbox;
    _ = &inbox;
    if (inbox == @as([*c]natsInbox, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    free(@as(?*anyopaque, @ptrCast(inbox)));
}
...
pub fn _overwriteInt64(arg_envVar: [*c]const u8, arg_val: [*c]i64) callconv(.c) void {
    var envVar = arg_envVar;
    _ = &envVar;
    var val = arg_val;
    _ = &val;
    var str: [*c]u8 = getenv(envVar);
    _ = &str;
    if (str != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        val.* = @as(i64, @bitCast(@as(c_long, @truncate(atoll(str)))));
    }
}
pub fn _overwriteInt(arg_envVar: [*c]const u8, arg_val: [*c]c_int) callconv(.c) void {
    var envVar = arg_envVar;
    _ = &envVar;
    var val = arg_val;
    _ = &val;
...
pub fn _overwriteBool(arg_envVar: [*c]const u8, arg_val: [*c]bool) callconv(.c) void {
    var envVar = arg_envVar;
    _ = &envVar;
    var val = arg_val;
    _ = &val;
    ...
    _ = &str;
    if (str != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        val.* = ((strcasecmp(str, "true") == @as(c_int, 0)) or (strcasecmp(str, "on") == @as(c_int, 0))) or (atoi(str) != @as(c_int, 0));
    }
}
pub fn _overrideWithEnv(arg_config: [*c]natsClientConfig) callconv(.c) void {
    var config = arg_config;
    _ = &config;
...
pub fn _versionGetString(arg_buffer: [*c]u8, arg_bufLen: usize, arg_verNumber: u32) callconv(.c) void {
    var buffer = arg_buffer;
    _ = &buffer;
    var bufLen = arg_bufLen;
    _ = &bufLen;
    var verNumber = arg_verNumber;
    _ = &verNumber;
    _ = snprintf(buffer, bufLen, "%u.%u.%u", (verNumber >> @intCast(16)) & @as(u32, @bitCast(@as(c_int, 15))), (verNumber >> @intCast(8)) & @as(u32, @bitCast(@as(c_int, 15))), verNumber & @as(u32, @bitCast(@as(c_int, 15))));
}

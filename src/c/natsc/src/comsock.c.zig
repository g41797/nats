...
pub export fn natsSock_Init(arg_ctx: [*c]natsSockCtx) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    _ = memset(@as(?*anyopaque, @ptrCast(ctx)), @as(c_int, 0), @sizeOf(natsSockCtx));
    ctx.*.fd = -@as(c_int, 1);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub extern fn natsSock_WaitReady(waitMode: c_int, ctx: [*c]natsSockCtx) natsStatus;
pub export fn natsSock_ShuffleIPs(arg_ctx: [*c]natsSockCtx, arg_tmp: [*c][*c]struct_addrinfo, arg_tmpSize: c_int, arg_ipListHead: [*c][*c]struct_addrinfo, arg_count: c_int) void {
    var ctx = arg_ctx;
    _ = &ctx;
    var tmp = arg_tmp;
    _ = &tmp;
    var tmpSize = arg_tmpSize;
    _ = &tmpSize;
    var ipListHead = arg_ipListHead;
    _ = &ipListHead;
    var count = arg_count;
    _ = &count;
    var ips: [*c][*c]struct_addrinfo = null;
    _ = &ips;
    var p: [*c]struct_addrinfo = null;
    _ = &p;
    var doFree: bool = @as(c_int, 0) != 0;
    _ = &doFree;
    var i: c_int = undefined;
    _ = &i;
    var j: c_int = undefined;
    _ = &j;
    if ((((@as(c_int, @intFromBool(ctx.*.noRandomize)) != 0) or (ipListHead == @as([*c][*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) or (ipListHead.* == @as([*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))))))) or (count <= @as(c_int, 1))) return;
    if (count > tmpSize) {
        ips = @as([*c][*c]struct_addrinfo, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, count))), @sizeOf([*c]struct_addrinfo)))));
        if (ips == @as([*c][*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
        doFree = @as(c_int, 1) != 0;
    } else {
        ips = tmp;
    }
    p = ipListHead.*;
    {
        ...
        while (i < count) : (i += 1) {
            (blk: {
                const tmp_1 = i;
                if (tmp_1 >= 0) break :blk ips + @as(usize, @intCast(tmp_1)) else break :blk ips - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
            }).* = p;
            p = p.*.ai_next;
        }
    }
    {
        i = 0;
        while (i < count) : (i += 1) {
            j = @import("std").zig.c_translation.signedRemainder(rand(), i + @as(c_int, 1));
            p = (blk: {
                const tmp_1 = i;
                if (tmp_1 >= 0) break :blk ips + @as(usize, @intCast(tmp_1)) else break :blk ips - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
            }).*;
    ...
    }
    {
        i = 0;
        while (i < count) : (i += 1) {
            if (i < (count - @as(c_int, 1))) {
                (blk: {
                    const tmp_1 = i;
                    if (tmp_1 >= 0) break :blk ips + @as(usize, @intCast(tmp_1)) else break :blk ips - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
                }).*.*.ai_next = (blk: {
                    const tmp_1 = i + @as(c_int, 1);
                    if (tmp_1 >= 0) break :blk ips + @as(usize, @intCast(tmp_1)) else break :blk ips - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_1)) +% -1));
                }).*;
            } else {
                (blk: {
    ...
    ipListHead.* = ips[@as(c_uint, @intCast(@as(c_int, 0)))];
    if (doFree) {
        free(@as(?*anyopaque, @ptrCast(ips)));
    }
}
pub export fn natsSock_ConnectTcp(arg_ctx: [*c]natsSockCtx, arg_phost: [*c]const u8, arg_port: c_int) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var phost = arg_phost;
    _ = &phost;
    ...
    _ = &res;
    var sport: [6]u8 = undefined;
    _ = &sport;
    var i: c_int = undefined;
    _ = &i;
    var max: c_int = 1;
    _ = &max;
    var hosta: [256]u8 = undefined;
    _ = &hosta;
    var hostLen: c_int = undefined;
    _ = &hostLen;
    var host: [*c]u8 = undefined;
    _ = &host;
    var servInfos: [2][*c]struct_addrinfo = [2][*c]struct_addrinfo{
        null,
        null,
    };
    _ = &servInfos;
    var numServInfo: c_int = 0;
    _ = &numServInfo;
    var numIPs: c_int = 0;
    _ = &numIPs;
    var start: i64 = 0;
    _ = &start;
    var totalTimeout: i64 = 0;
    _ = &totalTimeout;
    var timeoutPerIP: i64 = 0;
    _ = &timeoutPerIP;
    var tmpStorage: [64][*c]struct_addrinfo = undefined;
    _ = &tmpStorage;
    if (phost == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ConnectTcp", @as(c_int, 157), @as(c_uint, @bitCast(NATS_ADDRESS_MISSING)), "%s", "No host specified");
    hostLen = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(phost)))));
    if ((hostLen == @as(c_int, 0)) or ((hostLen == @as(c_int, 1)) and (@as(c_int, @bitCast(@as(c_uint, phost[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '[')))) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ConnectTcp", @as(c_int, 161), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "Invalid host name: %s", phost);
    if (@as(c_int, @bitCast(@as(c_uint, phost[@as(c_uint, @intCast(@as(c_int, 0)))]))) == @as(c_int, '[')) {
        _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&hosta))), @sizeOf([256]u8), "%.*s", hostLen - @as(c_int, 2), phost + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1))))));
        host = @as([*c]u8, @ptrCast(@alignCast(&hosta)));
    } else {
        host = @as([*c]u8, @ptrCast(@volatileCast(@constCast(phost))));
    }
    _ = snprintf(@as([*c]u8, @ptrCast(@alignCast(&sport))), @sizeOf([6]u8), "%d", port);
    if ((ctx.*.orderIP == @as(c_int, 46)) or (ctx.*.orderIP == @as(c_int, 64))) {
        max = 2;
    }
    start = nats_Now();
    {
        i = 0;
        while (i < max) : (i += 1) {
            var hints: struct_addrinfo = undefined;
            _ = &hints;
            var servinfo: [*c]struct_addrinfo = null;
            _ = &servinfo;
            var count: c_int = 0;
            _ = &count;
            var p: [*c]struct_addrinfo = undefined;
            _ = &p;
            _ = memset(@as(?*anyopaque, @ptrCast(&hints)), @as(c_int, 0), @sizeOf(struct_addrinfo));
            ...
            while (true) {
                switch (ctx.*.orderIP) {
                    ...
                    @as(c_int, 46) => {
                        hints.ai_family = if (i == @as(c_int, 0)) @as(c_int, 2) else @as(c_int, 10);
                        break;
                    },
                    @as(c_int, 64) => {
                        hints.ai_family = if (i == @as(c_int, 0)) @as(c_int, 10) else @as(c_int, 2);
                        break;
                    },
                    else => {
                        hints.ai_family = 0;
            ...
            if ((blk: {
                const tmp = getaddrinfo(host, @as([*c]u8, @ptrCast(@alignCast(&sport))), &hints, &servinfo);
                res = tmp;
                break :blk tmp;
            }) != @as(c_int, 0)) {
                s = nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ConnectTcp", @as(c_int, 200), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "getaddrinfo error: %s", gai_strerror(res));
                continue;
            }
            servInfos[@as(c_uint, @intCast(numServInfo))] = servinfo;
            {
                p = servinfo;
                while (p != @as([*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) : (p = p.*.ai_next) {
                    count += 1;
                    numIPs += 1;
                }
            }
            natsSock_ShuffleIPs(ctx, @as([*c][*c]struct_addrinfo, @ptrCast(@alignCast(&tmpStorage))), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([64][*c]struct_addrinfo))))), &servInfos[@as(c_uint, @intCast(numServInfo))], count);
            numServInfo += 1;
        }
    }
    if ((s != @as(c_uint, @bitCast(NATS_OK))) and (numServInfo == @as(c_int, 0))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_ConnectTcp");
    if (numIPs == @as(c_int, 0)) {
        {
            i = 0;
            while (i < numServInfo) : (i += 1) {
                nats_FreeAddrInfo(servInfos[@as(c_uint, @intCast(i))]);
            }
        }
        return if (NATS_NO_SERVER == NATS_OK) @as(c_uint, @bitCast(NATS_NO_SERVER)) else nats_updateErrStack(@as(c_uint, @bitCast(NATS_NO_SERVER)), "natsSock_ConnectTcp");
    }
    totalTimeout = @as(i64, @bitCast(@as(c_long, natsDeadline_GetTimeout(&ctx.*.writeDeadline))));
    if (totalTimeout > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
        timeoutPerIP = @divTrunc(totalTimeout, @as(i64, @bitCast(@as(c_long, numIPs))));
        if (timeoutPerIP < @as(i64, @bitCast(@as(c_long, @as(c_int, 10))))) {
            timeoutPerIP = 10;
        }
    }
    {
        ...
        while (i < numServInfo) : (i += 1) {
            ...
            _ = &p;
            {
                p = servInfos[@as(c_uint, @intCast(i))];
                while (p != @as([*c]struct_addrinfo, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) : (p = p.*.ai_next) {
                    ctx.*.fd = socket(p.*.ai_family, p.*.ai_socktype, p.*.ai_protocol);
                    if (ctx.*.fd == -@as(c_int, 1)) {
                        s = nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ConnectTcp", @as(c_int, 247), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "socket error: %d", __errno_location().*);
                        continue;
                    }
                    s = @as(c_uint, @bitCast(NATS_OK));
                    if (s == @as(c_uint, @bitCast(NATS_OK))) {
                        s = natsSock_SetBlocking(ctx.*.fd, @as(c_int, 0) != 0);
                    }
                    if (s == @as(c_uint, @bitCast(NATS_OK))) {
                        res = connect(ctx.*.fd, __CONST_SOCKADDR_ARG{
                            .__sockaddr__ = p.*.ai_addr,
                        }, @as(natsSockLen, @bitCast(p.*.ai_addrlen)));
                        if ((res == -@as(c_int, 1)) and (__errno_location().* == @as(c_int, 115))) {
                            if (timeoutPerIP > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
                                natsDeadline_Init(&ctx.*.writeDeadline, timeoutPerIP);
                            }
                            s = natsSock_WaitReady(@as(c_int, 2), ctx);
                            if ((s == @as(c_uint, @bitCast(NATS_OK))) and !natsSock_IsConnected(ctx.*.fd)) {
                                s = @as(c_uint, @bitCast(NATS_TIMEOUT));
                            }
                        } else if (res == -@as(c_int, 1)) {
                            s = nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ConnectTcp", @as(c_int, 281), @as(c_uint, @bitCast(NATS_NO_SERVER)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_SERVER))));
                        }
                    }
                    if (s == @as(c_uint, @bitCast(NATS_OK))) {
    ...
    if (totalTimeout > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
        var used: i64 = nats_Now() - start;
        _ = &used;
        var left: i64 = totalTimeout - used;
        _ = &left;
        natsDeadline_Init(&ctx.*.writeDeadline, if (left > @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) left else @as(i64, @bitCast(@as(c_long, @as(c_int, 0)))));
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_ConnectTcp");
}
...
pub export fn natsSock_ReadLine(arg_ctx: [*c]natsSockCtx, arg_buffer: [*c]u8, arg_maxBufferSize: usize) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var buffer = arg_buffer;
    _ = &buffer;
    var maxBufferSize = arg_maxBufferSize;
    _ = &maxBufferSize;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var readBytes: c_int = 0;
    _ = &readBytes;
    ...
    if (@as(c_int, @bitCast(@as(c_uint, p.*))) != @as(c_int, '\x00')) {
        var nextLine: [*c]u8 = undefined;
        _ = &nextLine;
        var nextStart: usize = undefined;
        _ = &nextStart;
        var len: usize = 0;
        _ = &len;
        nextStart = strlen(buffer) +% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 2))));
        nextLine = buffer + nextStart;
        if (@as(c_int, @bitCast(@as(c_uint, nextLine.*))) != @as(c_int, '\x00')) {
            len = strlen(nextLine);
            _ = memmove(@as(?*anyopaque, @ptrCast(buffer)), @as(?*const anyopaque, @ptrCast(nextLine)), len +% @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))));
            if ((blk: {
                const tmp = strstr(buffer, "\r\n");
                eol = tmp;
                break :blk tmp;
    ...
    while (true) {
        ...
        if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_ReadLine");
        if ((totalBytes +% @as(usize, @bitCast(@as(c_long, readBytes)))) == maxBufferSize) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_ReadLine", @as(c_int, 384), @as(c_uint, @bitCast(NATS_LINE_TOO_LONG)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_LINE_TOO_LONG))));
        (p + @as(usize, @bitCast(@as(isize, @intCast(readBytes))))).* = '\x00';
        if ((blk: {
            const tmp = strstr(p, "\r\n");
            eol = tmp;
            break :blk tmp;
        ...
        p += @as(usize, @bitCast(@as(isize, @intCast(readBytes))));
        totalBytes +%= @as(usize, @bitCast(@as(c_long, readBytes)));
    }
    return @import("std").mem.zeroes(natsStatus);
}
pub export fn natsSock_Read(arg_ctx: [*c]natsSockCtx, arg_buffer: [*c]u8, arg_maxBufferSize: usize, arg_n: [*c]c_int) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var buffer = arg_buffer;
    _ = &buffer;
    ...
    var readBytes: c_int = 0;
    _ = &readBytes;
    var needRead: bool = @as(c_int, 1) != 0;
    _ = &needRead;
    while (needRead) {
        readBytes = @as(c_int, @bitCast(@as(c_int, @truncate(recv(ctx.*.fd, @as(?*anyopaque, @ptrCast(buffer)), @as(natsRecvLen, @bitCast(maxBufferSize)), @as(c_int, 0))))));
        if (readBytes == @as(c_int, 0)) {
            return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_Read", @as(c_int, 418), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
        } else if (readBytes < @as(c_int, 0)) {
            if (__errno_location().* != @as(c_int, 11)) {
                return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_Read", @as(c_int, 454), @as(c_uint, @bitCast(NATS_IO_ERROR)), "recv error: %d", __errno_location().*);
            } else if (ctx.*.useEventLoop) {
                if (n != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    n.* = 0;
                }
                return @as(c_uint, @bitCast(NATS_OK));
            }
            s = natsSock_WaitReady(@as(c_int, 0), ctx);
            if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_Read");
            continue;
        }
        if (n != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            n.* = readBytes;
        }
        needRead = @as(c_int, 0) != 0;
    }
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsSock_Write(arg_ctx: [*c]natsSockCtx, arg_data: [*c]const u8, arg_len: c_int, arg_n: [*c]c_int) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var data = arg_data;
    _ = &data;
    ...
    var bytes: c_int = 0;
    _ = &bytes;
    var needWrite: bool = @as(c_int, 1) != 0;
    _ = &needWrite;
    while (needWrite) {
        bytes = @as(c_int, @bitCast(@as(c_int, @truncate(send(ctx.*.fd, @as(?*const anyopaque, @ptrCast(data)), @as(usize, @bitCast(@as(c_long, len))), MSG_NOSIGNAL)))));
        if (bytes == @as(c_int, 0)) {
            return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_Write", @as(c_int, 506), @as(c_uint, @bitCast(NATS_CONNECTION_CLOSED)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_CONNECTION_CLOSED))));
        } else if (bytes < @as(c_int, 0)) {
            if (__errno_location().* != @as(c_int, 11)) {
                return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_Write", @as(c_int, 542), @as(c_uint, @bitCast(NATS_IO_ERROR)), "send error: %d", __errno_location().*);
            } else if (ctx.*.useEventLoop) {
                if (n != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                    n.* = 0;
                }
            ...
            s = natsSock_WaitReady(@as(c_int, 1), ctx);
            if (s != @as(c_uint, @bitCast(NATS_OK))) return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_Write");
            continue;
        }
        if (n != @as([*c]c_int, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            n.* = bytes;
        }
        needWrite = @as(c_int, 0) != 0;
    }
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsSock_WriteFully(arg_ctx: [*c]natsSockCtx, arg_data: [*c]const u8, arg_len: c_int) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var data = arg_data;
    _ = &data;
    ...
    while (true) {
        ...
        if (s == @as(c_uint, @bitCast(NATS_OK))) {
            data += @as(usize, @bitCast(@as(isize, @intCast(n))));
            len -= n;
            if (len == @as(c_int, 0)) return @as(c_uint, @bitCast(NATS_OK));
        }
        if (!(s == @as(c_uint, @bitCast(NATS_OK)))) break;
    ...
    if (s == @as(c_uint, @bitCast(NATS_TIMEOUT))) {
        ...
        ctx.*.fdActive = @as(c_int, 0) != 0;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsSock_WriteFully");
}
pub extern fn natsSock_Flush(fd: natsSock) natsStatus;
pub export fn natsSock_Close(arg_fd: natsSock) void {
    var fd = arg_fd;
    _ = &fd;
    _closeFd(fd);
}
pub export fn natsSock_SetCommonTcpOptions(arg_fd: natsSock) natsStatus {
    var fd = arg_fd;
    _ = &fd;
    var l: struct_linger = undefined;
    _ = &l;
    var yes: c_int = 1;
    _ = &yes;
    if (setsockopt(fd, IPPROTO_TCP, @as(c_int, 1), @as(?*const anyopaque, @ptrCast(@as([*c]const u8, @ptrCast(@alignCast(&yes))))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))))) == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_SetCommonTcpOptions", @as(c_int, 63), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "setsockopt TCP_NO_DELAY error: %d", __errno_location().*);
    yes = 1;
    if (setsockopt(fd, @as(c_int, 1), @as(c_int, 2), @as(?*const anyopaque, @ptrCast(@as([*c]const u8, @ptrCast(@alignCast(&yes))))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))))) == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_SetCommonTcpOptions", @as(c_int, 69), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "setsockopt SO_REUSEADDR error: %d", __errno_location().*);
    l.l_onoff = 1;
    l.l_linger = 0;
    if (setsockopt(fd, @as(c_int, 1), @as(c_int, 13), @as(?*anyopaque, @ptrCast(&l)), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_linger)))))) == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_SetCommonTcpOptions", @as(c_int, 76), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "setsockopt SO_LINGER error: %d", __errno_location().*);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsSock_Shutdown(arg_fd: natsSock) void {
    var fd = arg_fd;
    _ = &fd;
    if (fd != -@as(c_int, 1)) {
...
pub export fn natsSock_ClearDeadline(arg_ctx: [*c]natsSockCtx) void {
    var ctx = arg_ctx;
    _ = &ctx;
    natsDeadline_Clear(&ctx.*.readDeadline);
    natsDeadline_Clear(&ctx.*.writeDeadline);
}
pub export fn natsSock_InitDeadline(arg_ctx: [*c]natsSockCtx, arg_timeout: i64) void {
    var ctx = arg_ctx;
    _ = &ctx;
...
pub export fn natsSock_GetLocalIPAndPort(arg_ctx: [*c]natsSockCtx, arg_ip: [*c][*c]u8, arg_port: [*c]c_int) natsStatus {
    var ctx = arg_ctx;
    _ = &ctx;
    var ip = arg_ip;
    _ = &ip;
    ...
    _ = &addr;
    var addrLen: natsSockLen = @as(natsSockLen, @bitCast(@as(c_uint, @truncate(@sizeOf(struct_sockaddr_storage)))));
    _ = &addrLen;
    var localIP: [64]u8 = undefined;
    _ = &localIP;
    var laddr: ?*anyopaque = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    _ = &laddr;
    var fam: c_int = undefined;
    _ = &fam;
    ip.* = null;
    port.* = 0;
    if (getsockname(ctx.*.fd, __SOCKADDR_ARG{
        .__sockaddr__ = @as([*c]struct_sockaddr, @ptrCast(@alignCast(&addr))),
    }, &addrLen) != @as(c_int, 0)) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_GetLocalIPAndPort", @as(c_int, 634), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "getsockname error: %d", __errno_location().*);
    fam = @as(c_int, @bitCast(@as(c_uint, @as([*c]struct_sockaddr, @ptrCast(@alignCast(&addr))).*.sa_family)));
    if (fam == @as(c_int, 2)) {
        var addr4: [*c]struct_sockaddr_in = @as([*c]struct_sockaddr_in, @ptrCast(@alignCast(&addr)));
        _ = &addr4;
        laddr = @as(?*anyopaque, @ptrCast(&addr4.*.sin_addr));
        port.* = @as(c_int, @bitCast(@as(c_uint, ntohs(addr4.*.sin_port))));
    } else if (fam == @as(c_int, 10)) {
        var addr6: [*c]struct_sockaddr_in6 = @as([*c]struct_sockaddr_in6, @ptrCast(@alignCast(&addr)));
        _ = &addr6;
        laddr = @as(?*anyopaque, @ptrCast(&addr6.*.sin6_addr));
        port.* = @as(c_int, @bitCast(@as(c_uint, ntohs(addr6.*.sin6_port))));
    } else {
        return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_GetLocalIPAndPort", @as(c_int, 654), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "unknown INET family: %d", fam);
    }
    if (inet_ntop(fam, laddr, @as([*c]u8, @ptrCast(@alignCast(&localIP))), @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf([64]u8)))))) == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_GetLocalIPAndPort", @as(c_int, 658), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "inet_ntop error: %d", __errno_location().*);
    if ((blk: {
        const tmp = strdup(@as([*c]u8, @ptrCast(@alignCast(&localIP))));
        ip.* = tmp;
        break :blk tmp;
    }) == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/comsock.c", "natsSock_GetLocalIPAndPort", @as(c_int, 661), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    return @as(c_uint, @bitCast(NATS_OK));
}
...
pub fn _closeFd(arg_fd: natsSock) callconv(.c) void {
    var fd = arg_fd;
    _ = &fd;
    if (fd != -@as(c_int, 1)) {
        _ = close(fd);
...
pub const MAX_HOST_NAME = @as(c_int, 256);

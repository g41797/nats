...
pub export fn natsMutex_Create(arg_newMutex: [*c][*c]natsMutex) natsStatus {
    var newMutex = arg_newMutex;
    _ = &newMutex;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    var attr: pthread_mutexattr_t = undefined;
    _ = &attr;
    var m: [*c]natsMutex = @as([*c]natsMutex, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsMutex)))));
    _ = &m;
    if (m == @as([*c]natsMutex, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/unix/mutex.c", "natsMutex_Create", @as(c_int, 25), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    if (pthread_mutexattr_init(&attr) != @as(c_int, 0)) {
        free(@as(?*anyopaque, @ptrCast(m)));
        return nats_setErrorReal("./src/submodules/nats.c/src/unix/mutex.c", "natsMutex_Create", @as(c_int, 31), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "pthread_mutexattr_init error: %d", __errno_location().*);
    }
    if (pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE) != @as(c_int, 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/unix/mutex.c", "natsMutex_Create", @as(c_int, 37), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "pthread_mutexattr_settype error: %d", __errno_location().*);
    }
    if ((s == @as(c_uint, @bitCast(NATS_OK))) and (pthread_mutex_init(m, &attr) != @as(c_int, 0))) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/unix/mutex.c", "natsMutex_Create", @as(c_int, 44), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "pthread_mutex_init error: %d", __errno_location().*);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        newMutex.* = m;
    } else {
        free(@as(?*anyopaque, @ptrCast(m)));
        _ = pthread_mutexattr_destroy(&attr);
    }
    return s;
}
// ./src/submodules/nats.c/src/unix/mutex.c:80:21: warning: TODO implement translation of stmt class GCCAsmStmtClass

// ./src/submodules/nats.c/src/unix/mutex.c:68:1: warning: unable to translate function, demoted to extern
pub extern fn natsMutex_Lock(arg_m: [*c]natsMutex) void;
pub export fn natsMutex_TryLock(arg_m: [*c]natsMutex) bool {
    var m = arg_m;
    _ = &m;
    if (pthread_mutex_trylock(m) == @as(c_int, 0)) return @as(c_int, 1) != 0;
    return @as(c_int, 0) != 0;
}
pub export fn natsMutex_Unlock(arg_m: [*c]natsMutex) void {
    var m = arg_m;
    _ = &m;
...
pub export fn natsMutex_Destroy(arg_m: [*c]natsMutex) void {
    var m = arg_m;
    _ = &m;
    if (m == @as([*c]natsMutex, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;

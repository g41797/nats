...
pub export fn natsSock_WaitReady(arg_waitMode: c_int, arg_ctx: [*c]natsSockCtx) natsStatus {
    var waitMode = arg_waitMode;
    _ = &waitMode;
    var ctx = arg_ctx;
    _ = &ctx;
    var deadline: [*c]natsDeadline = &ctx.*.writeDeadline;
    _ = &deadline;
    var pfd: struct_pollfd = undefined;
    _ = &pfd;
    ...
    if (deadline != @as([*c]natsDeadline, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        timeout = natsDeadline_GetTimeout(deadline);
    }
    res = poll(&pfd, @as(nfds_t, @bitCast(@as(c_long, @as(c_int, 1)))), timeout);
    if (res == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/unix/sock.c", "natsSock_WaitReady", @as(c_int, 56), @as(c_uint, @bitCast(NATS_IO_ERROR)), "poll error: %d", __errno_location().*) else if (res == @as(c_int, 0)) return nats_setErrorReal("./src/submodules/nats.c/src/unix/sock.c", "natsSock_WaitReady", @as(c_int, 58), @as(c_uint, @bitCast(NATS_TIMEOUT)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_TIMEOUT))));
...
pub export fn natsSock_SetBlocking(arg_fd: natsSock, arg_blocking: bool) natsStatus {
    var fd = arg_fd;
    _ = &fd;
    ...
    if ((blk: {
        ...
        flags = tmp;
        break :blk tmp;
    }) == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/unix/sock.c", "natsSock_SetBlocking", @as(c_int, 69), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "fcntl error: %d", __errno_location().*);
    if (blocking) {
        flags &= ~@as(c_int, 2048);
    } else {
        flags |= @as(c_int, 2048);
    }
    if (fcntl(fd, @as(c_int, 4), flags) == -@as(c_int, 1)) return nats_setErrorReal("./src/submodules/nats.c/src/unix/sock.c", "natsSock_SetBlocking", @as(c_int, 77), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "fcntl error: %d", __errno_location().*);
    return @as(c_uint, @bitCast(NATS_OK));
}
pub export fn natsSock_IsConnected(arg_fd: natsSock) bool {
    var fd = arg_fd;
    _ = &fd;
    var res: c_int = undefined;
    _ = &res;
    var @"error": c_int = 0;
    _ = &@"error";
    var errorLen: socklen_t = @as(socklen_t, @bitCast(@as(c_uint, @truncate(@sizeOf(c_int)))));
    _ = &errorLen;
    res = getsockopt(fd, @as(c_int, 1), @as(c_int, 4), @as(?*anyopaque, @ptrCast(&@"error")), &errorLen);
    if ((res == -@as(c_int, 1)) or (@"error" != @as(c_int, 0))) return @as(c_int, 0) != 0;
    return @as(c_int, 1) != 0;
}
...
pub export fn natsSock_Flush(arg_fd: natsSock) natsStatus {
    var fd = arg_fd;
    _ = &fd;
    if (fsync(fd) != @as(c_int, 0)) return nats_setErrorReal("./src/submodules/nats.c/src/unix/sock.c", "natsSock_Flush", @as(c_int, 102), @as(c_uint, @bitCast(NATS_IO_ERROR)), "Error flushing socket. Error: %d", __errno_location().*);
    return @as(c_uint, @bitCast(NATS_OK));
}
...
pub export fn nats_initForOS() void {}

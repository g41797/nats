...
pub export fn natsCondition_Create(arg_cond: [*c][*c]natsCondition) natsStatus {
    var cond = arg_cond;
    _ = &cond;
    var c: [*c]natsCondition = @as([*c]natsCondition, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsCondition)))));
    _ = &c;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (c == @as([*c]natsCondition, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/unix/cond.c", "natsCondition_Create", @as(c_int, 28), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    if (pthread_cond_init(c, null) != @as(c_int, 0)) {
        s = nats_setErrorReal("./src/submodules/nats.c/src/unix/cond.c", "natsCondition_Create", @as(c_int, 31), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "pthread_cond_init error: %d", __errno_location().*);
    }
...
pub export fn natsCondition_Wait(arg_cond: [*c]natsCondition, arg_mutex: [*c]natsMutex) void {
    var cond = arg_cond;
    _ = &cond;
    var mutex = arg_mutex;
    _ = &mutex;
    if (pthread_cond_wait(cond, mutex) != @as(c_int, 0)) {
        abort();
    }
}
pub export fn natsCondition_TimedWait(arg_cond: [*c]natsCondition, arg_mutex: [*c]natsMutex, arg_timeout: i64) natsStatus {
    var cond = arg_cond;
    _ = &cond;
    var mutex = arg_mutex;
    _ = &mutex;
    ...
    _ = &timeout;
    return _timedWait(cond, mutex, @as(c_int, 0) != 0, timeout);
}
pub export fn natsCondition_AbsoluteTimedWait(arg_cond: [*c]natsCondition, arg_mutex: [*c]natsMutex, arg_absoluteTime: i64) natsStatus {
    var cond = arg_cond;
    _ = &cond;
    var mutex = arg_mutex;
    _ = &mutex;
    var absoluteTime = arg_absoluteTime;
    _ = &absoluteTime;
    return _timedWait(cond, mutex, @as(c_int, 1) != 0, absoluteTime);
}
pub export fn natsCondition_Signal(arg_cond: [*c]natsCondition) void {
    var cond = arg_cond;
    _ = &cond;
    if (pthread_cond_signal(cond) != @as(c_int, 0)) {
        abort();
    }
}
pub export fn natsCondition_Broadcast(arg_cond: [*c]natsCondition) void {
    var cond = arg_cond;
    _ = &cond;
    if (pthread_cond_broadcast(cond) != @as(c_int, 0)) {
        abort();
    }
}
pub export fn natsCondition_Destroy(arg_cond: [*c]natsCondition) void {
    var cond = arg_cond;
    _ = &cond;
    if (cond == @as([*c]natsCondition, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    _ = pthread_cond_destroy(cond);
    free(@as(?*anyopaque, @ptrCast(cond)));
}
...
pub fn _timedWait(arg_cond: [*c]natsCondition, arg_mutex: [*c]natsMutex, arg_isAbsolute: bool, arg_timeout: i64) callconv(.c) natsStatus {
    var cond = arg_cond;
    _ = &cond;
    var mutex = arg_mutex;
    _ = &mutex;
    var isAbsolute = arg_isAbsolute;
    _ = &isAbsolute;
    var timeout = arg_timeout;
    _ = &timeout;
    ...
    _ = &target;
    if (timeout <= @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) return @as(c_uint, @bitCast(NATS_TIMEOUT));
    target = if (@as(c_int, @intFromBool(isAbsolute)) != 0) timeout else nats_setTargetTime(timeout);
    ts.tv_sec = @divTrunc(target, @as(i64, @bitCast(@as(c_long, @as(c_int, 1000)))));
    ts.tv_nsec = @import("std").zig.c_translation.signedRemainder(target, @as(i64, @bitCast(@as(c_long, @as(c_int, 1000))))) * @as(i64, @bitCast(@as(c_long, @as(c_int, 1000000))));
    if (ts.tv_nsec >= @as(c_long, 1000000000)) {
        ts.tv_sec += 1;
        ts.tv_nsec -= @as(__syscall_slong_t, @bitCast(@as(c_long, 1000000000)));
    }
    r = pthread_cond_timedwait(cond, mutex, &ts);
    if (r == @as(c_int, 0)) return @as(c_uint, @bitCast(NATS_OK));
    if (r == @as(c_int, 110)) return @as(c_uint, @bitCast(NATS_TIMEOUT));
    return nats_setErrorReal("./src/submodules/nats.c/src/unix/cond.c", "_timedWait", @as(c_int, 77), @as(c_uint, @bitCast(NATS_SYS_ERROR)), "pthread_cond_timedwait error: %d", __errno_location().*);
}
...
// ./src/submodules/nats.c/src/unix/../util.h:71:9

...
pub export fn nats_Now() i64 {
    var ts: struct_timespec = undefined;
    _ = &ts;
    if (clock_gettime(@as(c_int, 0), &ts) != @as(c_int, 0)) {
        abort();
    }
    return (@as(i64, @bitCast(ts.tv_sec)) * @as(i64, @bitCast(@as(c_long, @as(c_int, 1000))))) + @divTrunc(@as(i64, @bitCast(ts.tv_nsec)), @as(i64, @bitCast(@as(c_long, @as(c_int, 1000000)))));
}
pub export fn nats_NowInNanoSeconds() i64 {
    var ts: struct_timespec = undefined;
    _ = &ts;
    if (clock_gettime(@as(c_int, 0), &ts) != @as(c_int, 0)) {
        abort();
    }
    return (@as(i64, @bitCast(ts.tv_sec)) * @as(c_long, 1000000000)) + @as(i64, @bitCast(ts.tv_nsec));
}
...
pub export fn natsDeadline_Init(arg_deadline: [*c]natsDeadline, arg_timeout: i64) void {
    var deadline = arg_deadline;
    _ = &deadline;
    var timeout = arg_timeout;
    _ = &timeout;
    deadline.*.active = @as(c_int, 1) != 0;
    deadline.*.absoluteTime = nats_setTargetTime(timeout);
}
pub export fn natsDeadline_GetTimeout(arg_deadline: [*c]natsDeadline) c_int {
    var deadline = arg_deadline;
    _ = &deadline;
    var timeout: c_int = undefined;
    _ = &timeout;
    if (!deadline.*.active) return -@as(c_int, 1);
    timeout = @as(c_int, @bitCast(@as(c_int, @truncate(deadline.*.absoluteTime - nats_Now()))));
    if (timeout < @as(c_int, 0)) {
        timeout = 0;
    }
    return timeout;
}
pub export fn natsDeadline_Clear(arg_deadline: [*c]natsDeadline) void {
    var deadline = arg_deadline;
    _ = &deadline;
    deadline.*.active = @as(c_int, 0) != 0;
}
pub export fn nats_setTargetTime(arg_timeout: i64) i64 {
    var timeout = arg_timeout;
    _ = &timeout;
    var target: i64 = nats_Now() + timeout;
    _ = &target;

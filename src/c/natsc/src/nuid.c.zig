...
pub const struct_natsNUID = extern struct {
    pre: [12]u8 = @import("std").mem.zeroes([12]u8),
    seq: i64 = @import("std").mem.zeroes(i64),
    inc: i64 = @import("std").mem.zeroes(i64),
};
pub const natsNUID = struct_natsNUID;
pub const struct_natsLockedNUID = extern struct {
    mu: [*c]natsMutex = @import("std").mem.zeroes([*c]natsMutex),
    nuid: natsNUID = @import("std").mem.zeroes(natsNUID),
};
pub const natsLockedNUID = struct_natsLockedNUID;
pub export fn natsNUID_init() natsStatus {
    var s: natsStatus = undefined;
    _ = &s;
    var seed: c_uint = @as(c_uint, @bitCast(@as(c_int, @truncate(nats_NowInNanoSeconds()))));
    _ = &seed;
    _ = memset(@as(?*anyopaque, @ptrCast(&globalNUID)), @as(c_int, 0), @sizeOf(natsLockedNUID));
    srand(seed);
    _initCMWC(seed);
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsNUID_init");
}
pub export fn natsNUID_Next(arg_buffer: [*c]u8, arg_bufferLen: c_int) natsStatus {
    var buffer = arg_buffer;
    _ = &buffer;
    var bufferLen = arg_bufferLen;
    _ = &bufferLen;
    var s: natsStatus = undefined;
    _ = &s;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsNUID_Next");
}
pub export fn nats_Rand64() i64 {
    return _rand64(@as(c_long, 9223372036854775807));
}
pub export fn natsNUID_free() void {
...
pub var Q: [4096]u32 = @import("std").mem.zeroes([4096]u32);
pub var carry: u32 = @as(u32, @bitCast(@as(c_int, 362436)));
pub fn _rand32() callconv(.c) u32 {
    var result: u32 = 0;
    _ = &result;
    result = @as(u32, @bitCast(rand()));
    result <<= @intCast(@as(c_int, 16));
    result |= @as(u32, @bitCast(rand()));
    return result;
}
pub fn _initCMWC(arg_seed: c_uint) callconv(.c) void {
    var seed = arg_seed;
    _ = &seed;
    ...
    _ = &i;
    {
        ...
        while (i < @as(c_int, 4096)) : (i += 1) {
            Q[@as(c_uint, @intCast(i))] = _rand32();
        }
    }
    while (true) {
        carry = _rand32();
        if (!(carry >= @as(u32, @bitCast(@as(c_int, 809430660))))) break;
    }
}
pub fn _randCMWC() callconv(.c) u32 {
    const i = struct {
        var static: u32 = @as(u32, @bitCast(@as(c_int, 4096) - @as(c_int, 1)));
    };
    _ = &i;
    var t: u64 = 0;
    _ = &t;
    var a: u64 = @as(u64, @bitCast(@as(c_long, @as(c_int, 18782))));
    _ = &a;
    var r: u32 = 4294967294;
    _ = &r;
    var x: u32 = 0;
    _ = &x;
    i.static = (i.static +% @as(u32, @bitCast(@as(c_int, 1)))) & @as(u32, @bitCast(@as(c_int, 4096) - @as(c_int, 1)));
    t = (a *% @as(u64, @bitCast(@as(c_ulong, Q[i.static])))) +% @as(u64, @bitCast(@as(c_ulong, carry)));
    carry = @as(u32, @bitCast(@as(c_uint, @truncate(t >> @intCast(32)))));
    x = @as(u32, @bitCast(@as(c_uint, @truncate(t +% @as(u64, @bitCast(@as(c_ulong, carry)))))));
    if (x < carry) {
        x +%= 1;
        carry +%= 1;
    }
...
pub fn _rand64(arg_maxValue: i64) callconv(.c) i64 {
    var maxValue = arg_maxValue;
    _ = &maxValue;
    var v: i64 = undefined;
    _ = &v;
    v = @as(i64, @bitCast(@as(u64, @bitCast(@as(c_ulong, _randCMWC()))) << @intCast(32)));
    v |= @as(i64, @bitCast(@as(c_ulong, _randCMWC())));
    if (v < @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) {
        v = v * @as(i64, @bitCast(@as(c_long, -@as(c_int, 1))));
    }
    v = @import("std").zig.c_translation.signedRemainder(v, maxValue);
    return v;
}
pub var digits: [*c]const u8 = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
pub const base: c_int = 36;
pub const maxPre: i64 = 4738381338321616896;
pub const maxSeq: i64 = 3656158440062976;
pub const minInc: i64 = 33;
pub const maxInc: i64 = 333;
pub const totalLen: c_int = @as(c_int, 12) + @as(c_int, 10);
pub var globalNUID: natsLockedNUID = @import("std").mem.zeroes(natsLockedNUID);
pub fn _nextLong(arg_next: [*c]i64, arg_useCrypto: bool, arg_maxValue: i64) callconv(.c) natsStatus {
    var next = arg_next;
    _ = &next;
    var useCrypto = arg_useCrypto;
    _ = &useCrypto;
    var maxValue = arg_maxValue;
    _ = &maxValue;
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (maxValue <= @as(i64, @bitCast(@as(c_long, @as(c_int, 0))))) return nats_setErrorReal("./src/submodules/nats.c/src/nuid.c", "_nextLong", @as(c_int, 140), @as(c_uint, @bitCast(NATS_INVALID_ARG)), "Invalid argument for nextLong: %ld", maxValue);
    next.* = _rand64(maxValue);
    return s;
}
pub fn _resetSequential(arg_nuid: [*c]natsNUID) callconv(.c) natsStatus {
    var nuid = arg_nuid;
    _ = &nuid;
    var s: natsStatus = undefined;
    _ = &s;
    s = _nextLong(&nuid.*.seq, @as(c_int, 0) != 0, maxSeq);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        s = _nextLong(&nuid.*.inc, @as(c_int, 0) != 0, maxInc - minInc);
    }
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        nuid.*.inc += minInc;
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_resetSequential");
}
pub fn _randomizePrefix(arg_nuid: [*c]natsNUID) callconv(.c) natsStatus {
    var nuid = arg_nuid;
    _ = &nuid;
    var s: natsStatus = undefined;
    _ = &s;
    var r: i64 = 0;
    _ = &r;
    s = _nextLong(&r, @as(c_int, 1) != 0, maxPre);
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        var l: i64 = undefined;
        _ = &l;
        var i: c_int = @as(c_int, 12);
        _ = &i;
        {
            l = r;
            while (i > @as(c_int, 0)) : (l = @divTrunc(l, @as(i64, @bitCast(@as(c_long, base))))) {
                i -= 1;
                nuid.*.pre[@as(c_uint, @intCast(i))] = (blk: {
                    const tmp = @as(c_int, @bitCast(@as(c_int, @truncate(@import("std").zig.c_translation.signedRemainder(l, @as(i64, @bitCast(@as(c_long, base))))))));
                    if (tmp >= 0) break :blk digits + @as(usize, @intCast(tmp)) else break :blk digits - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*;
            }
        }
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_randomizePrefix");
}
pub fn _nextNUID(arg_nuid: [*c]natsNUID, arg_buffer: [*c]u8, arg_bufferLen: c_int) callconv(.c) natsStatus {
    var nuid = arg_nuid;
    _ = &nuid;
    var buffer = arg_buffer;
    _ = &buffer;
    ...
    var s: natsStatus = @as(c_uint, @bitCast(NATS_OK));
    _ = &s;
    if (bufferLen <= totalLen) return nats_setErrorReal("./src/submodules/nats.c/src/nuid.c", "_nextNUID", @as(c_int, 236), @as(c_uint, @bitCast(NATS_INSUFFICIENT_BUFFER)), "Buffer should be at least %d bytes, it is only %d bytes", totalLen, bufferLen);
    nuid.*.seq += nuid.*.inc;
    if (nuid.*.seq >= maxSeq) {
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        ...
        var i: c_int = undefined;
        _ = &i;
        _ = memcpy(@as(?*anyopaque, @ptrCast(buffer)), @as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&nuid.*.pre))))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 12)))));
        l = nuid.*.seq;
        {
            i = totalLen;
            while (i > @as(c_int, 12)) : (l = @divTrunc(l, @as(i64, @bitCast(@as(c_long, base))))) {
                i -= 1;
                (blk: {
                    const tmp = i;
    ...
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "_nextNUID");
}
...
pub const DUP_STRING = @compileError("unable to translate macro: undefined identifier `NATS_STRDUP`");
...
pub const CMWC_CYCLE = @as(c_int, 4096);
pub const CMWC_C_MAX = @import("std").zig.c_translation.promoteIntLiteral(c_int, 809430660, .decimal);
pub const NUID_PRE_LEN = @as(c_int, 12);
pub const NUID_SEQ_LEN = @as(c_int, 10);

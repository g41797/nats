...
pub export fn natsTimer_Create(arg_timer: [*c][*c]natsTimer, arg_timerCb: natsTimerCb, arg_stopCb: natsTimerStopCb, arg_interval: i64, arg_closure: ?*anyopaque) natsStatus {
    var timer = arg_timer;
    _ = &timer;
    var timerCb = arg_timerCb;
    _ = &timerCb;
    var stopCb = arg_stopCb;
    _ = &stopCb;
    ...
    _ = &s;
    var t: [*c]natsTimer = @as([*c]natsTimer, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @sizeOf(natsTimer)))));
    _ = &t;
    if (t == @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return nats_setErrorReal("./src/submodules/nats.c/src/timer.c", "natsTimer_Create", @as(c_int, 52), @as(c_uint, @bitCast(NATS_NO_MEMORY)), "%s", natsStatus_GetText(@as(c_uint, @bitCast(NATS_NO_MEMORY))));
    t.*.refs = 1;
    t.*.cb = timerCb;
    t.*.stopCb = stopCb;
    t.*.closure = closure;
    ...
    if (s == @as(c_uint, @bitCast(NATS_OK))) {
        t.*.stopped = @as(c_int, 1) != 0;
        nats_resetTimer(t, interval);
        timer.* = t;
    } else {
        _freeTimer(t);
    }
    return if (s == @as(c_uint, @bitCast(NATS_OK))) s else nats_updateErrStack(s, "natsTimer_Create");
}
pub export fn natsTimer_Stop(arg_timer: [*c]natsTimer) void {
    var timer = arg_timer;
    _ = &timer;
    if (timer == @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    nats_stopTimer(timer);
}
pub export fn natsTimer_Reset(arg_timer: [*c]natsTimer, arg_interval: i64) void {
    var timer = arg_timer;
    _ = &timer;
    var interval = arg_interval;
    _ = &interval;
...
pub export fn natsTimer_Release(arg_t: [*c]natsTimer) void {
    var t = arg_t;
    _ = &t;
    var refs: c_int = 0;
    _ = &refs;
...
pub export fn natsTimer_Destroy(arg_timer: [*c]natsTimer) void {
    var timer = arg_timer;
    _ = &timer;
    if (timer == @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
    nats_stopTimer(timer);
...
pub fn _freeTimer(arg_t: [*c]natsTimer) callconv(.c) void {
    var t = arg_t;
    _ = &t;
    if (t == @as([*c]natsTimer, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return;
